    ...
    loop {
        ...
        for src in domtree.cfg_rpo().copied() {
            ...
            for edge in &src_summary.dests {
                ...
                // Bail out instead of panicking when a frontend creates
                // mismatched block parameter counts (e.g. explicit + implicit
                // SSA params for the same loop counter variable).  Skipping
                // the edge means this phi won't be simplified, but the IR
                // remains correct — the pass is purely an optimization.
                if edge.args.len() != dst_formals.len() {
                    continue;
                }
