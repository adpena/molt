...
impl GreedyRewriteStrictness {
    ...
    fn to_raw(self) -> MlirGreedyRewriteStrictness {
        match self {
            Self::AnyOp => MlirGreedyRewriteStrictness_MLIR_GREEDY_REWRITE_STRICTNESS_ANY_OP,
            Self::ExistingAndNewOps => {
                MlirGreedyRewriteStrictness_MLIR_GREEDY_REWRITE_STRICTNESS_EXISTING_AND_NEW_OPS
            }
            Self::ExistingOps => {
...
impl GreedySimplifyRegionLevel {
    ...
    fn to_raw(self) -> MlirGreedySimplifyRegionLevel {
        match self {
            Self::Disabled => {
                MlirGreedySimplifyRegionLevel_MLIR_GREEDY_SIMPLIFY_REGION_LEVEL_DISABLED
            }
            Self::Normal => MlirGreedySimplifyRegionLevel_MLIR_GREEDY_SIMPLIFY_REGION_LEVEL_NORMAL,
            Self::Aggressive => {
                MlirGreedySimplifyRegionLevel_MLIR_GREEDY_SIMPLIFY_REGION_LEVEL_AGGRESSIVE
            }
        }
...
impl GreedyRewriteDriverConfig {
    ...
    pub fn set_strictness(&self, strictness: GreedyRewriteStrictness) {
        unsafe { mlirGreedyRewriteDriverConfigSetStrictness(self.raw, strictness.to_raw()) }
    }
    ...
    pub fn set_region_simplification_level(&self, level: GreedySimplifyRegionLevel) {
        unsafe {
            mlirGreedyRewriteDriverConfigSetRegionSimplificationLevel(self.raw, level.to_raw())
        }
    }
...
mod tests {
    ...
    #[test]
    fn public_enums_convert_through_mlir_sys_raw_types() {
        assert_eq!(
            size_of::<GreedyRewriteStrictness>(),
            size_of::<u32>(),
            "the public enum representation must remain stable",
        );
        assert_eq!(
            size_of::<GreedySimplifyRegionLevel>(),
            size_of::<u32>(),
            "the public enum representation must remain stable",
        );
        assert_eq!(
            GreedyRewriteStrictness::AnyOp.to_raw(),
            MlirGreedyRewriteStrictness_MLIR_GREEDY_REWRITE_STRICTNESS_ANY_OP
        );
        assert_eq!(
            GreedySimplifyRegionLevel::Aggressive.to_raw(),
            MlirGreedySimplifyRegionLevel_MLIR_GREEDY_SIMPLIFY_REGION_LEVEL_AGGRESSIVE
        );
    }
