...
def main() -> int:
    ...
    # 1) curvelet bank
    Be, Bt = (
        wf.curvelet_directional_B(wf.CurveletBankConfig()),
        T.curvelet_directional_B(T.CurveletBankConfig()),
    )
    ...
    sdf_e = wf.numpy_levelset_forward(
        p,
        wf.curvelet_feats(coords, wf.LevelSetConfig().build_B()),
        p["code"][0],
        wf.LevelSetConfig(**_cfg_kw()),
    )
    sdf_t = T.numpy_levelset_forward(
        p, T.curvelet_feats(coords, tcfg.build_B()), p["code"][0], tcfg
    )
