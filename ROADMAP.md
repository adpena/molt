...
- Implemented: recursive loop now writes tool-promotion events/distillations
  under the canonical Symphony state root and readiness exposes
...
- Implemented: launchd-managed Symphony is now self-healing under operator
...
Planned milestones:
- Implemented(async-runtime, owner:runtime, milestone:RT2, priority:P0, status:done): Rust event loop + I/O poller with cancellation propagation and deterministic scheduling guarantees now back the `asyncio` core. Pipe transports (`connect_read_pipe`/`connect_write_pipe`) are implemented with 11 intrinsics; 42 Rust intrinsics for Future/Event/Lock/Semaphore/Queue state machines landed; WASM capability gating for 6 I/O operations is complete; Transport/Protocol base classes are added; 3.13/3.14 version-specific APIs are gated.
...
- Implemented: Molt-specific runtime helpers now live under `moltlib`, including `moltlib.molt_db`, `moltlib.concurrency`, `moltlib.net`, and `moltlib.asgi`; `molt.molt_db`, `molt.net`, `molt.concurrency`, and `molt.asgi` remain as compatibility shims, while the root `molt` package stays reserved for compiler/runtime core.
- Implemented: `ast.parse` / `ast.walk` / `ast.get_docstring` route through Rust intrinsics (`molt_ast_parse`, `molt_ast_walk`, `molt_ast_get_docstring`); wasm now explicitly gates parser-backed `ast.parse` with `RuntimeError` while preserving intrinsic-backed `ast.walk`/`ast.get_docstring`.
...
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): scan-driven parity push expanded CPython source-compat wrappers/macros in `include/molt/Python.h` (type/ref helpers, tuple/list/dict helpers, memory allocators, GIL/thread shims, compare/call builders, `Py_BuildValue` subset, capsule/buffer helpers, module/capsule import helpers, `PyArg_UnpackTuple`, iter/number/float helpers, `PyObject_Dir`, `PyObject_Format`, `PyObject_GetIter`, `PyObject_Length`, `PyObject_Size`, `PyTraceMalloc_Track`, `PyTraceMalloc_Untrack`, `PyTupleObject`, `PyVarObject`, `Py_SIZE`, `Py_uhash_t`, `Py_HUGE_VAL`, `PyUnicode_AsEncodedString`, `PyUnicode_FromWideChar`, set/complex checks) and landed initial NumPy compatibility headers (`include/numpy/*` + `import_array*` capsule wiring + DType/scalar helper stubs) plus a minimal `datetime.h` shim lane (`PyDateTimeAPI`/`PyDateTime_IMPORT`/basic checks); the latest libmolt tranche now routes `PyCapsule_*` through runtime-native `molt_capsule_*` wrappers instead of header-local emulation. Follow-on scan hardening now filters local `#define Py*`/`static Py*` helpers, adds a new `include/numpy/dtype_api.h` compatibility lane, broadens NumPy 2.x shims across DType metadata, array-method flags/types, order/casting converters, descriptor promotion/adaptation helpers, writeback/buffer helpers, iterator/neighborhood structs, public dtype aliases, and high-frequency array accessors (`PyArray_BASE`, `PyArray_DTYPE`, `PyArray_IS*`, `PyDataType_NAMES/FIELDS/ISUNSIZED`, `PyArray_NewLikeArray`, `PyArray_View`, `PyArray_CanCastTypeTo`, `PyArray_CanCastArrayTo`, `PyArray_OrderConverter`, `PyArray_PromoteTypes`, `PyArray_SafeCast`, `PyArray_Newshape`, `PyArray_CopyConverter`, `PyArray_DTypeFromObject`, `PyArray_DTypeOrDescrConverterOptional`, `PyArray_ExtractDTypeAndDescriptor`, `PyArray_OverflowMultiplyList`, `PyArray_DiscardWritebackIfCopy`, `PyArray_CheckFromAny`, `PyArray_CopyAnyInto`, `PyUFunc_Type`, `PyUFunc_None`, `PyArrayInterface`, `PyArrayNeighborhoodIterObject`, `PyArrayNeighborhoodIter_Next`, `PyArrayNeighborhoodIter_Reset`, `PyArray_Any`, `PyArray_BufferConverter`, `PyArray_ByteorderConverter`, `PyArray_CFloatDType`, `PyArray_CDoubleDType`, `PyArray_CLongDoubleDType`, `PyUFunc_AddLoopsFromSpecs`), and adds another source-compat tranche for `PyObject_Vectorcall`, lock/mutex shims, `PyNumberMethods`, `PyTuple_Type`/`PyType_Type`, `PyType_Check`, `PyUnicode_GET_LENGTH`, `Py_VISIT`, `PyArray_GetItemFunc`, `PyArrayMapIterObject`, internal NumPy compile helpers such as `PyArray_CopyObject`, `PyArray_PromoteDTypeSequence`, `PyArray_GetCastingImpl`, `PyArray_CastDescrToDType`, `PyArray_AssignRawScalar`, `PyArray_GetStridedCopyFn`, `PyArray_CastRawArrays`, `PyArray_PrepareTwoRawArrayIter`, `PyArray_LookupSpecial(_OnInstance)`, and `PyUFunc_FromFuncAndData(_AndSignature)`, reducing scan-reported missing `Py*` tokens on current NumPy/pandas sources (NumPy `2.4.2`, pandas `3.0.1`) from `1074/27` to `734/13`.
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): latest NumPy source-compat tranche tightened `include/numpy/ndarraytypes.h`/`ndarrayobject.h` with exact `NPY_DEVICE` and `NPY_CORRELATEMODE` enums, `npy_dtype_info`, dtype aliases (`PyArray_DoubleDType`, `PyArray_DatetimeDType`, `PyArray_FloatAbstractDType`), wrapper entry points (`PyArray_Converter`, `PyArray_CorrelatemodeConverter`, `PyArray_FromAny_int`, `PyArray_FromArrayAttr(_int)`, `PyArray_Empty_int`, `PyArray_DTypeOrDescrConverterRequired`, `PyArray_DTypeFromObjectStringDiscovery`, `PyArray_EquivTypenums`, `PyArray_Flatten`, `PyArray_CumSum`, `PyArray_CumProd`, `PyArray_GenericAccumulateFunction`, `PyArray_Diagonal`), and an exact `PyTime_Check` datetime shim, with expanded `clang` smoke coverage. Archived scan deltas on 2026-03-05 moved NumPy `2.4.2` missing `734 -> 702` and pandas `3.0.1` missing `13 -> 12` (NumPy coverage `0.495 -> 0.517`, pandas `0.925 -> 0.931`); the remaining top NumPy misses are now concentrated in deeper internals such as `PyArrayIdentityHash*`, `PyArray_AddLegacyWrapping_CastingImpl`, `PyArray_ConvertToCommonType`, `PyArray_DescrHash`, and cast/transfer initialization helpers.
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): the 2026-03-06 libmolt burndown added project-shared/header-local scan filtering, `ndarrayobject.h`-level `import_array*` support, `arrayscalars.h`, `numpyconfig.h`, `utils.h`, datetime/int-format aliases and metadata shims (`npy_float64`, `NPY_INT32_FMT`, `NPY_MIN_INT64`, `NPY_MAXDIMS`, `NPY_INT64`, `PyArray_DatetimeDTypeMetaData`, `PyDataType_C_METADATA`), `NPY_UNUSED` / `NPY_TLS` / visibility helpers, token-compatible `PyArray_IsScalar`, `PyUnicode_InternFromString`, `PySys_GetObject`, and a new `structmember.h` compatibility lane backed by `Py_T_*` / `Py_READONLY` exports and fail-fast `PyMember_GetOne` / `PyMember_SetOne`. Archived scans now report NumPy `2.4.2` at `256` missing `Py*` symbols (coverage `0.604`) and pandas `3.0.1` at `0` (coverage `1.000`). Real `clang -fsyntax-only` validation now passes NumPy `limited_api1.c`, `limited_api_latest.c`, and `numpy/_core/src/multiarray/npy_static_data.c` (with `-Wno-sign-compare` to ignore upstream warning noise), plus all checked pandas `_libs/src` C files except `parser/tokenizer.c` without an extra forced `inttypes.h` include; the next NumPy core frontier is now split between generated-build blockers such as `numpy/_core/src/common/config.h` / `npy_cpu_dispatch_config.h` and deeper internal-header closure (`npy_3kcompat.h`, `frameobject.h`, richer `PyUFuncObject` field shapes, `NPY_OFF_T_PYFMT`).
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): follow-on libmolt C-API reconciliation aligned mapping/dict helper behavior across header/runtime/docs by promoting `PyDict_DelItem` / `PyDict_DelItemString` out of fail-fast stubs, adding `PyMapping_HasKey` / `PyMapping_HasKeyString`, and making runtime/native `PyMapping_Keys` / `PyMapping_Values` / `PyMapping_Items` plus `PyDict_Keys` / `PyDict_Values` / `PyDict_Items` materialize CPython-style lists instead of internal dict-view objects; the C-API matrix now also reflects the already-landed `PySequence_DelItem`, `PyMapping_Check`, and `PyByteArray_*` surface instead of stale `Missing` markers.
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): the follow-on 2026-03-06 NumPy/core-header tranche added LP64-correct `npy_int64` / `NPY_INT64` sizing, `NPY_SIZEOF_OFF_T` and pointer-size config metadata, tagged `enum NPY_TYPES`, `PyArray_ArrFuncs` `getitem`/`setitem` slots, repo `frameobject.h`, top-level `arrayobject.h` / `pymem.h` forwarding, and a more concrete `PyObject`/`PyCodeObject` compatibility shape. Public-vs-internal helper guards now let NumPy core probes step past earlier header collisions; the remaining checked frontier is concentrated in `NpyAuxData` lifecycle helpers, richer `PyUFuncObject` field layouts, and `pythoncapi-compat` thread/frame/module closure rather than the earlier `config.h`/`frameobject.h`/LP64 bootstrap gaps.
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): the latest 2026-03-06 libmolt burndown added generated NumPy config bridge headers (`_numpyconfig.h`, `config.h`, `npy_cpu_dispatch_config.h`, `numpy/npy_cpu.h`), `Python.h` coverage for `inttypes.h`, `PY_VERSION_HEX`, `PyErr_BadInternalCall`, selected weakref/unicode/private-helper shims, and NumPy-core `PYTHONCAPI_COMPAT` suppression so vendored `pythoncapi-compat` no longer dominates `_MULTIARRAYMODULE` / `_UMATHMODULE` probes. The same tranche added `NpyAuxData` lifecycle macros, `NPY_MAXARGS`, and upstream-shaped legacy `PyUFunc_Loop1d` / `PyUFuncObject` source layouts. Archived scans now report NumPy `2.4.2` at `239` missing `Py*` symbols (coverage `0.631`) and pandas `3.0.1` at `0`, while real `clang -fsyntax-only` validation now also passes pandas `parser/tokenizer.c` and NumPy `numpy/_core/src/umath/ufunc_type_resolution.c`. The checked NumPy frontier is now explicitly split between private/generated build artifacts (`arraytypes.h` in `conversion_utils.c`) and deeper internal source-shape gaps in `scalarapi.c` (`PyArray_DiscoverDTypeAndShape`, `PyArray_AssignFromCache`, `PyArray_NewFromDescr*`, `npy_dtype_info`, `NPY_NSCALARKINDS`, and `PyArrayScalar_VAL` macro-family coverage).
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): the follow-on 2026-03-06 libmolt burndown promoted additional upstream-shipped NumPy contract headers (`numpy/arrayobject.h`, `numpy/dtype_api.h`, `numpy/npy_2_compat.h`, `numpy/npy_cpu.h`, `numpy/npy_math.h`, `numpy/numpyconfig.h`, `numpy/utils.h`) and added upstream-derived overlay headers (`numpy/_public_dtype_api_table.h`, `numpy/halffloat.h`, `numpy/npy_2_complexcompat.h`, `numpy/npy_3kcompat.h`, `numpy/npy_endian.h`, `numpy/npy_no_deprecated_api.h`, `numpy/npy_os.h`, `numpy/random/bitgen.h`, `numpy/random/distributions.h`) plus a compile-focused `templ_common.h` bridge for generated-source probes. The same tranche aligned `PyArray_ArrFuncs`, dtype flags, `NPY_SELECTKIND`, `NPY_DATETIME_NUMUNITS`, arrayscalar/object source shapes, and internal-build prototype-only lanes for NumPy-owned functions. Archived scans now report NumPy `2.4.2` at `227` missing `Py*` symbols (coverage `0.649`, `420/647`, `34` contract headers consulted) and pandas `3.0.1` at `0`, while `clang -fsyntax-only` now also passes NumPy `numpy/_core/src/multiarray/scalarapi.c` and keeps `numpy/_core/src/umath/ufunc_type_resolution.c` plus pandas `parser/tokenizer.c` green. The checked NumPy frontier is now centered on deeper internal closure in `descriptor.c` (`npy_packed_static_string`, `PyDict_Merge`, `PyArray_DescrNewFromType`, `PyArray_DescrConverter2`, `PyDataType_ISDATETIME`, `PyMemoryView_Type`) instead of the earlier public-header/bootstrap gaps.
...
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): defined the libmolt extension ABI/header contract in `0217` and wired extension tooling to use it. `include/molt/molt.h` is now the explicit stable ABI tier, `Python.h` plus the small forwarding headers are documented as bounded source-compat facades, NumPy headers are documented as ecosystem overlays, `molt extension scan` now consults an explicit contract header list, and `molt extension build` records that contract in `extension_manifest.json`. Private/generated upstream headers remain out of contract.
- TODO(c-api, owner:runtime, milestone:SL3, priority:P2, status:partial): extend the initial C API shim from bootstrap wrappers to broader source-compat and ABI coverage, including the scan-driven CPython/NumPy header parity sprint in `include/molt/Python.h` and `include/numpy/*` (type/ref helpers, tuple/list/dict fast helpers, memory/thread/GIL shims, compare/call builders, `Py_BuildValue` subset, capsule/buffer and import helpers, `PyArg_UnpackTuple`, iter/number/float/set/complex helpers, capsule context helpers, `PyErr_Print`/normalization/thread-save-restore coverage, dtype/type-object exports, `PyDataType_*`/`PyDataMem_*` helpers, array flag/conversion/copy/setup shims, internal include wrappers, fail-fast `PyUFunc_*` constructor/registration surface, broader NumPy 2.x DType/method/type structs, order/casting/converter helpers, descriptor promotion/adaptation helpers, and array metadata/accessor macros). Archived C-source sdist scans on 2026-03-05 reduced NumPy `2.4.2` missing `Py*` tokens from `1193` to `873` while holding pandas `3.0.1` at `28`, and grew the exported header surface from `416` to `770` tokens; heavy ndarray allocation/ownership, transfer machinery, and real ufunc execution remain explicit fail-fast gaps pending runtime backing.
...
- TODO(wasm-parity, owner:runtime, milestone:RT2, priority:P0, status:partial): Node/V8 Zone OOM root-cause remains open for large linked artifacts, but `run_wasm.js` now auto-reexecs large modules with `--liftoff-only --no-wasm-tier-up --no-wasm-dynamic-tiering --wasm-num-compilation-tasks=1` (opt-out: `MOLT_WASM_AUTO_NODE_FLAGS=0`) so default runs avoid crash-on-start.
