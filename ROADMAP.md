...
- Implemented(runtime/tooling, owner:runtime, milestone:SL3, priority:P1, status:partial): extension import spec/load boundaries now enforce manifest ABI/capability/checksum gates (finder + loader/exec lanes) with fingerprint-aware cache invalidation for replaced artifacts, and CI includes extension wheel `build + audit + verify + publish --dry-run` matrix lanes (`linux native`, `linux cross-target`, `macos native`, `windows native`) plus a wasm-target rejection contract check on the `linux native` lane.
