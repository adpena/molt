...
def measure_molt(script):
    ...
    env = os.environ.copy()
    env["PYTHONPATH"] = "src"
    res = subprocess.run(["python3", "-m", "molt.cli", "build", script], env=env, capture_output=True, text=True)
...
def run_bench():
    ...
    for script in BENCHMARKS:
        ...
        print(f"{name:<30} | {results['CPython']:<10.4f} | {results['PyPy']:<10.4f} | {molt_time:<10.4f} | {speedup:<12.2f}x | {molt_size:.1f} KB")
