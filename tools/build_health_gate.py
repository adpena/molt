...
def check(diag: dict, log_text: str | None, th: dict) -> list[Anomaly]:
    ...
    if observed >= min_observed and hit_rate < floor and relowered_s >= waste_floor_s:
        out.append(
            Anomaly(
                "frontend_lowering_cache_hit_rate",
                f"{hit_rate:.2f} ({flc.get('hits')}/{observed})",
                f">= {floor:.2f}",
                f"{relowered_s:.0f}s re-lowered",
    ...
    if total > 0 and phase:
        ...
        if top_val / total >= dom_frac and total >= float(
            th.get("phase_dominance_min_total_s", 120)
        ):
            out.append(
                Anomaly(
                    "phase_dominance",
                    f"{top_name}={top_val:.0f}s ({top_val / total:.0%} of {total:.0f}s)",
                    f"< {dom_frac:.0%}",
                    None,
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--strict", action="store_true", help="exit nonzero on a hard invariant"
    )
    args = ap.parse_args(argv)

    if not args.diagnostics.is_file():
        print(
            f"build_health_gate: no diagnostics at {args.diagnostics}", file=sys.stderr
        )
        return 3
    try:
    ...
    if args.cold_diagnostics is not None:
        try:
            ...
            print(
                f"build_health_gate: cannot read cold diagnostics: {exc}",
                file=sys.stderr,
            )
            return 3
    ...
    print(f"\n{bar}", file=sys.stderr)
    print(
        "!! BUILD-HEALTH ATTENTION — redundant work / configured!=effective anomalies:",
        file=sys.stderr,
    )
    for a in anomalies:
