...
def run_benchmark(args: argparse.Namespace) -> dict[str, object]:
    ...
    for index, raw_path in enumerate(args.external_link_plan):
        manifest_path = Path(raw_path).expanduser().resolve(strict=True)
        try:
            resolved = read_source_extension_link_plan(
                manifest_path,
                expected_target_triple=args.target_triple or _host_target_triple(),
            )
        except (OSError, ValueError) as exc:
            raise LinkBenchmarkError(
                f"invalid source-extension link plan {manifest_path}: {exc}"
            ) from exc
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Complete ordered local source-extension link plan (checksummed inputs, "
            "loading policies, providers and retained roots)"
        ),
    )
