...
class ImportCategory(str, Enum):
    ...
    INDIRECT_CALL = "indirect_call"  # molt_call_indirectN — required for function pointers
...
IMPORT_RULES: list[tuple[str, str, ImportCategory, str]] = [
    ...
    ("wasi_snapshot_preview1", "args_sizes_get",      ImportCategory.ESSENTIAL,    "Argument count query"),
    ("wasi_snapshot_preview1", "args_get",            ImportCategory.ESSENTIAL,    "Argument retrieval"),
    ("wasi_snapshot_preview1", "environ_sizes_get",   ImportCategory.ESSENTIAL,    "Environment size query"),
    ("wasi_snapshot_preview1", "environ_get",         ImportCategory.ESSENTIAL,    "Environment retrieval"),
    ("wasi_snapshot_preview1", "clock_time_get",      ImportCategory.ESSENTIAL,    "Wall-clock / monotonic time"),
    ...
    ("wasi_snapshot_preview1", "sched_yield",         ImportCategory.ESSENTIAL,    "Cooperative yield"),
    ("env",                    "__indirect_function_table", ImportCategory.TABLE,   "Indirect call dispatch table"),

    # === IO: stdout/stderr (needed for print()) ===
    ("wasi_snapshot_preview1", "fd_write",            ImportCategory.IO_STDOUT,    "Write to fd (stdout/stderr)"),
    ("wasi_snapshot_preview1", "fd_read",             ImportCategory.IO_STDOUT,    "Read from fd (stdin stub)"),
    ("wasi_snapshot_preview1", "fd_close",            ImportCategory.IO_STDOUT,    "Close file descriptor"),
    ("wasi_snapshot_preview1", "fd_seek",             ImportCategory.IO_STDOUT,    "Seek file descriptor"),
    ("wasi_snapshot_preview1", "fd_tell",             ImportCategory.IO_STDOUT,    "Tell file descriptor position"),
    ("wasi_snapshot_preview1", "fd_prestat_get",      ImportCategory.IO_STDOUT,    "Preopened fd stat"),
    ("wasi_snapshot_preview1", "fd_prestat_dir_name", ImportCategory.IO_STDOUT,    "Preopened fd dir name"),

    # === IO: filesystem (pure-computation never uses) ===
    ("wasi_snapshot_preview1", "fd_readdir",          ImportCategory.IO_FILESYSTEM, "Read directory entries"),
    ("wasi_snapshot_preview1", "fd_filestat_get",     ImportCategory.IO_FILESYSTEM, "File stat by fd"),
    ("wasi_snapshot_preview1", "fd_filestat_set_size", ImportCategory.IO_FILESYSTEM, "Truncate file"),
    ("wasi_snapshot_preview1", "path_open",           ImportCategory.IO_FILESYSTEM, "Open file by path"),
    ("wasi_snapshot_preview1", "path_rename",         ImportCategory.IO_FILESYSTEM, "Rename path"),
    ("wasi_snapshot_preview1", "path_readlink",       ImportCategory.IO_FILESYSTEM, "Read symlink"),
    ("wasi_snapshot_preview1", "path_unlink_file",    ImportCategory.IO_FILESYSTEM, "Delete file"),
    ("wasi_snapshot_preview1", "path_create_directory", ImportCategory.IO_FILESYSTEM, "Create directory"),
    ("wasi_snapshot_preview1", "path_remove_directory", ImportCategory.IO_FILESYSTEM, "Remove directory"),
    ("wasi_snapshot_preview1", "path_filestat_get",   ImportCategory.IO_FILESYSTEM, "File stat by path"),
    ("wasi_snapshot_preview1", "poll_oneoff",         ImportCategory.IO_FILESYSTEM, "Poll for I/O events"),

    # === Process ===
    ("env", "molt_process_write_host",       ImportCategory.PROCESS,   "Write to child process stdin"),
    ("env", "molt_process_close_stdin_host", ImportCategory.PROCESS,   "Close child stdin pipe"),
    ("env", "molt_process_terminate_host",   ImportCategory.PROCESS,   "Terminate child process"),
    ...
    ("env", "molt_process_stdio_host",       ImportCategory.PROCESS,   "Access child stdio pipes"),
    ...
    ("env", "molt_socket_connect_ex_host",    ImportCategory.SOCKET, "Connect socket (extended)"),
    ...
    ("env", "molt_socket_getservbyname_host", ImportCategory.SOCKET, "Resolve service by name"),
    ("env", "molt_socket_getservbyport_host", ImportCategory.SOCKET, "Resolve service by port"),
    ("env", "molt_socket_getsockname_host",   ImportCategory.SOCKET, "Get socket address"),
...
def _parse_type_section(data: bytes, sec_offset: int, sec_size: int) -> dict[int, tuple[list, list]]:
...
def _valtype_name(vt: int) -> str:
    return {0x7F: "i32", 0x7E: "i64", 0x7D: "f32", 0x7C: "f64",
            0x7B: "v128", 0x70: "funcref", 0x6F: "externref"}.get(vt, f"0x{vt:02x}")
...
def parse_imports(wasm_path: Path) -> list[ImportInfo]:
    ...
    for idx in range(count):
        ...
        # Classify the import
        info = ImportInfo(index=idx, module=module, name=name, kind=kind, type_index=type_index)
...
def strip_imports(wasm_path: Path, output_path: Path, result: AnalysisResult) -> Path:
    ...
    print(f"Converting to WAT text form...")
    ...
    print(f"Stripping debug and name sections...")
    ...
    print(f"Stripped debug/name sections: {original_size:,} -> {stripped_size:,} bytes "
...
def print_report(result: AnalysisResult, verbose: bool = False) -> None:
    ...
    print(f"File size: {result.file_size_bytes:,} bytes ({result.file_size_bytes / 1024 / 1024:.1f} MB)")
    ...
    pct = result.strippable_count / result.total_imports * 100 if result.total_imports else 0
    ...
    print(f"  Host stub code lines saved:   ~{result.strippable_count * 2} lines in molt-wasm-host.ts")
...
def print_json(result: AnalysisResult) -> None:
    ...
    data = {
        ...
        "strippable_imports": [
            {"index": i.index, "module": i.module, "name": i.name,
             "category": i.category.value, "description": i.description}
...
def main() -> None:
    ...
    if args.strip:
        ...
        print(f"Stripped file: {stripped_result.file_size_bytes:,} bytes "
