...
class NativeCallableAbi:
    ...
    @property
    def requires_explicit_export_arity(self) -> bool:
        return self.python_callable and self.fixed_arity is None
...
def load_schema(path: Path = SOURCE) -> Schema:
    ...
    if data["schema_version"] != 2:
        raise SchemaError("native-callable ABI schema_version must be 2")
    rows = data["abi"]
    if (
    ...
    for index, raw_row in enumerate(rows):
        ...
        python_callable = row["python_callable"]
        ...
        if not isinstance(python_callable, bool):
            raise SchemaError(f"abi {name!r} python_callable must be boolean")
...
def render_python(schema: Schema) -> str:
    ...
    for abi in schema.abis:
        if abi.requires_explicit_export_arity:
            lines.append(f"        {abi.constant},\n")
    lines.extend(
        [
            "    }\n",
...
def render_javascript(schema: Schema) -> str:
    ...
    return "".join(
        [
            ...
            "// runtime/native_callable_abi.toml. DO NOT EDIT.\n\n",
            *(
                f"export const {abi.constant} = {json.dumps(abi.token)};\n"
                for abi in schema.abis
            ),
            "\n",
...
def render_rust(schema: Schema) -> str:
    ...
    lines.extend(_rust_bool_match(schema, "python_callable"))
    lines.extend(
        [
            "    }\n\n",
    ...
    lines.extend(_rust_bool_match(schema, "requires_explicit_export_arity"))
    lines.extend(
        [
            "    }\n\n",
