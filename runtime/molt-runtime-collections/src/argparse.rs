...
// === FILE: runtime/molt-runtime-collections/src/argparse.rs ===
...
fn parsed_val_to_bits(_py: &CoreGilToken, val: &ParsedVal) -> u64 {
    match val {
...
fn result_to_dict(_py: &CoreGilToken, map: HashMap<String, ParsedVal>) -> u64 {
...
fn list_to_string_vec(_py: &CoreGilToken, bits: u64) -> Result<Vec<String>, u64> {
    let obj = obj_from_bits(bits);
    if obj.is_none() {
        return Ok(Vec::new());
    }
