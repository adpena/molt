...
pub unsafe extern "C" fn PyDict_SetItem(
    ...
    let dict_bits = match bridge.pyobj_to_handle(op)    { Some(b) => b, None => return -1 };
    let key_bits  = match bridge.pyobj_to_handle(key)   { Some(b) => b, None => return -1 };
    let val_bits  = match bridge.pyobj_to_handle(value) { Some(b) => b, None => return -1 };
    drop(bridge);
    let h = hooks_or_stubs();
...
pub unsafe extern "C" fn PyDict_GetItem(
    ...
    let bridge = GLOBAL_BRIDGE.lock();
    let dict_bits = match bridge.pyobj_to_handle(op)  { Some(b) => b, None => return ptr::null_mut() };
    let key_bits  = match bridge.pyobj_to_handle(key) { Some(b) => b, None => return ptr::null_mut() };
    drop(bridge);
    let h = hooks_or_stubs();
    let val_bits = unsafe { (h.dict_get)(dict_bits, key_bits) };
    if val_bits == 0 { return ptr::null_mut(); }
...
pub unsafe extern "C" fn PyDict_GetItemString(
    ...
    if op.is_null() || key.is_null() { return ptr::null_mut(); }
    let key_obj = unsafe { crate::api::strings::PyUnicode_FromString(key) };
    if key_obj.is_null() { return ptr::null_mut(); }
