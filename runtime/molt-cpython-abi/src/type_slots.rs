//! CPython 3.12 stable-ABI type-slot number authority.
//!
//! Values are the public, append-only ids from CPython `Include/typeslots.h`.
//! `tools/gen_cpython_type_slots.py` projects this one Rust authority into the
//! C header consumed by source extensions. Never duplicate these numbers in
//! `Python.h` or type-object consumers.
