...
unsafe extern "C" {
    fn __molt_logging_runtime_state_get_or_init(
        key_ptr: *const u8,
        key_len: usize,
...
pub fn runtime_state_get_or_init(
    ...
    unsafe { __molt_logging_runtime_state_get_or_init(key.as_ptr(), key.len(), init, clear, drop) }
}
...
pub fn raise_exception<T: ExceptionSentinel>(_py: &CoreGilToken, type_name: &str, msg: &str) -> T {
    let bits = unsafe {
        __molt_logging_raise_exception(type_name.as_ptr(), type_name.len(), msg.as_ptr(), msg.len())
    };
    T::from_bits(bits)
}
