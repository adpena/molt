...
// === FILE: molt-runtime-math/src/math.rs ===
//
// Python `math` and `statistics` module intrinsics, extracted from the
...
// ---------------------------------------------------------------------------
// Compatibility wrappers: match monolith-style API over bridge functions
// ---------------------------------------------------------------------------

/// Wraps bridge::molt_iter — monolith version has no _py parameter.
#[inline]
fn compat_molt_iter(_py: &PyToken, bits: u64) -> u64 {
    molt_iter(_py, bits)
}

/// Wraps bridge::molt_iter_next — monolith returns u64 (None check via exception_pending).
#[inline]
fn compat_molt_iter_next(_py: &PyToken, iter_bits: u64) -> u64 {
...
/// Wraps bridge::raise_not_iterable — monolith is generic, bridge is not.
#[inline]
fn compat_raise_not_iterable<T: ExceptionSentinel>(_py: &PyToken, bits: u64) -> T {
    let _ = raise_not_iterable(_py, bits);
    T::from_bits(0)
}

/// Wraps bridge::molt_is_callable — monolith returns u64 (bool as bits).
#[inline]
fn compat_molt_is_callable(_py: &PyToken, bits: u64) -> bool {
    molt_is_callable(_py, bits)
}
...
/// Wraps bridge::molt_mul — monolith has no _py.
#[inline]
fn compat_molt_mul(_py: &PyToken, a: u64, b: u64) -> u64 {
    molt_mul(_py, a, b)
}

/// Wraps bridge::bigint_to_inline — monolith returns Option<i64>, bridge returns u64.
#[inline]
fn compat_bigint_to_inline(_py: &PyToken, value: &BigInt) -> Option<i64> {
    let bits = bigint_to_inline(_py, value);
    if bits != 0 {
        // The bridge returns MoltObject int bits; extract the i64
...
/// Wraps bridge::bigint_bits — monolith takes BigInt by value, bridge takes by ref.
#[inline]
fn compat_bigint_bits(_py: &PyToken, value: BigInt) -> u64 {
...
fn render_float(_py: &PyToken, value: f64) -> String {
...
fn coerce_real(_py: &PyToken, val_bits: u64) -> Option<RealValue> {
...
fn coerce_to_f64(_py: &PyToken, value: RealValue) -> Option<f64> {
    match value {
...
fn check_finite_round(_py: &PyToken, value: f64) -> Option<()> {
    if value.is_nan() {
...
fn int_bits_from_f64_trunc(_py: &PyToken, value: f64) -> u64 {
    let big = bigint_from_f64_trunc(value);
    if let Some(i) = compat_bigint_to_inline(_py, &big) {
        MoltObject::from_int(i).bits()
    } else {
        compat_bigint_bits(_py, big)
    }
}
...
fn round_float_bits(_py: &PyToken, value: f64, mode: RoundMode) -> Option<u64> {
...
fn tuple2_bits(_py: &PyToken, first_bits: u64, second_bits: u64) -> u64 {
...
fn binary_type_error(_py: &PyToken, lhs_bits: u64, rhs_bits: u64, op: &str) -> u64 {
    let msg = format!(
...
fn math_domain_error(_py: &PyToken) -> u64 {
    raise_exception::<_>(_py, "ValueError", "math domain error")
}

fn log_domain_error(_py: &PyToken, got: Option<f64>) -> u64 {
...
fn log1p_domain_error(_py: &PyToken, value: f64) -> u64 {
    let rendered = render_float(_py, value);
...
fn statistics_mean_value(_py: &PyToken, data_bits: u64) -> Option<f64> {
...
fn statistics_stdev_value(_py: &PyToken, data_bits: u64, xbar_bits: u64) -> Option<f64> {
...
fn statistics_multimode_value(_py: &PyToken, data_bits: u64) -> Option<u64> {
    let values = collect_values_vec(_py, data_bits)?;
    if values.is_empty() {
...
fn statistics_harmonic_mean_value(_py: &PyToken, data_bits: u64) -> Option<f64> {
    let values = collect_real_vec(_py, data_bits)?;
    if values.is_empty() {
        raise_exception::<()>(
            _py,
...
fn statistics_geometric_mean_value(_py: &PyToken, data_bits: u64) -> Option<f64> {
    let values = collect_real_vec(_py, data_bits)?;
    if values.is_empty() {
        raise_exception::<()>(
            _py,
...
fn statistics_covariance_value(_py: &PyToken, x_bits: u64, y_bits: u64) -> Option<f64> {
...
fn statistics_correlation_value(_py: &PyToken, x_bits: u64, y_bits: u64) -> Option<f64> {
    let x_values = collect_real_vec(_py, x_bits)?;
    let y_values = collect_real_vec(_py, y_bits)?;
    if x_values.len() != y_values.len() {
        raise_exception::<()>(
...
fn statistics_coerce_elem_fast_f64(_py: &PyToken, val_bits: u64, name: &str) -> Option<f64> {
...
fn statistics_seed_type_error<T>(_py: &PyToken) -> Option<T> {
...
fn statistics_seed_bigint(_py: &PyToken, seed_bits: u64) -> Option<BigInt> {
...
fn statistics_normal_dist_sample_count(_py: &PyToken, n_bits: u64) -> Option<usize> {
...
fn materialize_statistics_slice(
    ...
    let slice_bits = molt_slice_new(start_obj, end_obj, none_bits);
    if exception_pending(_py) {
        return None;
    }
    let sliced_bits = molt_index(data_bits, slice_bits);
