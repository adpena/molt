// Regex byte-matching primitives (re module support).
//
// Moved out of `molt-runtime/src/builtins/functions_re.rs` into the
// `molt-runtime-regex` leaf crate so the byte-matching ABI lives beside the
// higher-level compiled-regex engine.  The god-crate keeps only the `this`
// easter-egg text and the shared `CharClassParse` alias (fnmatch/glob).
//
// Bridge translation from the in-tree copy is mechanical:
//   * runtime primitives -> `crate::bridge::*` (via `super::*`) and
//     `molt_runtime_core::prelude`.
//   * `with_gil_entry_nopanic!` -> `molt_runtime_core::with_core_gil!`.
//   * the god-crate `iter_next_pair` helper (which consumed a raw-bits
//     `molt_iter_next`) -> a crate-local `iter_next_pair` over the bridge's
//     `Option`-returning `molt_iter_next`, matching the existing satellite
//     idiom in `verbose_backref.rs`.
...
/// Advance one `(value, done)` step over a Molt iterator, matching the in-tree
/// `builtins::functions::iter_next_pair`.  The bridge's `molt_iter_next`
/// returns `None` on StopIteration/error and `Some(bits)` for a yielded pair
/// tuple `(value, done_bool)`.
fn iter_next_pair(_py: &CoreGilToken, iter_bits: u64) -> Result<(u64, bool), u64> {
    let Some(pair_bits) = molt_iter_next(_py, iter_bits) else {
...
/// Collect every item of an iterable into a `Vec<String>`, matching the in-tree
/// `builtins::functions::iterable_to_string_vec`.
fn iterable_to_string_vec(_py: &CoreGilToken, values_bits: u64) -> Result<Vec<String>, u64> {
...
pub(super) fn re_literal_matches_impl(segment: &str, literal: &str, flags: i64) -> bool {
...
pub(super) fn re_is_ascii_digit(ch: char) -> bool {
    ch.is_ascii_digit()
}

pub(super) fn re_is_ascii_alpha(ch: char) -> bool {
    ch.is_ascii_alphabetic()
}

pub(super) fn re_is_space(ch: char) -> bool {
    matches!(ch, ' ' | '\t' | '\n' | '\r' | '\u{000C}' | '\u{000B}')
}

pub(super) fn re_is_word_char(ch: &str, flags: i64) -> bool {
...
pub(super) fn re_category_matches_impl(ch: &str, category: &str, flags: i64) -> bool {
...
pub(super) fn re_char_in_range_impl(ch: &str, start: &str, end: &str, flags: i64) -> bool {
    if flags & RE_IGNORECASE != 0 {
...
pub(super) fn re_char_at(chars: &[char], index: i64) -> Option<char> {
    let idx = usize::try_from(index).ok()?;
    chars.get(idx).copied()
}

pub(super) fn re_anchor_matches_impl(
    kind: &str,
    text: &str,
    pos: i64,
    end: i64,
...
pub(super) fn re_backref_advance_impl(
    text: &str,
    pos: i64,
    end: i64,
    start_ref: i64,
...
pub(super) fn re_apply_scoped_flags_impl(flags: i64, add_flags: i64, clear_flags: i64) -> i64 {
    (flags | add_flags) & !clear_flags
}

pub(super) fn re_extract_range_pairs(
    _py: &CoreGilToken,
    ranges_bits: u64,
) -> Result<Vec<(String, String)>, u64> {
    let iter_bits = molt_iter(_py, ranges_bits);
    if exception_pending(_py) {
        return Err(MoltObject::none().bits());
    }
...
pub(super) fn re_charclass_matches_impl(
    ch: &str,
    negated: bool,
    chars: &[String],
...
pub(super) fn re_charclass_advance_impl(
    text: &str,
    pos: i64,
    end: i64,
    negated: bool,
...
pub(super) fn re_alloc_group_spans(
    _py: &CoreGilToken,
...
pub(super) fn re_literal_advance_impl(
    text: &str,
    pos: i64,
    end: i64,
    literal: &str,
...
pub(super) fn re_any_advance_impl(text: &str, pos: i64, end: i64, flags: i64) -> i64 {
    let text_chars: Vec<char> = text.chars().collect();
    let text_len = i64::try_from(text_chars.len()).unwrap_or(i64::MAX);
    if pos < 0 || end < 0 || pos >= end || end > text_len {
        return -1;
