...
//! Python-side `re` module cannot implement efficiently with existing helpers.
//!
//! All functions follow the canonical Molt intrinsic ABI:
//!   `pub extern "C" fn molt_re_*(args: u64) -> u64`
//!   with `with_gil_entry!(_py, { ... })` as the outer frame.

use molt_obj_model::MoltObject;
use molt_runtime_core::prelude::*;

use crate::bridge::{
    self, alloc_dict_with_pairs, alloc_list, alloc_string, alloc_tuple,
    attr_name_bits_from_bytes, dec_ref_bits, exception_pending, inc_ref_bits,
...
fn decode_group_names(_py: &PyToken, dict_bits: u64) -> Result<Vec<(String, i64)>, u64> {
    ...
    let iter_bits = bridge::molt_iter(_py,dict_bits);
    ...
    loop {
        let item_bits = bridge::molt_iter_next(_py,iter_bits);
                ...
                if let Some(gn_ptr) = obj_from_bits(group_names_bits).as_ptr() {
                    ...
                    if gn_ty == TYPE_ID_DICT {
                        ...
                        if let Some(name_key_bits) = attr_name_bits_from_bytes(_py, name.as_bytes()) {
                            if let Some(val_bits) = unsafe { bridge::dict_get_in_place(_py, gn_ptr, name_key_bits) } {
...
pub extern "C" fn molt_re_match_groupdict(
    ...
    with_gil_entry!(_py, {
        ...
        let order = unsafe { bridge::dict_order(gn_ptr) }.clone();
        for pair in order.chunks(2) {
            ...
            unsafe {
                bridge::dict_set_in_place(_py, result_ptr, name_key_bits, val_bits);
            }
        }
