...
fn resolve_nbytes(_py: &PyToken, nbytes_bits: u64, default: usize) -> Result<usize, u64> {
...
pub extern "C" fn molt_secrets_compare_digest(a_bits: u64, b_bits: u64) -> u64 {
    molt_runtime_core::with_gil_entry!(_py, {
        ...
        let a_slice = a_obj
            .as_ptr()
            .and_then(|ptr| unsafe { bytes_like_slice(ptr) });
        let b_slice = b_obj
            .as_ptr()
            .and_then(|ptr| unsafe { bytes_like_slice(ptr) });
...
pub extern "C" fn molt_secrets_choice(seq_bits: u64) -> u64 {
    molt_runtime_core::with_gil_entry!(_py, {
        ...
        let len = unsafe {
            let type_id = object_type_id(seq_ptr);
...
fn random_index_below(_py: &PyToken, upper: usize) -> Result<usize, u64> {
    if upper == 0 {
        return Err(raise_exception::<u64>(
            _py,
            "ValueError",
