...
impl HashHandle {
    ...
    pub fn finalize_bytes(&self, length: Option<usize>) -> Result<Vec<u8>, HashError> {
...
pub fn normalize_hash_name(name: &str) -> String {
...
fn bytes_like_from_bits(_py: &PyToken, bits: u64) -> Result<&'static [u8], u64> {
...
fn bytes_like_required(_py: &PyToken, bits: u64, label: &str) -> Result<&'static [u8], u64> {
    let obj = obj_from_bits(bits);
    let Some(ptr) = obj.as_ptr() else {
...
fn int_from_bits(_py: &PyToken, bits: u64, _name: &str) -> Result<i64, u64> {
    let obj = obj_from_bits(bits);
    let type_name = type_name(_py, obj);
...
fn bigint_from_bits(_py: &PyToken, bits: u64, _name: &str, overflow: &str) -> Result<i64, u64> {
    let obj = obj_from_bits(bits);
    let type_name = type_name(_py, obj);
    let msg = format!("'{type_name}' object cannot be interpreted as an integer");
...
fn pbkdf2_rounds(_py: &PyToken, bits: u64) -> Result<u32, u64> {
...
fn pbkdf2_dklen(_py: &PyToken, bits: u64) -> Result<usize, u64> {
    let value = bigint_from_bits(
        _py,
        bits,
        "dklen",
...
fn scrypt_int_required(_py: &PyToken, bits: u64, name: &str) -> Result<u64, u64> {
