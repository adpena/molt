...
fn has_gui_window_capability(py: &PyToken) -> bool {
    has_capability(py, TK_CAPABILITY_GUI_WINDOW) || has_capability(py, "gui")
}

fn has_process_spawn_capability(py: &PyToken) -> bool {
...
fn tk_gate_state(py: &PyToken, op: TkOperation) -> TkGateState {
...
fn require_tk_operation(py: &PyToken, op: TkOperation) -> Result<(), u64> {
...
fn require_tk_app_new(py: &PyToken, _use_tk: bool) -> Result<(), u64> {
...
fn raise_tcl_error(py: &PyToken, message: &str) -> u64 {
...
fn raise_invalid_handle_error(py: &PyToken) -> u64 {
    raise_exception_u64(py, "ValueError", "invalid tkinter app handle")
}

fn parse_app_handle(py: &PyToken, app_bits: u64) -> Result<i64, u64> {
...
fn clear_value_map_refs(py: &PyToken, values: &mut HashMap<String, u64>) {
...
fn value_map_set_bits(py: &PyToken, values: &mut HashMap<String, u64>, key: String, bits: u64) {
...
fn clear_treeview_refs(py: &PyToken, treeview: &mut TkTreeviewState) {
...
fn clear_widget_refs(py: &PyToken, widget: TkWidgetState) {
...
fn clear_ttk_style_refs(py: &PyToken, style: &mut TkTtkStyleState) {
...
fn clear_wm_refs(py: &PyToken, wm: &mut TkWmState) {
...
fn drop_app_state_refs(py: &PyToken, app: &mut TkAppState) {
    #[cfg(all(not(target_arch = "wasm32"), feature = "tk"))]
...
fn unregister_callback_command(py: &PyToken, app: &mut TkAppState, command_name: &str) {
    app.one_shot_callbacks.remove(command_name);
...
fn normalize_bind_add_prefix(py: &PyToken, add_bits: u64) -> Result<String, u64> {
...
fn alloc_after_info_all(py: &PyToken, app: &TkAppState) -> Result<u64, u64> {
...
fn alloc_after_info_token(py: &PyToken, app: &mut TkAppState, token: &str) -> Result<u64, u64> {
...
fn cleanup_after_tokens(py: &PyToken, app: &mut TkAppState, tokens: &HashSet<String>) {
    #[cfg(all(not(target_arch = "wasm32"), feature = "tk"))]
...
#[cfg(all(unix, not(target_arch = "wasm32"), not(feature = "tk")))]
fn next_ready_filehandler_commands(py: &PyToken, handle: i64) -> Result<Vec<String>, u64> {
...
fn next_ready_filehandler_commands(_py: &PyToken, _handle: i64) -> Result<Vec<String>, u64> {
    Ok(Vec::new())
}
...
fn parse_bool_arg(py: &PyToken, handle: i64, bits: u64, label: &str) -> Result<bool, u64> {
...
fn parse_i64_arg(py: &PyToken, handle: i64, bits: u64, label: &str) -> Result<i64, u64> {
...
fn treeview_remove_item(py: &PyToken, treeview: &mut TkTreeviewState, item_id: &str) {
...
fn treeview_set_pairs_to_tuple(py: &PyToken, item: &TkTreeviewItem) -> Result<u64, u64> {
...
#[cfg(all(not(target_arch = "wasm32"), feature = "tk"))]
fn option_use_tk(py: &PyToken, options_bits: u64) -> bool {
...
#[cfg(all(not(target_arch = "wasm32"), feature = "tk"))]
fn tcl_obj_from_bits(py: &PyToken, bits: u64) -> TclObj {
    let obj = obj_from_bits(bits);
    if obj.is_none() {
        return TclObj::from("");
    }
...
#[cfg(all(not(target_arch = "wasm32"), feature = "tk"))]
fn build_native_tk_app(py: &PyToken, use_tk: bool) -> Result<TkAppState, u64> {
...
#[cfg(all(not(target_arch = "wasm32"), not(feature = "tk")))]
fn pump_tcl_events(_py: &PyToken, _handle: i64, _flags: i32) -> Result<bool, u64> {
    Ok(false)
}
...
fn raise_tcl_for_handle(py: &PyToken, handle: i64, message: impl Into<String>) -> u64 {
    let message = message.into();
    set_last_error(handle, message.clone());
    raise_tcl_error(py, &message)
}

fn get_string_arg(py: &PyToken, handle: i64, bits: u64, label: &str) -> Result<String, u64> {
...
fn get_text_arg(py: &PyToken, handle: i64, bits: u64, label: &str) -> Result<String, u64> {
    let obj = obj_from_bits(bits);
    if obj.is_none() {
        return Ok(String::new());
    }
...
fn raise_unsupported_commondialog_command(py: &PyToken, handle: i64, command: &str) -> u64 {
    raise_tcl_for_handle(
        py,
        handle,
...
fn raise_unsupported_filedialog_command(py: &PyToken, handle: i64, command: &str) -> u64 {
    raise_tcl_for_handle(
        py,
        handle,
...
#[cfg(all(not(target_arch = "wasm32"), feature = "tk"))]
fn ensure_native_tk_loaded_for_commondialog(py: &PyToken, handle: i64) -> Result<(), u64> {
    let mut registry = tk_registry().lock().unwrap();
    let app = app_mut_from_registry(py, &mut registry, handle)?;
    if app.tk_loaded {
        return Ok(());
...
#[cfg(any(target_arch = "wasm32", not(feature = "tk")))]
fn ensure_native_tk_loaded_for_commondialog(_py: &PyToken, _handle: i64) -> Result<(), u64> {
    Ok(())
}
...
#[cfg(all(not(target_arch = "wasm32"), feature = "tk"))]
fn tk_dispatch_string_command(py: &PyToken, handle: i64, args: &[String]) -> Result<u64, u64> {
...
fn invoke_callback(py: &PyToken, callback_bits: u64, args: &[u64]) -> u64 {
    if args.is_empty() {
...
fn lookup_bound_callback(py: &PyToken, handle: i64, name: &str) -> Result<Option<u64>, u64> {
    let mut registry = tk_registry().lock().unwrap();
    let app = app_mut_from_registry(py, &mut registry, handle)?;
...
fn release_result_bits(py: &PyToken, result_bits: u64) {
...
fn handle_set_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
...
fn handle_unset_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() != 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_expr_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_loadtk_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() != 1 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_after_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_bind_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
...
fn handle_bindtags_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() != 2 && args.len() != 3 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_event_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_update_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() == 1 {
        clear_last_error(handle);
        return Ok(MoltObject::none().bits());
    }
...
fn wait_for_tk_condition<F>(py: &PyToken, handle: i64, mut done: F) -> Result<(), u64>
where
...
fn handle_tkwait_window_target(py: &PyToken, handle: i64, target: &str) -> Result<u64, u64> {
    let start_exists = {
...
fn handle_tkwait_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() != 3 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_trace_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_focus_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    let mut registry = tk_registry().lock().unwrap();
    let app = app_mut_from_registry(py, &mut registry, handle)?;
    match args.len() {
        1 => {
...
fn handle_grab_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_clipboard_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_selection_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_wm_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 3 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_winfo_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_image_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_font_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_tix_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
...
fn handle_tix_form_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_tix_set_silent_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() != 3 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_option_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_send_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 3 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_tk_global_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.is_empty() {
...
fn command_is_image_instance(py: &PyToken, handle: i64, command: &str) -> Result<bool, u64> {
    let mut registry = tk_registry().lock().unwrap();
    let app = app_mut_from_registry(py, &mut registry, handle)?;
    Ok(app.images.contains_key(command))
}

fn handle_rename_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() != 3 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_ttk_style_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn alloc_empty_string_bits(py: &PyToken) -> Result<u64, u64> {
    alloc_string_bits(py, "")
}

fn alloc_empty_tuple_bits(py: &PyToken) -> Result<u64, u64> {
    alloc_tuple_from_strings(py, &[], "failed to allocate empty tkinter tuple")
}

fn alloc_widget_bbox_bits(py: &PyToken) -> Result<u64, u64> {
    let values = [
        String::from("0"),
        String::from("0"),
        String::from("0"),
...
fn handle_eval_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() < 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_source_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() != 2 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn handle_tk_popup_command(py: &PyToken, handle: i64, args: &[u64]) -> Result<u64, u64> {
    if args.len() != 4 && args.len() != 5 {
        return Err(raise_tcl_for_handle(
            py,
            handle,
...
fn parse_do_one_event_flags(py: &PyToken, handle: i64, flags_bits: u64) -> Result<i32, u64> {
...
fn dispatch_next_pending_event(py: &PyToken, handle: i64) -> Result<bool, u64> {
