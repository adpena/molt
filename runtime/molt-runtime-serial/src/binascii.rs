...
fn bytes_like_arg(_py: &PyToken, bits: u64, func: &str) -> Result<Vec<u8>, u64> {
...
fn ascii_or_bytes_arg(_py: &PyToken, bits: u64, _func: &str) -> Result<Vec<u8>, u64> {
    let obj = obj_from_bits(bits);
...
fn alloc_bytes_or_oom(_py: &PyToken, data: &[u8], context: &str) -> u64 {
...
pub extern "C" fn molt_uu_codec_decode(data_bits: u64) -> u64 {
    molt_runtime_core::with_gil_entry!(_py, {
        ...
        loop {
            ...
            // Try normal decode; on error, honor CPython's broken-uuencoder recovery.
            match uu_decode(line) {
                Ok(decoded) => out.extend_from_slice(&decoded),
                Err(_) => {
                    // Broken uuencoders can emit trailing padding; the byte-count header
                    // gives the recoverable payload width.
                    if !line.is_empty() {
