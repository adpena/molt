...
/// Maximum inline list/dict size for arena allocation (matches molt-runtime constant).
...
fn decode_json_text(_py: &PyToken<'_>, obj: MoltObject, data: &[u8]) -> Result<String, u64> {
    ...
    match decode_bytes_text_or_raise(_py, encoding, data, obj.bits()) {
...
impl<'a, 'py> JsonParser<'a, 'py> {
    fn new(_py: &'py PyToken, text: &'a str, options: JsonLoadsOptions) -> Self {
        let chars: Vec<char> = text.chars().collect();
    ...
    fn parse_number(&mut self) -> Result<u64, JsonParseError> {
        ...
        if is_float {
            ...
            let out_bits =
                unsafe { call_callable1(self._py, builtin_float_bits(self._py), arg_bits) };
            dec_ref_bits(self._py, arg_bits);
            if exception_pending(self._py) {
                return Err(JsonParseError::Raised(out_bits));
            }
        ...
        let arg_bits = self.alloc_string_bits(&raw)?;
        let out_bits = unsafe { call_callable1(self._py, builtin_int_bits(self._py), arg_bits) };
        dec_ref_bits(self._py, arg_bits);
        if exception_pending(self._py) {
            return Err(JsonParseError::Raised(out_bits));
        }
    ...
    molt_runtime_core::with_gil_entry!(_py, {
        ...
        if let Some(ptr) = obj.as_ptr() {
            unsafe {
                ...
                if tid == TYPE_ID_BYTES || tid == TYPE_ID_BYTEARRAY {
                    let len = bytes_len(ptr);
                    if len == 0 {
