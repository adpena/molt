...
fn validate_dialect(_py: &PyToken, dialect: &Dialect) -> Result<(), u64> {
...
fn opt_char_from_bits(_py: &PyToken, bits: u64, param_name: &str) -> Result<Option<char>, u64> {
...
fn char_from_bits(_py: &PyToken, bits: u64, param_name: &str) -> Result<char, u64> {
...
fn release_bits(_py: &PyToken, bits: &[u64]) {
    for bit in bits {
...
// is unaffected (version-stable). Gate the STRINGS/NOTNULL branches on the host
// target version (via the `rt_target_at_least` FFI bridge) so this satellite
// matches its in-tree twin exactly on every target version.
// ─────────────────────────────────────────────────────────────────────────────
fn fields_to_list(_py: &PyToken, fields: &[ParsedField], dialect: &Dialect) -> Result<u64, u64> {
    let strings_notnull_active = rt_target_at_least(3, 13);
...
fn render_field_text(_py: &PyToken, bits: u64) -> String {
    let obj = obj_from_bits(bits);
    if obj.is_none() {
        return String::new();
    }
...
fn write_row_to_string(_py: &PyToken, row_bits: u64, dialect: &Dialect) -> Result<String, u64> {
...
fn raise_csv_parse_error(_py: &PyToken, err: CsvError) -> u64 {
    match err {
...
fn alloc_char_bits(_py: &PyToken, ch: char) -> Result<u64, u64> {
...
fn alloc_optional_char_bits(_py: &PyToken, value: Option<char>) -> Result<(u64, bool), u64> {
