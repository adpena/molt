...
impl MslRenderer {
    ...
    fn detect_fma(
        ...
        source_expr::detect_fma(self, op, op_idx, kernel, idx_var, |dtype| {
            dtype.narrow_metal().is_float()
        })
    }
}

impl SourceExprRenderer for MslRenderer {
    ...
    fn render(&self, kernel: &FusedKernel) -> String {
        ...
        if kernel.body == KernelBody::MaterializeCopy {
            ...
            assert_eq!(
                ...
                "MSL MaterializeCopy requires a non-narrowed dtype"
            );
        ...
        if kernel.vectorize_width == 4 && !has_reduce {
            ...
            assert_eq!(
                domain.output_numel(),
                output_numel,
                "MSL reduction domain output shape must match kernel output"
            );
