    ...
    pub(in crate::wasm::module_abi) fn build_table_abi(
        ...
        let table_entries = slots
            ...
            .map(|func_index| WasmCallableTableEntry {
                func_index,
                symbol: function_symbol(self, func_index, sentinel_func_idx),
            })
            .collect();
...
fn function_symbol(
    ...
    func_index: u32,
    sentinel_func_idx: u32,
) -> Option<WasmFunctionSymbol> {
    if func_index == sentinel_func_idx {
        return None;
    }
    if func_index < backend.func_import_count {
        ...
        return Some(WasmFunctionSymbol::RuntimeImport(import));
    }
    Some(WasmFunctionSymbol::Defined {
...
fn callable_target(
    ...
    let (current_table_index, address) = if slot < fixed_prefix_len {
        ...
        (
            ...
            WasmCallableTableAddress::Relocatable(
                function_symbol(backend, func_index, sentinel_func_idx).unwrap_or_else(|| {
                    panic!("relocatable callable table slot {slot} targets sentinel function")
                }),
            ),
        )
    };
