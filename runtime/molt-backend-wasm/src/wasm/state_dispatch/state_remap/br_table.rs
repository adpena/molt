...
pub(super) fn try_emit_br_table_state_remap_lookup(
    func: &mut Function,
    state_local: u32,
    sorted_entries: &[(i64, i64)],
) -> bool {
    let Some((min_state, table_size)) = br_table_state_remap_params(sorted_entries) else {
        return false;
    };
