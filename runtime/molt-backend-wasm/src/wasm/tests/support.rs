pub(super) use super::super::container_runtime_select::selected_container_runtime_import;
pub(super) use super::super::{WasmBackend, WasmCompileOptions, WasmCompileOutput, WasmProfile};
pub(super) use crate::representation_plan::ScalarRepresentationPlan;
pub(super) use crate::wasm::lir_fast::is_production_lir_wasm_fast_path_name;
...
pub(super) fn wasm_method_ic_ir(kind: &str, extra_arg_count: usize) -> SimpleIR {
...
pub(super) fn wasm_native_callable_ir(abi: &str) -> SimpleIR {
    wasm_native_callable_ir_with_args(abi, vec!["arg"])
}

pub(super) fn wasm_native_callable_ir_with_args(abi: &str, args: Vec<&str>) -> SimpleIR {
...
pub(super) fn wasm_module_attr_native_callable_ir(abi: &str, args: Vec<&str>) -> SimpleIR {
    let mut native_call = wasm_test_op("invoke_ffi", Some("out"), args.clone());
    native_call.native_callable_export =
        Some("nativepkg.ndimage.distance_transform_edt".to_string());
...
pub(super) fn wasm_function_import_modules(wasm: &[u8]) -> BTreeMap<String, String> {
...
pub(super) fn wasm_function_import_type_indices(wasm: &[u8]) -> BTreeMap<String, u32> {
    let mut imports = BTreeMap::new();
    for payload in Parser::new(0).parse_all(wasm) {
        if let Ok(Payload::ImportSection(reader)) = payload {
            for import in reader.into_imports().flatten() {
...
pub(super) fn wasm_function_import_indices(wasm: &[u8]) -> BTreeMap<String, u32> {
    let mut imports = BTreeMap::new();
...
pub(super) fn wasm_direct_call_indices(wasm: &[u8]) -> Vec<u32> {
...
pub(super) fn wasm_data_segment_payloads(wasm: &[u8]) -> Vec<Vec<u8>> {
...
pub(super) fn wasm_function_section_type_indices(wasm: &[u8]) -> Vec<u32> {
...
pub(super) fn wasm_function_exports(wasm: &[u8]) -> BTreeSet<String> {
...
pub(super) fn wasm_type_section_signatures(wasm: &[u8]) -> Vec<(usize, usize)> {
...
pub(super) fn wasm_type_section_value_signatures(wasm: &[u8]) -> Vec<(Vec<String>, Vec<String>)> {
    use wasmparser::CompositeInnerType;
    let mut sigs = Vec::new();
    for payload in Parser::new(0).parse_all(wasm) {
        if let Ok(Payload::TypeSection(reader)) = payload {
