...
use self::callee::build_callee_layout;
use self::caller::build_call_layout;
...
impl WasmMultiReturnLayout {
    pub(super) fn build(
        ...
        let (callee_return_count, callee_value_locals, callee_tuple_vars) = build_callee_layout(
            func_ir,
        ...
        let (call_value_locals, call_tuple_vars) = build_call_layout(
            func_ir,
            multi_return_candidates,
            locals,
            local_types,
