...
// this module's private free helpers (`var_get_boxed_overflow_safe_base`,
// `box_raw_bool_value`, …) without widening any visibility. See `fc/mod.rs`.
mod fc;
        ...
        for op_idx in 0..ops.len() {
            ...
            match op.kind.as_str() {
                ...
                "tuple_new" => {
                    ...
                    let tuple_from_values = Self::import_func_id_split(
                        &mut self.module,
                        &mut self.import_ids,
                    ...
                    let tuple_from_values_local = self
                        .module
                        .declare_func_in_func(tuple_from_values, builder.func);
                ...
                "if" => {
                        ...
                        int_raw_value(&mut builder, &vars, &int_primary_vars, &args[0])
                    {
                        ...
                        seal_block_once(&mut builder, &mut sealed_blocks, not_bool_block);
                        let int_tag = builder.ins().iconst(types::I64, nbc.qnan_tag_int);
                ...
                "br_if" => {
                    ...
                    if debug_block_origins.is_some() {
                        eprintln!(
                            ...
                            func_ir.name, op_idx, target_id, target_block, fallthrough_block
                        );
                    }
                    // cond is NaN-boxed unless representation facts prove a raw
                    // bool-primary value; dispatch from bool_primary_vars to avoid
                ...
                "load_var" | "copy_var" => {
                        ...
                        && op.args.as_ref().is_none_or(|args| args.is_empty())
                    {
                        ...
                        {
                            let raw_f64 =
                                float_value_for(&mut builder, &vars, &float_primary_vars, var_name)
                                    .unwrap_or_else(|| {
                                        let boxed = var_get_boxed_overflow_safe(
                                            &mut self.module,
                                            &mut self.import_ids,
