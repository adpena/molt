...
pub struct OsRandomError(Cow<'static, str>);
...
pub fn fill_os_random(_buf: &mut [u8]) -> Result<(), OsRandomError> {
...
pub fn fill_os_random(buf: &mut [u8]) -> Result<(), OsRandomError> {
    getrandom::fill(buf).map_err(|err| OsRandomError(Cow::Owned(err.to_string())))
}

pub fn os_random_supported() -> bool {
    !cfg!(all(target_arch = "wasm32", feature = "wasm_freestanding"))
}
