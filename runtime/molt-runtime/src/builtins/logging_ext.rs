...
use crate::{
    MoltObject, PyToken, alloc_string, dec_ref_bits, obj_from_bits, raise_exception,
...
fn handle_from_bits(_py: &PyToken<'_>, handle_bits: u64, kind: &str) -> Option<i64> {
            ...
            StreamTarget::Custom(stream_obj_bits) => {
                ...
                if !msg_ptr.is_null() {
                    ...
                    if let Some(stream_ptr) = obj_from_bits(stream_obj_bits).as_ptr() {
                        unsafe {
                            let write_name = crate::intern_static_name(
                                _py,
