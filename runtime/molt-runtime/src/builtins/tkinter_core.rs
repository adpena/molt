...
/// The elements are inc-ref'd by `alloc_list`.
...
/// The elements are inc-ref'd by `alloc_tuple`.
/// Returns `Err(bits)` on allocation failure.
fn alloc_tuple_result(_py: &PyToken<'_>, elems: &[u64]) -> Result<u64, u64> {
...
pub extern "C" fn molt_tk_event_state_decode(state_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        ...
        match alloc_list_bits(_py, &parts) {
            Ok(list_bits) => {
    ...
    crate::with_gil_entry!(_py, {
        ...
        while i + 1 < items.len() {
            ...
            let key_bits = match alloc_str_bits(_py, &key) {
                Ok(bits) => bits,
                Err(bits) => {
                    cleanup_list(_py, &pairs);
                    return bits;
                }
            };
            let value_bits = match alloc_str_bits(_py, value) {
                Ok(bits) => bits,
                Err(bits) => {
            ...
            let pair_bits = match alloc_list_bits(_py, &pair_elems) {
                Ok(bits) => bits,
                Err(bits) => {
                    dec_ref_bits(_py, key_bits);
        ...
        match alloc_list_bits(_py, &pairs) {
            Ok(list_bits) => {
...
fn cleanup_list(_py: &PyToken<'_>, items: &[u64]) {
    for &bits in items {
...
pub extern "C" fn molt_tk_flatten_args(args_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        ...
        flatten_recursive(_py, args_bits, &mut out);
        if exception_pending(_py) {
            return MoltObject::none().bits();
        }
        match alloc_list_bits(_py, &out) {
            Ok(bits) => bits,
            Err(bits) => bits,
        }
    })
...
fn flatten_recursive(_py: &PyToken<'_>, bits: u64, out: &mut Vec<u64>) {
    if bits_is_none(bits) {
        return;
    }
    if let Some(elems) = read_seq_elements(bits) {
        for elem_bits in elems {
            flatten_recursive(_py, elem_bits, out);
        }
    } else {
...
fn emit_option_pair(
    ...
    let key_str_bits = alloc_str_bits(_py, &normalized)?;
...
pub extern "C" fn molt_tk_normalize_option(name_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        ...
        match alloc_str_bits(_py, &normalized) {
            Ok(bits) => bits,
            Err(bits) => bits,
        }
    })
...
pub extern "C" fn molt_tk_hex_to_rgb(color_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        ...
        match alloc_tuple_result(_py, &elems) {
            Ok(bits) => bits,
            Err(bits) => bits,
        }
    })
