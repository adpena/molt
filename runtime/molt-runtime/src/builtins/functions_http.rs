...
use crate::builtins::numbers::{index_bigint_from_obj, int_bits_from_bigint, to_f64};
...
pub(super) fn urllib_request_attr_optional(
    _py: &crate::PyToken<'_>,
    obj_bits: u64,
    name: &[u8],
...
fn ctypes_scalar_kind(_py: &crate::PyToken<'_>, ctype_bits: u64) -> Result<String, u64> {
...
fn ctypes_coerce_char(_py: &crate::PyToken<'_>, value_bits: u64) -> Result<u64, u64> {
...
fn ctypes_coerce_float(_py: &crate::PyToken<'_>, value_bits: u64, width: i64) -> Result<u64, u64> {
    let float_bits = crate::molt_float_from_obj(value_bits);
    if exception_pending(_py) {
...
struct HttpClientExecuteInput {
    ...
    /// taken from `host`. Default false preserves existing HTTP behavior.
    use_tls: bool,
}
        ...
        if let Some(server_name) = req.tls_server_name.as_deref() {
            ...
            #[cfg(not(feature = "stdlib_net"))]
            {
                ...
                return Err(std::io::Error::new(
                    ...
                    "https requires the stdlib_net feature (rustls)",
                ));
            }
        } else {
...
fn urllib_https_send_over_tls(
    ...
    // Build a default-secure rustls ClientConfig with webpki-roots trust anchors.
    // Cache the config across requests so we pay the trust-store build cost once.
    ...
    if let Err(err) = tls.read_to_end(out) {
        // Some servers cleanly close the TLS stream after the response; rustls surfaces
        // that as UnexpectedEof. Treat it as a clean end-of-response if we already
        // received parseable bytes.
