    ...
    for base in bases_vec.iter() {
        ...
        unsafe {
            ...
            if base_ptr == class_ptr {
                return raise_exception::<_>(_py, "TypeError", "class cannot inherit from itself");
            }
        }
    }
    ...
    unsafe {
        ...
        if let Some(dict_ptr) = obj_from_bits(dict_bits).as_ptr() {
            if object_type_id(dict_ptr) == TYPE_ID_DICT {
                ...
                let mro_name = intern_static_name(_py, &runtime_state(_py).interned.mro_name, b"__mro__");
...
pub extern "C" fn molt_class_apply_set_name(class_bits: u64) -> u64 {
    ...
    unsafe {
        ...
        let set_name_bits =
            intern_static_name(_py, &runtime_state(_py).interned.set_name_method, b"__set_name__");
