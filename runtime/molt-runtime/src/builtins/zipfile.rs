...
pub extern "C" fn molt_zipfile_open(path_bits: u64, mode_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        let Some(path) = string_obj_to_owned(obj_from_bits(path_bits)) else {
            return raise_exception::<u64>(_py, "TypeError", "path must be a string");
        };
        let Some(mode) = string_obj_to_owned(obj_from_bits(mode_bits)) else {
            return raise_exception::<u64>(_py, "TypeError", "mode must be a string");
        };
        ...
        match mode.as_str() {
            "r" => {
                let data = match std::fs::read(&path) {
                    Ok(d) => d,
                    Err(e) => return raise_exception::<u64>(_py, "FileNotFoundError", &format!("{}", e)),
                };
                ZIP_MAP.with(|m| {
            ...
            _ => return raise_exception::<u64>(_py, "ValueError", "unsupported zipfile mode"),
        }
        ...
        match state {
            Some(ZipState::Writer { path, entries }) => {
                ...
                let file = match std::fs::File::create(&path) {
                    Ok(f) => f,
                    Err(e) => return raise_exception::<u64>(_py, "IOError", &format!("{}", e)),
                };
                ...
                for (name, data, method) in &entries {
                    ...
                    if let Err(e) = writer.start_file(name.as_str(), options) {
                        return raise_exception::<u64>(_py, "IOError", &format!("{}", e));
                    }
...
pub extern "C" fn molt_zipfile_writestr(
    ...
    crate::with_gil_entry!(_py, {
        ...
        let Some(name) = string_obj_to_owned(obj_from_bits(name_bits)) else {
            return raise_exception::<u64>(_py, "TypeError", "name must be a string");
        };
        ...
        let data: Vec<u8> = if let Some(s) = string_obj_to_owned(data_obj) {
            ...
            if unsafe { object_type_id(ptr) } == TYPE_ID_BYTES {
                ...
                return raise_exception::<u64>(_py, "TypeError", "data must be bytes or str");
            }
        } else {
            return raise_exception::<u64>(_py, "TypeError", "data must be bytes or str");
        };
        ...
        ZIP_MAP.with(|m| {
            ...
            let Some(state) = map.get_mut(&id) else {
                return raise_exception::<u64>(_py, "ValueError", "invalid zip handle");
            };
            match state {
                ...
                _ => raise_exception::<u64>(_py, "ValueError", "writestr requires mode='w'"),
            }
        })
    })
}
            ...
            match state {
                ZipState::Reader { data } => {
                    ...
                    let mut archive = match zip::ZipArchive::new(cursor) {
                        Ok(a) => a,
                        Err(e) => return raise_exception::<u64>(_py, "ValueError", &format!("bad zip: {}", e)),
                    };
            ...
            match state {
                ZipState::Reader { data } => {
                    ...
                    let mut entry = match archive.by_name(&name) {
                        Ok(e) => e,
                        Err(_) => return raise_exception::<u64>(_py, "KeyError", &name),
                    };
                    let mut buf = Vec::new();
                    if let Err(e) = entry.read_to_end(&mut buf) {
                        return raise_exception::<u64>(_py, "ValueError", &format!("read error: {}", e));
                    }
                    let ptr = alloc_bytes(_py, &buf);
                    MoltObject::from_ptr(ptr).bits()
                }
                _ => raise_exception::<u64>(_py, "ValueError", "read requires mode='r'"),
            }
        })
    })
}
