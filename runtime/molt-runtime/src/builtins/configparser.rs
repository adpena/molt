...
thread_local! {
    static CONFIG_HANDLES: RefCell<HashMap<i64, ConfigState>> =
...
fn str_list_to_bits(_py: &PyToken<'_>, items: &[String]) -> u64 {
...
fn string_pairs_to_list(_py: &PyToken<'_>, pairs: &[(String, String)]) -> u64 {
            ...
            if let Some(ptr) = obj.as_ptr() {
                unsafe {
                    if object_type_id(ptr) == TYPE_ID_DICT {
                        ...
                        let order = dict_order(ptr).clone();
...
pub extern "C" fn molt_configparser_read(handle_bits: u64, filename_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        ...
        let ok = CONFIG_HANDLES.with(|map| {
...
pub extern "C" fn molt_configparser_sections(handle_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        ...
        let sections = CONFIG_HANDLES.with(|map| {
            map.borrow()
                .get(&id)
...
pub extern "C" fn molt_configparser_items(handle_bits: u64, section_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        ...
        let result = CONFIG_HANDLES.with(|map| {
            map.borrow().get(&id).and_then(|state| {
...
pub extern "C" fn molt_configparser_write(handle_bits: u64, filename_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        ...
        let content = CONFIG_HANDLES.with(|map| map.borrow().get(&id).map(|state| state.write()));
