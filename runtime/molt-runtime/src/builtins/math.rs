...
use crate::builtins::numbers::{
    index_bigint_from_obj, index_i64_from_obj, int_bits_from_bigint, to_f64,
};
...
use crate::{
    ...
    is_truthy, maybe_ptr_from_bits, molt_int_from_obj, molt_iter, molt_iter_next, molt_mod,
    molt_mul, molt_sorted_builtin, obj_from_bits, object_type_id, raise_exception,
    raise_not_iterable, runtime_state, seq_vec_ref, to_i64, type_of_bits,
};
...
fn colorsys_coerce_f64(_py: &PyToken<'_>, value_bits: u64) -> Option<f64> {
    let value = obj_from_bits(value_bits);
    if let Some(out) = to_f64(value) {
        return Some(out);
    }
    let msg = format!("must be real number, not {}", type_name(_py, value));
    raise_exception::<Option<f64>>(_py, "TypeError", &msg)
}

fn alloc_float_tuple3(_py: &PyToken<'_>, a: f64, b: f64, c: f64) -> Option<u64> {
    let a_bits = MoltObject::from_float(a).bits();
    let b_bits = MoltObject::from_float(b).bits();
    let c_bits = MoltObject::from_float(c).bits();
    let tuple_ptr = alloc_tuple(_py, &[a_bits, b_bits, c_bits]);
    dec_ref_bits(_py, a_bits);
    dec_ref_bits(_py, b_bits);
    dec_ref_bits(_py, c_bits);
...
fn colorsys_v(m1: f64, m2: f64, h: f64) -> f64 {
...
pub extern "C" fn molt_colorsys_rgb_to_hsv(r_bits: u64, g_bits: u64, b_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        let Some(r) = colorsys_coerce_f64(_py, r_bits) else {
            return MoltObject::none().bits();
        };
        let Some(g) = colorsys_coerce_f64(_py, g_bits) else {
            return MoltObject::none().bits();
        };
        let Some(b) = colorsys_coerce_f64(_py, b_bits) else {
            return MoltObject::none().bits();
        };
        ...
        let v = maxc;
        if minc == maxc {
            let Some(bits) = alloc_float_tuple3(_py, 0.0, 0.0, v) else {
                return MoltObject::none().bits();
            };
            return bits;
        }
        ...
        let Some(bits) = alloc_float_tuple3(_py, h, s, v) else {
            return MoltObject::none().bits();
        };
        bits
    })
...
pub extern "C" fn molt_colorsys_hsv_to_rgb(h_bits: u64, s_bits: u64, v_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        let Some(h) = colorsys_coerce_f64(_py, h_bits) else {
            return MoltObject::none().bits();
        };
        let Some(s) = colorsys_coerce_f64(_py, s_bits) else {
            return MoltObject::none().bits();
        };
        let Some(v) = colorsys_coerce_f64(_py, v_bits) else {
            return MoltObject::none().bits();
        };
        if s == 0.0 {
            let Some(bits) = alloc_float_tuple3(_py, v, v, v) else {
                return MoltObject::none().bits();
            };
            return bits;
        }
        let h6 = h * 6.0;
        let h6_bits = MoltObject::from_float(h6).bits();
        let i_bits = molt_int_from_obj(h6_bits, MoltObject::none().bits(), 0);
        if obj_from_bits(i_bits).is_none() {
            return MoltObject::none().bits();
        }
        let i_mod_bits = molt_mod(i_bits, MoltObject::from_int(6).bits());
        if maybe_ptr_from_bits(i_bits).is_some() {
            dec_ref_bits(_py, i_bits);
        }
        if obj_from_bits(i_mod_bits).is_none() {
            return MoltObject::none().bits();
        }
        let Some(i_mod) = to_i64(obj_from_bits(i_mod_bits)) else {
            if maybe_ptr_from_bits(i_mod_bits).is_some() {
                dec_ref_bits(_py, i_mod_bits);
            }
            return raise_exception::<_>(
                _py,
                "TypeError",
                "hsv_to_rgb internal error: expected int modulo result",
            );
        };
        if maybe_ptr_from_bits(i_mod_bits).is_some() {
            dec_ref_bits(_py, i_mod_bits);
        ...
        let Some(bits) = alloc_float_tuple3(_py, r, g, b) else {
            return MoltObject::none().bits();
        };
        bits
    })
...
pub extern "C" fn molt_colorsys_rgb_to_hls(r_bits: u64, g_bits: u64, b_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        ...
        let l = (minc + maxc) / 2.0;
        if minc == maxc {
            let Some(bits) = alloc_float_tuple3(_py, 0.0, l, 0.0) else {
                return MoltObject::none().bits();
            };
            return bits;
        }
        ...
        h = (h / 6.0).rem_euclid(1.0);
        let Some(bits) = alloc_float_tuple3(_py, h, l, s) else {
            return MoltObject::none().bits();
        };
        bits
    })
...
pub extern "C" fn molt_colorsys_hls_to_rgb(h_bits: u64, l_bits: u64, s_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        ...
        let Some(l) = colorsys_coerce_f64(_py, l_bits) else {
            return MoltObject::none().bits();
        };
        let Some(s) = colorsys_coerce_f64(_py, s_bits) else {
            return MoltObject::none().bits();
        ...
        if s == 0.0 {
            let Some(bits) = alloc_float_tuple3(_py, l, l, l) else {
                return MoltObject::none().bits();
            };
            return bits;
        }
...
pub extern "C" fn molt_colorsys_rgb_to_yiq(r_bits: u64, g_bits: u64, b_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        ...
        let Some(bits) = alloc_float_tuple3(_py, y, i, q) else {
            return MoltObject::none().bits();
        };
        bits
    })
...
pub extern "C" fn molt_colorsys_yiq_to_rgb(y_bits: u64, i_bits: u64, q_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        let Some(y) = colorsys_coerce_f64(_py, y_bits) else {
            return MoltObject::none().bits();
        };
        let Some(i) = colorsys_coerce_f64(_py, i_bits) else {
            return MoltObject::none().bits();
        };
        let Some(q) = colorsys_coerce_f64(_py, q_bits) else {
            return MoltObject::none().bits();
        };
