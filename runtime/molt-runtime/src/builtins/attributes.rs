...
pub(crate) unsafe fn attr_lookup_ptr(
    ...
    if type_id == TYPE_ID_TYPE {
        if let Some(name) = string_obj_to_owned(obj_from_bits(attr_bits)) {
            ...
            if name == "__annotate__" {
                ...
                if annotate_bits == 0 {
                    let annotate_name_bits = intern_static_name(_py,
            ...
            if name == "__annotations__" {
                let annotations_bits = intern_static_name(_py,
    ...
    if type_id == TYPE_ID_FUNCTION {
        ...
        if obj_eq(_py, obj_from_bits(attr_bits), obj_from_bits(annotations_bits)) {
    ...
    if type_id == TYPE_ID_DATACLASS {
        ...
        if !desc_ptr.is_null() {
            ...
            if class_bits != 0 {
                if let Some(class_ptr) = obj_from_bits(class_bits).as_ptr() {
                    if object_type_id(class_ptr) == TYPE_ID_TYPE {
                        let getattribute_bits = intern_static_name(_py,
                            &runtime_state(_py).interned.getattribute_name,
                            b"__getattribute__",
                        );
                        if !obj_eq(_py, obj_from_bits(attr_bits), obj_from_bits(getattribute_bits)) {
                            if let Some(call_bits) = class_attr_lookup(_py,
                                class_ptr,
                                class_ptr,
                                Some(obj_ptr),
                                getattribute_bits,
                                ...
                                if exception_pending(_py) {
                                    ...
                                    if kind.as_deref() == Some("AttributeError") {
                                        let getattr_bits = intern_static_name(_py,
                                            ...
                                            .is_some()
                                        {
                                            ...
                                            if let Some(getattr_call_bits) = class_attr_lookup(_py,
                                                class_ptr,
                                                class_ptr,
                                                Some(obj_ptr),
                                                getattr_bits,
    ...
    if type_id == TYPE_ID_OBJECT {
        ...
        if class_bits != 0 {
            if let Some(class_ptr) = obj_from_bits(class_bits).as_ptr() {
                if object_type_id(class_ptr) == TYPE_ID_TYPE {
                    let getattr_bits =
                        intern_static_name(_py, &runtime_state(_py).interned.getattr_name, b"__getattr__");
                    if !obj_eq(_py, obj_from_bits(attr_bits), obj_from_bits(getattr_bits)) {
                        if let Some(call_bits) =
                            class_attr_lookup(_py, class_ptr, class_ptr, Some(obj_ptr), getattr_bits)
                        {
...
pub unsafe extern "C" fn molt_get_attr_generic(
    ...
    attr_error(_py, type_name(_py, MoltObject::from_ptr(obj_ptr)), attr_name)
...
pub unsafe extern "C" fn molt_set_attr_generic(
    ...
    if type_id == TYPE_ID_TYPE {
        let class_bits = MoltObject::from_ptr(obj_ptr).bits();
        if is_builtin_class_bits(_py, class_bits) {
            return raise_exception::<_>(_py, "TypeError", "cannot set attributes on builtin type");
        }
        if attr_name == "__annotate__" {
            ...
            if let Some(dict_ptr) = obj_from_bits(dict_bits).as_ptr() {
                if object_type_id(dict_ptr) == TYPE_ID_DICT {
                    let annotate_bits = intern_static_name(_py,
                        &runtime_state(_py).interned.annotate_name,
                        b"__annotate__",
                    );
    ...
    if type_id == TYPE_ID_DATACLASS {
        ...
        if !desc_ptr.is_null() {
            ...
            if class_bits != 0 {
                if let Some(class_ptr) = obj_from_bits(class_bits).as_ptr() {
                    if object_type_id(class_ptr) == TYPE_ID_TYPE {
                        let setattr_bits = intern_static_name(_py,
                            &runtime_state(_py).interned.setattr_name,
                            b"__setattr__",
                        );
                        if let Some(call_bits) =
                            class_attr_lookup(_py, class_ptr, class_ptr, Some(obj_ptr), setattr_bits)
                        {
                        ...
                        if let Some(desc_bits) = class_attr_lookup_raw_mro(_py, class_ptr, attr_bits) {
                            if descriptor_is_data(_py, desc_bits) {
                                ...
                                let set_bits = intern_static_name(_py,
    ...
    if type_id == TYPE_ID_OBJECT {
        ...
        if class_bits != 0 {
            if let Some(class_ptr) = obj_from_bits(class_bits).as_ptr() {
                if object_type_id(class_ptr) == TYPE_ID_TYPE {
                    let setattr_bits =
                        intern_static_name(_py, &runtime_state(_py).interned.setattr_name, b"__setattr__");
                    if let Some(call_bits) =
                        class_attr_lookup(_py, class_ptr, class_ptr, Some(obj_ptr), setattr_bits)
                    {
                        let _ = call_callable2(_py, call_bits, attr_bits, val_bits);
...
pub(crate) unsafe fn del_attr_ptr(_py: &PyToken<'_>, obj_ptr: *mut u8, attr_bits: u64, attr_name: &str) -> i64 {
    let type_id = object_type_id(obj_ptr);
    if type_id == TYPE_ID_MODULE {
    ...
    if type_id == TYPE_ID_TYPE {
        let class_bits = MoltObject::from_ptr(obj_ptr).bits();
        if is_builtin_class_bits(_py, class_bits) {
            return raise_exception::<_>(_py, "TypeError", "cannot delete attributes on builtin type");
        }
        if attr_name == "__annotate__" {
        ...
        let dict_bits = class_dict_bits(obj_ptr);
        if let Some(dict_ptr) = obj_from_bits(dict_bits).as_ptr() {
            if object_type_id(dict_ptr) == TYPE_ID_DICT && dict_del_in_place(_py, dict_ptr, attr_bits) {
                class_bump_layout_version(obj_ptr);
                return MoltObject::none().bits() as i64;
            }
        }
    ...
    if type_id == TYPE_ID_DATACLASS {
        ...
        if !desc_ptr.is_null() {
            ...
            if class_bits != 0 {
                if let Some(class_ptr) = obj_from_bits(class_bits).as_ptr() {
                    if object_type_id(class_ptr) == TYPE_ID_TYPE {
                        let delattr_bits = intern_static_name(_py,
                        ...
                        if let Some(desc_bits) = class_attr_lookup_raw_mro(_py, class_ptr, attr_bits) {
                            ...
                            let del_bits = intern_static_name(_py,
    ...
    if type_id == TYPE_ID_OBJECT {
        ...
        if class_bits != 0 {
            if let Some(class_ptr) = obj_from_bits(class_bits).as_ptr() {
                if object_type_id(class_ptr) == TYPE_ID_TYPE {
                    let delattr_bits =
                        intern_static_name(_py, &runtime_state(_py).interned.delattr_name, b"__delattr__");
                    if let Some(call_bits) =
                        class_attr_lookup(_py, class_ptr, class_ptr, Some(obj_ptr), delattr_bits)
                    {
                        let _ = call_callable1(_py, call_bits, attr_bits);
...
pub(crate) unsafe fn object_setattr_raw(_py: &PyToken<'_>,
    obj_ptr: *mut u8,
    attr_bits: u64,
    attr_name: &str,
    val_bits: u64,
...
pub(crate) unsafe fn dataclass_setattr_raw(_py: &PyToken<'_>,
    obj_ptr: *mut u8,
    attr_bits: u64,
    attr_name: &str,
    val_bits: u64,
...
pub(crate) unsafe fn object_delattr_raw(_py: &PyToken<'_>, obj_ptr: *mut u8, attr_bits: u64, attr_name: &str) -> i64 {
    let header = header_from_obj_ptr(obj_ptr);
    if (*header).poll_fn != 0 {
        return attr_error(_py, "object", attr_name);
    }
...
pub(crate) unsafe fn dataclass_delattr_raw(_py: &PyToken<'_>, obj_ptr: *mut u8, attr_bits: u64, attr_name: &str) -> i64 {
    let desc_ptr = dataclass_desc_ptr(obj_ptr);
    if !desc_ptr.is_null() {
        let class_bits = (*desc_ptr).class_bits;
        if class_bits != 0 {
...
pub extern "C" fn molt_get_attr_name(obj_bits: u64, name_bits: u64) -> u64 {
    ...
    unsafe {
        ...
        if let Some(obj_ptr) = maybe_ptr_from_bits(obj_bits) {
            ...
            return attr_error(_py, type_name(_py, MoltObject::from_ptr(obj_ptr)), &attr_name) as u64;
        }
        let obj = obj_from_bits(obj_bits);
