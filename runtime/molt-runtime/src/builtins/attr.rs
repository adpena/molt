...
use crate::{
    ...
    type_of_bits, AttrNameCacheEntry, DescriptorCacheEntry, ATTR_NAME_TLS, DESCRIPTOR_CACHE_TLS,
...
pub(crate) fn attr_error(type_label: impl AsRef<str>, attr_name: &str) -> i64 {
    let msg = format!(
        "'{}' object has no attribute '{}'",
        type_label.as_ref(),
        attr_name
    );
    raise_exception("AttributeError", &msg)
}

pub(crate) fn property_no_setter(attr_name: &str, class_ptr: *mut u8) -> i64 {
...
pub(crate) fn property_no_deleter(attr_name: &str, class_ptr: *mut u8) -> i64 {
    let class_name = if class_ptr.is_null() || unsafe { object_type_id(class_ptr) } != TYPE_ID_TYPE
    {
        "object".to_string()
    } else {
...
pub(crate) fn descriptor_no_setter(attr_name: &str, class_ptr: *mut u8) -> i64 {
...
pub(crate) fn descriptor_no_deleter(attr_name: &str, class_ptr: *mut u8) -> i64 {
    let class_name = if class_ptr.is_null() {
        "object".to_string()
    } else {
        class_name_for_error(MoltObject::from_ptr(class_ptr).bits())
...
pub(crate) fn attr_name_bits_from_bytes(slice: &[u8]) -> Option<u64> {
...
pub(crate) fn raise_attr_name_type_error(name_bits: u64) -> u64 {
...
pub(crate) fn clear_attribute_error_if_pending() -> bool {
...
pub(crate) unsafe fn module_attr_lookup(ptr: *mut u8, attr_bits: u64) -> Option<u64> {
    ...
    if obj_eq(
        ...
        if let Some(val_bits) = dict_get_in_place(dict_ptr, annotations_name_bits) {
        ...
        let annotate_bits = dict_get_in_place(dict_ptr, annotate_name_bits)
        ...
        let res_bits = if !obj_from_bits(annotate_bits).is_none() {
            ...
            let Some(res_ptr) = res_obj.as_ptr() else {
                ...
                return raise_exception("TypeError", &msg);
            };
        ...
        let complete_name_bits =
            attr_name_bits_from_bytes(b"__molt_module_complete__").unwrap_or(0);
        let mut cache = false;
        if complete_name_bits != 0 {
            if let Some(complete_bits) = dict_get_in_place(dict_ptr, complete_name_bits) {
                cache = is_truthy(obj_from_bits(complete_bits));
            }
            dec_ref_bits(complete_name_bits);
        }
        if cache {
            dict_set_in_place(dict_ptr, annotations_name_bits, res_bits);
        }
...
pub(crate) unsafe fn dir_collect_from_instance(
    ...
    let dict_name_bits = intern_static_name(&runtime_state().interned.dict_name, b"__dict__");
    let Some(dict_bits) = attr_lookup_ptr_allow_missing(obj_ptr, dict_name_bits) else {
        return;
    };
    let dict_obj = obj_from_bits(dict_bits);
...
pub(crate) unsafe fn class_attr_lookup_raw_mro(class_ptr: *mut u8, attr_bits: u64) -> Option<u64> {
    ...
    loop {
        ...
        if let Some(name) = attr_name.as_deref() {
            ...
            if is_builtin_class_bits(current_bits) {
                if let Some(func_bits) = builtin_class_method_bits(current_bits, name) {
                    return Some(func_bits);
                }
            }
        }
...
pub(crate) unsafe fn class_field_offset(class_ptr: *mut u8, attr_bits: u64) -> Option<usize> {
    let dict_bits = class_dict_bits(class_ptr);
...
pub(crate) unsafe fn is_iterator_bits(bits: u64) -> bool {
...
pub(crate) unsafe fn descriptor_method_bits(val_bits: u64, name_bits: u64) -> Option<u64> {
...
pub(crate) unsafe fn descriptor_has_method(val_bits: u64, name_bits: u64) -> bool {
    descriptor_method_bits(val_bits, name_bits).is_some()
}

pub(crate) unsafe fn descriptor_is_data(val_bits: u64) -> bool {
...
pub(crate) unsafe fn attr_lookup_ptr_any(obj_ptr: *mut u8, attr_bits: u64) -> Option<u64> {
    match object_type_id(obj_ptr) {
        TYPE_ID_OBJECT => object_attr_lookup_raw(obj_ptr, attr_bits),
        TYPE_ID_DATACLASS => dataclass_attr_lookup_raw(obj_ptr, attr_bits),
        _ => attr_lookup_ptr(obj_ptr, attr_bits),
    }
}
...
pub(crate) unsafe fn attr_lookup_ptr_allow_missing(
    ...
    let res = attr_lookup_ptr_any(obj_ptr, attr_bits);
    if res.is_some() {
        return res;
    }
    if exception_pending() {
        ...
        TYPE_ID_PROPERTY => {
            if let Some(inst_ptr) = instance_ptr {
                ...
                if obj_from_bits(get_bits).is_none() {
                    return raise_exception("AttributeError", "unreadable property");
                }
                let inst_bits = instance_bits_for_call(inst_ptr);
...
pub(crate) unsafe fn object_attr_lookup_raw(obj_ptr: *mut u8, attr_bits: u64) -> Option<u64> {
...
pub(crate) unsafe fn dataclass_attr_lookup_raw(obj_ptr: *mut u8, attr_bits: u64) -> Option<u64> {
