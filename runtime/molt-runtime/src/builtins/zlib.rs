...
fn zlib_compression_from_level(_py: &PyToken<'_>, level_bits: u64) -> Result<Compression, u64> {
...
pub extern "C" fn molt_zlib_compress(data_bits: u64, level_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        ...
        if encoder.write_all(data).is_err() {
            return raise_exception::<u64>(_py, "error", "zlib.error: compress failed");
        }
        let out = match encoder.finish() {
            Ok(v) => v,
            Err(_) => return raise_exception::<u64>(_py, "error", "zlib.error: compress failed"),
        };
...
pub extern "C" fn molt_zlib_max_wbits() -> u64 {
    crate::with_gil_entry!(_py, { int_bits_from_i64(_py, 15) })
}

#[unsafe(no_mangle)]
pub extern "C" fn molt_zlib_def_mem_level() -> u64 {
    crate::with_gil_entry!(_py, { int_bits_from_i64(_py, 8) })
}

#[unsafe(no_mangle)]
pub extern "C" fn molt_zlib_def_buf_size() -> u64 {
    crate::with_gil_entry!(_py, { int_bits_from_i64(_py, 16384) })
}

#[unsafe(no_mangle)]
pub extern "C" fn molt_zlib_z_default_compression() -> u64 {
    crate::with_gil_entry!(_py, { int_bits_from_i64(_py, -1) })
}
...
pub extern "C" fn molt_zlib_z_best_compression() -> u64 {
    crate::with_gil_entry!(_py, { int_bits_from_i64(_py, 9) })
}

#[unsafe(no_mangle)]
pub extern "C" fn molt_zlib_z_no_compression() -> u64 {
    crate::with_gil_entry!(_py, { int_bits_from_i64(_py, 0) })
}
...
pub extern "C" fn molt_zlib_z_huffman_only() -> u64 {
    crate::with_gil_entry!(_py, { int_bits_from_i64(_py, 2) })
}
...
pub extern "C" fn molt_zlib_z_finish() -> u64 {
    crate::with_gil_entry!(_py, { int_bits_from_i64(_py, 4) })
}
...
pub extern "C" fn molt_zlib_z_full_flush() -> u64 {
    crate::with_gil_entry!(_py, { int_bits_from_i64(_py, 3) })
}
...
pub extern "C" fn molt_zlib_compressobj_compress(handle_bits: u64, data_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        let Some(handle) = compressor_from_bits(handle_bits) else {
            return raise_exception::<u64>(_py, "error", "zlib.error: invalid compressor handle");
        };
        let data = match require_bytes_slice(_py, data_bits) {
            Ok(s) => s,
            Err(bits) => return bits,
        ...
        if mode == 4 {
            ...
            let out = match inner {
                CompressorInner::Raw(enc) => match enc.finish() {
                    Ok(v) => v,
                    Err(_) => {
                        return raise_exception::<u64>(_py, "error", "zlib.error: flush failed");
                    }
                },
...
pub extern "C" fn molt_zlib_decompressobj_decompress(
    ...
    crate::with_gil_entry!(_py, {
        let Some(handle) = decompressor_from_bits(handle_bits) else {
            return raise_exception::<u64>(_py, "error", "zlib.error: invalid decompressor handle");
        };
        if handle.eof {
            return raise_exception::<u64>(_py, "error", "zlib.error: inconsistent stream state");
        }
        ...
        let (mut out, consumed) = match decompress_chunk(handle.format, &input) {
            ...
            Err(msg) => {
                ...
                return raise_exception::<u64>(_py, "error", &msg);
            }
        };
