use crate::{
    ...
    inc_ref_bits, intern_static_name, obj_from_bits, object_type_id, raise_exception, runtime_state,
    to_i64, type_of_bits, MoltHeader, MoltObject, PyToken, CONTEXT_STACK, TYPE_ID_CONTEXT_MANAGER,
    TYPE_ID_FILE_HANDLE,
};
...
fn alloc_context_manager(_py: &PyToken<'_>, enter_fn: *const (), exit_fn: *const (), payload_bits: u64) -> *mut u8 {
...
unsafe fn context_exit_unchecked(_py: &PyToken<'_>, ctx_bits: u64, exc_bits: u64) {
    ...
    let exit_name_bits = intern_static_name(_py, &runtime_state(_py).interned.exit_name, b"__exit__");
...
pub extern "C" fn molt_context_new(
    ...
    if enter_fn.is_null() || exit_fn.is_null() {
        return raise_exception::<_>(_py, "TypeError", "context manager hooks must be non-null");
    }
        ...
        match type_id {
            TYPE_ID_CONTEXT_MANAGER => {
                ...
                if enter_fn_addr.is_null() {
                    return raise_exception::<_>(_py, "TypeError", "context manager missing __enter__");
                }
                let enter_fn =
            ...
            _ => {
                let enter_name_bits =
                    intern_static_name(_py, &runtime_state(_py).interned.enter_name, b"__enter__");
                let Some(enter_bits) = attr_lookup_ptr_allow_missing(_py, ptr, enter_name_bits) else {
                    return raise_exception::<_>(_py, "TypeError", "context manager missing __enter__");
                };
    ...
    unsafe {
        ...
        match type_id {
            TYPE_ID_CONTEXT_MANAGER => {
                let exit_fn_addr = context_exit_fn(ptr);
                if exit_fn_addr.is_null() {
                    return raise_exception::<_>(_py, "TypeError", "context manager missing __exit__");
                }
                let exit_fn =
                    std::mem::transmute::<*const (), extern "C" fn(u64, u64) -> u64>(exit_fn_addr);
...
pub extern "C" fn molt_context_unwind_to(depth_bits: u64, exc_bits: u64) -> u64 {
    ...
    let depth = match to_i64(obj_from_bits(depth_bits)) {
        ...
        _ => return raise_exception::<_>(_py, "TypeError", "context depth must be a non-negative int"),
    };
