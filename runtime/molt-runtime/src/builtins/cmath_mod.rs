// === FILE: runtime/molt-runtime/src/builtins/cmath_mod.rs ===
//
...
use crate::{
    MoltObject, PyToken, alloc_tuple, dec_ref_bits, obj_from_bits, raise_exception, to_f64,
};
...
fn complex_tuple(_py: &PyToken<'_>, re: f64, im: f64) -> u64 {
...
fn float_arg(_py: &PyToken<'_>, bits: u64, name: &str) -> Result<f64, u64> {
...
fn catan(re: f64, im: f64) -> (f64, f64) {
    ...
    // i/2 * (ln_re + i*ln_im) = i*ln_re/2 - ln_im/2 = (-ln_im/2, ln_re/2)
    (-ln_im / 2.0, ln_re / 2.0)
}

// sinh(z) = sin(iz) / i — or directly: sinh(a)cos(b) + i*cosh(a)sin(b)
...
/// isclose(a, b, rel_tol=1e-09, abs_tol=0.0) — four float args (a_real, a_imag, b_real, b_imag)
...
/// Returns a 7-element tuple matching CPython's cmath constants:
/// pi, e, tau, inf, complex(0, inf), nan, complex(0, nan)
