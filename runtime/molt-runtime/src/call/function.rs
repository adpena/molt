use crate::{
    ...
    profile_hit, raise_exception, recursion_guard_enter, recursion_guard_exit, CALL_DISPATCH_COUNT,
    TYPE_ID_FUNCTION,
};
...
pub(crate) unsafe fn call_function_obj1(func_bits: u64, arg0_bits: u64) -> u64 {
    ...
    if arity != 1 {
        return raise_exception::<_>("TypeError", "call arity mismatch");
    }
    ...
    if !recursion_guard_enter() {
        return raise_exception::<_>("RecursionError", "maximum recursion depth exceeded");
    }
...
pub(crate) unsafe fn call_function_obj0(func_bits: u64) -> u64 {
    profile_hit(&CALL_DISPATCH_COUNT);
    let func_obj = obj_from_bits(func_bits);
    let Some(func_ptr) = func_obj.as_ptr() else {
        return raise_exception::<_>("TypeError", "call expects function object");
...
pub(crate) unsafe fn call_function_obj2(func_bits: u64, arg0_bits: u64, arg1_bits: u64) -> u64 {
    profile_hit(&CALL_DISPATCH_COUNT);
    let func_obj = obj_from_bits(func_bits);
    let Some(func_ptr) = func_obj.as_ptr() else {
        return raise_exception::<_>("TypeError", "call expects function object");
...
pub(crate) unsafe fn call_function_obj_vec(func_bits: u64, args: &[u64]) -> u64 {
    match args.len() {
