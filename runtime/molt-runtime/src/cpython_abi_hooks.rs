...
use crate::object::{
    bits_from_ptr, dec_ref_bits, object_type_id, string_bytes, string_len,
    bytes_data, bytes_len, MoltHeader,
};
use crate::object::type_ids::{
    TYPE_ID_BYTES, TYPE_ID_DICT, TYPE_ID_LIST, TYPE_ID_MODULE, TYPE_ID_SET,
...
unsafe extern "C" fn hook_list_append(list_bits: u64, item_bits: u64) {
    ...
    let ptr = match obj.as_ptr() { Some(p) => p, None => return };
...
unsafe extern "C" fn hook_list_len(bits: u64) -> usize {
    ...
    let ptr = match obj.as_ptr() { Some(p) => p, None => return 0 };
    if unsafe { object_type_id(ptr) } != TYPE_ID_LIST { return 0; }
    unsafe { list_len(ptr) }
}
...
unsafe extern "C" fn hook_tuple_len(bits: u64) -> usize {
    let obj = MoltObject::from_bits(bits);
    let ptr = match obj.as_ptr() { Some(p) => p, None => return 0 };
    if unsafe { object_type_id(ptr) } != TYPE_ID_TUPLE { return 0; }
...
unsafe extern "C" fn hook_dict_set(dict_bits: u64, key_bits: u64, val_bits: u64) {
    ...
    let ptr = match obj.as_ptr() { Some(p) => p, None => return };
    if unsafe { object_type_id(ptr) } != TYPE_ID_DICT { return; }
...
unsafe extern "C" fn hook_dict_get(dict_bits: u64, key_bits: u64) -> u64 {
    let obj = MoltObject::from_bits(dict_bits);
    let ptr = match obj.as_ptr() { Some(p) => p, None => return 0 };
    if unsafe { object_type_id(ptr) } != TYPE_ID_DICT { return 0; }
    let order = unsafe { dict_order(ptr) };
    for chunk in order.chunks(2) {
        if chunk[0] == key_bits { return chunk[1]; }
    }
    0
}
