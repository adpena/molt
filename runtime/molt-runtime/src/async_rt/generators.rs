...
use crate::{
    alloc_exception, alloc_object, alloc_tuple, async_sleep_poll_fn_addr, asyncgen_poll_fn_addr,
    asyncgen_registry, await_waiter_clear, await_waiter_register, await_waiters_take, call_poll_fn,
    class_name_for_error, clear_exception, current_task_ptr, dec_ref_bits,
    ...
    is_block_on_task, maybe_ptr_from_bits, molt_anext, molt_exception_clear, molt_exception_kind,
    molt_exception_last, molt_float_from_obj, molt_raise, obj_from_bits, object_class_bits,
    object_mark_has_ptrs, object_type_id, pending_bits_i64, process_poll_fn_addr,
    process_task_state, ptr_from_bits, raise_cancelled_with_message, raise_exception,
    ...
    task_set_cancel_pending, task_take_cancel_pending, task_waiting_on,
    thread_poll_fn_addr, thread_task_state, token_id_from_bits, to_f64, to_i64, type_name,
    wake_task_ptr, ACTIVE_EXCEPTION_STACK, ASYNCGEN_CONTROL_SIZE, ASYNCGEN_GEN_OFFSET,
    ...
    HEADER_FLAG_GEN_STARTED, HEADER_FLAG_SPAWN_RETAIN, MoltHeader, PtrSlot, TASK_KIND_FUTURE,
    TASK_KIND_GENERATOR, TYPE_ID_ASYNC_GENERATOR, TYPE_ID_EXCEPTION, TYPE_ID_GENERATOR,
    TYPE_ID_OBJECT, TYPE_ID_TUPLE,
};
...
mod tests {
    ...
    fn asyncgen_registry_removes_on_drop() {
        ...
        crate::with_gil_entry!(_py, {
            ...
            assert!(!obj_from_bits(gen_bits).is_none(), "generator allocation failed");
...
pub extern "C" fn molt_generator_close(gen_bits: u64) -> u64 {
    ...
    unsafe {
        ...
        if pending {
            ...
            let is_exit = if let Some(exc_ptr) = exc_obj.as_ptr() {
                if object_type_id(exc_ptr) == TYPE_ID_EXCEPTION {
                    let kind = string_obj_to_owned(obj_from_bits(crate::exception_kind_bits(exc_ptr)))
                        .unwrap_or_default();
                    kind == "GeneratorExit"
                } else {
                    false
        ...
        if let Some((_val, done)) = generator_unpack_pair(res) {
            if !done {
                return raise_exception::<_>(_py, "RuntimeError", "generator ignored GeneratorExit");
            }
        }
        generator_set_closed(_py, ptr, true);
    }
...
unsafe fn asyncgen_future_new(_py: &PyToken<'_>, asyncgen_bits: u64, op_kind: i64, arg_bits: u64) -> u64 {
...
pub extern "C" fn molt_asyncgen_anext(asyncgen_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
    unsafe { asyncgen_future_new(_py, asyncgen_bits, ASYNCGEN_OP_ANEXT, MoltObject::none().bits()) }
...
pub extern "C" fn molt_asyncgen_aclose(asyncgen_bits: u64) -> u64 {
    ...
    let exc_bits = MoltObject::from_ptr(exc_ptr).bits();
    let future_bits = unsafe { asyncgen_future_new(_py, asyncgen_bits, ASYNCGEN_OP_ACLOSE, exc_bits) };
    dec_ref_bits(_py, exc_bits);
    future_bits
...
pub unsafe extern "C" fn molt_asyncgen_poll(obj_bits: u64) -> i64 {
    ...
    if let Some((val_bits, done)) = generator_unpack_pair(res_bits) {
        ...
        if op == ASYNCGEN_OP_ACLOSE {
            ...
            return raise_exception::<i64>(_py, "RuntimeError", "async generator ignored GeneratorExit");
        }
        if done {
            match op {
...
pub unsafe extern "C" fn molt_async_sleep(obj_bits: u64) -> i64 {
    ...
    if (*header).state == 0 {
        ...
        if payload_len >= 1 {
            let deadline = crate::monotonic_now_secs(_py) + delay_secs;
    ...
    if payload_len >= 1 {
        ...
        if let Some(deadline) = to_f64(deadline_obj) {
            if deadline.is_finite() && crate::monotonic_now_secs(_py) < deadline {
                return pending_bits_i64();
            }
        }
    }
