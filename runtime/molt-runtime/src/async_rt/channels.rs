...
// channel creation/send/recv helpers once ExceptionSentinel supports channel pointers to
...
pub extern "C" fn molt_chan_new(capacity_bits: u64) -> *mut u8 {
    ...
    let capacity = match to_i64(obj_from_bits(capacity_bits)) {
        Some(val) => val,
        None => return raise_exception::<_>(_py, "TypeError", "channel capacity must be an integer"),
    };
    if capacity < 0 {
        return raise_exception::<_>(_py, "ValueError", "channel capacity must be non-negative");
    }
...
pub(crate) fn has_capability(_py: &PyToken<'_>, name: &str) -> bool {
    let caps = runtime_state(_py).capabilities.get_or_init(load_capabilities);
    caps.contains(name)
}
...
pub unsafe extern "C" fn molt_ws_connect(
    ...
    if hook_ptr == 0 {
        // TODO(runtime, owner:runtime, milestone:RT2, priority:P2, status:missing): provide
        // a host-level connect hook for production websocket sockets.
        return 7;
    }
