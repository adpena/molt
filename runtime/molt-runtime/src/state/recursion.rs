...
// Aliases for the fast-path API (same implementation — TLS-based)
pub(crate) fn recursion_guard_enter_fast() -> bool { recursion_guard_enter() }
pub(crate) fn recursion_guard_exit_fast() { recursion_guard_exit() }
pub(crate) fn sync_fast_depth_to_tls() { /* no-op — TLS is already authoritative */ }
