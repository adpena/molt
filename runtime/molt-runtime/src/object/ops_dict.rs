...
/// Fast dict getitem for known-dict containers.
///
/// Skips the 18-type dispatch in `molt_index` and goes directly to the
/// dict hash-table lookup.  Falls back to `molt_index` if the object turns
/// out not to be a plain dict (safety net).
#[unsafe(no_mangle)]
pub extern "C" fn molt_dict_getitem(obj_bits: u64, key_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        if let Some(obj_ptr) = obj_from_bits(obj_bits).as_ptr() {
            unsafe {
                ...
                // dict subclass or dict-like: resolve the backing dict.
                if let Some(real_dict_bits) = dict_like_bits_from_ptr(_py, obj_ptr) {
                    let Some(dict_ptr) = obj_from_bits(real_dict_bits).as_ptr() else {
                        return MoltObject::none().bits();
                    };
                    if object_type_id(dict_ptr) == TYPE_ID_DICT {
        ...
        // Fallback: not a dict at runtime — use generic dispatch.
        molt_index(obj_bits, key_bits)
    })
}

/// Fast dict setitem for known-dict containers.
///
/// Skips the type dispatch in `molt_store_index` and calls
/// `dict_set_in_place` directly.  Falls back to `molt_store_index` if the
/// object is not a plain dict (safety net).
#[unsafe(no_mangle)]
pub extern "C" fn molt_dict_setitem(obj_bits: u64, key_bits: u64, val_bits: u64) -> u64 {
    crate::with_gil_entry!(_py, {
        if let Some(obj_ptr) = obj_from_bits(obj_bits).as_ptr() {
            unsafe {
                if object_type_id(obj_ptr) == TYPE_ID_DICT {
                    dict_set_in_place(_py, obj_ptr, key_bits, val_bits);
                    if exception_pending(_py) {
                        return MoltObject::none().bits();
                    }
                    return obj_bits;
        ...
        // Fallback: not a dict at runtime — use generic dispatch.
        molt_store_index(obj_bits, key_bits, val_bits)
    })
}
