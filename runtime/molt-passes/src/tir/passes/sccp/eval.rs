    ...
    match name {
        ...
        "str" => {
            ...
            match a {
                ...
                // Rust's f64 formatter is not byte-for-byte CPython even in the
                // finite non-scientific regime (e.g. 0x4289368ec8725340). Until
                // the runtime's CPython formatter is extracted below this pass
                // layer, SCCP must defer every float string conversion.
        ...
        "repr" => {
            ...
            match a {
                ConstVal::Int(v) => Some(ConstVal::Str(v.to_string())),
                // Same authority boundary as `str(float)`: do not synthesize
                // float repr text inside SCCP without the shared CPython
                // formatter.
                ConstVal::Float(_) => None,
                ConstVal::Bool(v) => {
                    Some(ConstVal::Str(if *v { "True" } else { "False" }.to_string()))
...
mod unicode_fold_tests {
    ...
    fn str_repr_fold_matches_cpython_or_refuses() {
        // Float str/repr: defer every float until SCCP can use the same
        // CPython-compatible float formatter as the runtime. Rust Display can
        // diverge even inside the old finite non-scientific predicate:
        // CPython 3.12 repr(f64::from_bits(0x4289368ec8725340)) ==
        // "3465264303690.4062", while Rust prints "...4063".
        for v in [
            0.0_f64,
            -0.0,
            1.5,
