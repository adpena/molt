...
fn loop_slot_accumulator_no_double_drop() {
    use molt_passes::ir::{FunctionIR, OpIR};
    use molt_passes::tir::passes::alias_analysis::build_alias_union_find;
    use molt_passes::tir::passes::run_pipeline;
    use molt_passes::tir::type_refine::refine_types;
    ...
    let mut tir_func = molt_passes::tir::lower_from_simple::lower_to_tir_for_target(
        &func_ir,
        &molt_passes::tir::target_info::TargetInfo::native_release_fast(),
    );
...
fn loadattr_keepalive_through_copy_aliased_source() {
    ...
    let aliases = molt_passes::tir::passes::alias_analysis::build_alias_union_find(&func);
...
fn unsupported_stack_alloc_keeps_owned_drop_obligation() {
    ...
    assert!(
        decrefs.contains(&s),
        "allocation placement does not remove owned release"
    );
}

#[test]
fn class_allocation_keeps_final_drop() {
    let mut func = TirFunction::new(
    ...
    func.value_types.insert(instance, TirType::DynBox);
    func.value_types.insert(loaded, TirType::DynBox);
    let entry = func.blocks.get_mut(&func.entry_block).unwrap();
    let mut allocation = op(OpCode::ObjectNewBound, vec![ValueId(0)], vec![instance]);
    allocation.attrs.insert("value".into(), AttrValue::Int(16));
    entry.ops.push(allocation);
    entry
        .ops
        .push(op(OpCode::LoadAttr, vec![instance], vec![loaded]));
    ...
    assert!(
        func.blocks
            ...
            .any(|op| op.opcode == OpCode::DecRef && op.operands == [instance]),
        "a class allocation must release its owned result"
    );
}
...
fn loop_reassign_backedge_decref_survives_refcount_elim_post() {
    ...
    molt_passes::tir::passes::refcount_elim::run_post_drop(&mut func, &mut am2);
...
fn mixed_phi_critical_edge_split_inserts_fresh_incref_block() {
    ...
    let preds = molt_passes::tir::dominators::build_pred_map_with(
        &func,
        molt_passes::tir::dominators::CfgEdgePolicy::Full,
    );
    let reachable = molt_passes::tir::dominators::reachable_blocks_with(
        &func,
        molt_passes::tir::dominators::CfgEdgePolicy::Full,
    );
    assert!(
