...
fn result_carrying_store_var_keeps_container_owner_to_return_boundary() {
    ...
    let list_idx = ops
        .iter()
        .position(|op| op.opcode == OpCode::Copy && matches!(op.attrs.get("_original_kind"), Some(AttrValue::Str(kind)) if kind == "list_new"))
        .expect("list_new op must survive");
    let store_idx = ops
        .iter()
        .position(|op| op.opcode == OpCode::Copy && matches!(op.attrs.get("_original_kind"), Some(AttrValue::Str(kind)) if kind == "store_var"))
...
fn result_carrying_store_var_later_container_absorb_keeps_owner_to_return_boundary() {
    ...
    let append_idx = ops
        .iter()
        .position(|op| op.opcode == OpCode::Copy && matches!(op.attrs.get("_original_kind"), Some(AttrValue::Str(kind)) if kind == "list_append"))
...
fn copy_class_def_descriptor_temp_releases_at_class_construction_boundary() {
    ...
    let class_def_idx = ops
        .iter()
        .position(|op| op.opcode == OpCode::Copy && matches!(op.attrs.get("_original_kind"), Some(AttrValue::Str(kind)) if kind == "class_def"))
...
fn module_set_attr_releases_absorbed_value_before_later_borrowed_use() {
    ...
    let pop_idx = ops
        .iter()
        .position(|op| op.opcode == OpCode::Copy && matches!(op.attrs.get("_original_kind"), Some(AttrValue::Str(kind)) if kind == "list_pop"))
...
fn generic_attr_store_releases_absorbed_defaults_tuple_before_later_borrowed_use() {
    ...
    let tuple_idx = ops
        .iter()
        .position(|op| op.opcode == OpCode::Copy && matches!(op.attrs.get("_original_kind"), Some(AttrValue::Str(kind)) if kind == "tuple_new"))
        .expect("tuple_new op must survive");
    let store_idx = ops
        .iter()
        .position(|op| {
            op.opcode == OpCode::StoreAttr && matches!(op.attrs.get("_original_kind"), Some(AttrValue::Str(kind)) if kind == "set_attr_generic_obj")
        })
...
#[test]
fn positive_named_ownership_not_finalizer_absence_selects_python_boundary() {
    // This boundaryless TIR fixture deliberately omits frontend DEL_BOUNDARY
    // at return. Ordinary source lowering already emits that explicit authority.
    for producer_kind in [OpCode::Call, OpCode::ObjectNewBound] {
        for known_absence_hint in [false, true] {
            for named in [false, true] {
                for slot in [false, true] {
                    for release in ["none", "decref", "delete_var"] {
                        let mut func = TirFunction::new(
                        ...
                        if known_absence_hint {
                            // False is transport data, never a stable no-finalizer proof.
                            producer
                                .attrs
                                .insert("defines_del".into(), AttrValue::Bool(false));
                        }
                        let entry = func.blocks.get_mut(&func.entry_block).unwrap();
                        entry.ops.push(producer);
                        if slot {
                            entry.ops.push(original_copy_with_operands(
                                "store_var",
                                vec![value],
                                vec![],
                            ));
                        ...
                        match release {
                            "decref" => entry.ops.push(op(OpCode::DecRef, vec![value], vec![])),
                            "delete_var" => {
                                entry.ops.push(op(OpCode::ConstNone, vec![], vec![missing]));
                                entry.ops.push(op(
                                    OpCode::DeleteVar,
                                    vec![missing, value],
                                    vec![deleted],
                        ...
                        let releases: Vec<usize> = ops
                            .iter()
                            .enumerate()
                            .filter(|(_, op)| op.opcode == OpCode::DecRef && op.operands == [value])
                            .map(|(index, _)| index)
                            .collect();
                        assert_eq!(
                            releases.len(),
                            1,
                            "{producer_kind:?}/{named}/{slot}/{release}"
                        );
                        let should_defer = named && release == "none";
                        assert_eq!(
                            releases[0] > marker,
                            should_defer,
                            "{producer_kind:?}/{named}/{slot}/{release}: {releases:?}, marker={marker}"
                        );
                    }
                }
            }
