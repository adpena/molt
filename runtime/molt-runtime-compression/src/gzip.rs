...
fn compression_from_level(_py: &PyToken, level_bits: u64) -> Result<Compression, u64> {
...
pub extern "C" fn molt_gzip_compress(
    ...
    molt_runtime_core::with_gil_entry!(_py, {
        ...
        if encoder.write_all(data).is_err() {
            return raise_exception(_py, "OSError", "gzip compress failed");
        }
        let out = match encoder.finish() {
            Ok(v) => v,
            Err(_) => return raise_exception(_py, "OSError", "gzip compress failed"),
        };
...
pub extern "C" fn molt_gzip_decompress(data_bits: u64) -> u64 {
    molt_runtime_core::with_gil_entry!(_py, {
        ...
        if decoder.read_to_end(&mut out).is_err() {
            return raise_exception(_py, "OSError", "Not a gzipped file (b'\\x1f\\x8b')");
        }
        return_bytes(_py, &out)
    })
}
...
pub extern "C" fn molt_gzip_read(handle_bits: u64, size_bits: u64) -> u64 {
    molt_runtime_core::with_gil_entry!(_py, {
        let Some(handle) = gzip_handle_from_bits(handle_bits) else {
            return raise_exception(_py, "OSError", "invalid gzip handle");
        };
        ...
        if !ok {
            return raise_exception(_py, "OSError", "gzip read failed");
        }
        return_bytes(_py, &out)
    })
}
    ...
    molt_runtime_core::with_gil_entry!(_py, {
        ...
        match &mut handle.inner {
            GzipHandleInner::Writing(enc) => {
                if enc.flush().is_err() {
                    return raise_exception(_py, "OSError", "gzip flush failed");
                }
            }
        ...
        match inner {
            GzipHandleInner::Writing(enc) => {
                if enc.finish().is_err() {
                    return raise_exception(_py, "OSError", "gzip close/finish failed");
                }
            }
            GzipHandleInner::Reading(_) | GzipHandleInner::Closed => {}
        }
