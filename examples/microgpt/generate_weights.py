"""
Train microGPT on tiny name dataset and export weights as JSON.
Runs on HOST Python (not WASM). Based on Karpathy's microGPT.
"""
...
# ---------------------------------------------------------------------------
# Tokenizer — characters a-z plus BOS token
# ---------------------------------------------------------------------------
...
for key, mat in state_dict.items():
    out[key] = [[p.data for p in row] for row in mat]

# Also export the charset so inference.py can reconstruct the tokenizer
out["_meta"] = {
    "uchars": uchars,
