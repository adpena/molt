...
- Orchestration orchestration: run `PYTHONPATH=src python3 tools/symphony_bootstrap.py --project-slug <slug> --install-launchd` once, then `PYTHONPATH=src python3 tools/symphony_run.py WORKFLOW.md --port 8089` (runtime env in `ops/linear/runtime/orchestration.env`).
