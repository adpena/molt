...
Top speedups: `bench_sum.py` 201.62x, `bench_channel_throughput.py` 47.69x,
`bench_async_await.py` 12.69x, `bench_matrix_math.py` 10.69x,
`bench_deeply_nested_loop.py` 7.67x.
Regressions: none (slowest wins: `bench_fib.py` 1.39x, `bench_struct.py` 1.52x).
...
5.92s, `bench_struct.py` 2.13s; largest sizes: `bench_channel_throughput.py` 146.1 KB,
...
Baseline microbenchmarks (2026-01-12): `bench_min_list.py` 1.91x, `bench_max_list.py` 1.90x,
`bench_prod_list.py` 6.64x, `bench_str_find_unicode.py` 4.68x, `bench_str_count_unicode.py` 1.88x.

| Benchmark | Molt vs CPython | Notes |
| --- | --- | --- |
| bench_matrix_math.py | 10.69x | buffer2d matmul lowering |
| bench_deeply_nested_loop.py | 7.67x | nested loop lowering |
| bench_str_endswith.py | 5.83x | string endswith fast path |
| bench_str_startswith.py | 4.89x | string startswith fast path |
| bench_str_count.py | 5.30x | string count fast path |
| bench_str_split.py | 4.21x | optimized split builder |
| bench_str_replace.py | 4.30x | SIMD-friendly replace path |
| bench_str_join.py | 2.56x | pre-sized join buffer |
| bench_sum_list.py | 2.57x | vector reduction fast path |
