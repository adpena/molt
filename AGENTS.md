...
- [docs/SYMPHONY.md](docs/SYMPHONY.md): canonical Symphony entrypoint (scope, architecture, run flow).
- [docs/SYMPHONY_HUMAN_ROLE.md](docs/SYMPHONY_HUMAN_ROLE.md): canonical human authority/escalation model.
- [docs/SYMPHONY_OPERATOR_PLAYBOOK.md](docs/SYMPHONY_OPERATOR_PLAYBOOK.md): operator runbook for Symphony execution.

## Symphony Orchestration Foundation (Non-Negotiable)
- Use `docs/SYMPHONY.md` as the canonical Symphony map; treat `docs/SYMPHONY_HUMAN_ROLE.md` + `docs/SYMPHONY_OPERATOR_PLAYBOOK.md` as the execution/decision contract.
...
- Preferred Symphony tool-call flow:
...
## Symphony Formalization Integration (Lean + Quint)
- For risky orchestration changes (`src/molt/symphony/**`, `tools/symphony_*.py`, workflow/dispatch/state transitions), run Lean + bounded Quint in normal turns:
...
- This policy is mandatory for Symphony tooling too (for example: `PYTHONPATH=src uv run --python 3.12 python3 tools/symphony_run.py WORKFLOW.md --once`).
...
- For automated PR acceptance/merge flows, enforce author allowlisting (`adpena`, `symphony`) before merge/auto-merge execution.
