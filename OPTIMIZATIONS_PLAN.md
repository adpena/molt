...
| 2026-03-18 | Reduced import-closure and startup overhead on the compiler/runtime path: shared module-resolution caches now span module-graph discovery, package-parent closure, core-closure reuse, and namespace-parent closure, and generated import dispatchers now use constant-time known-module membership plus cached top-level resolution instead of repeated linear scans/splits. | `src/molt/cli.py`, `tests/cli/test_cli_import_collection.py`, `OPTIMIZATIONS_PLAN.md`, `docs/spec/STATUS.md` | Validation gates passed (`python3 -m py_compile src/molt/cli.py tests/cli/test_cli_import_collection.py`; `UV_NO_SYNC=1 uv run --python 3.12 python3 -m pytest -q tests/cli/test_cli_import_collection.py` -> `48 passed`). |
...
- [x] Capture current lowering/program counts (`docs/spec/areas/compat/surfaces/stdlib/stdlib_intrinsics_audit.generated.md`, runtime import-call density snapshot).
...
- TODO(tooling, owner:tooling, milestone:TL2, priority:P1, status:done): process-level frontend module lowering is the default for dependency-safe layers, with deterministic topological merge ordering, SCC serialization, per-layer cost scaling, CPU/live-memory/cgroup worker caps, process-tree custody, and machine-readable worker timing/memory policy telemetry. `MOLT_FRONTEND_PARALLEL_MODULES=0` is an explicit diagnostic opt-out, not the production default; calibrate the real Pact wall-clock delta through the existing telemetry rather than a synthetic backend proxy.
- TODO(compiler, owner:compiler, milestone:LF3, priority:P1, status:planned): extend dependency-safe parallel scheduling into large-function mid-end work after per-function work budgets expose stable scheduling units; preserve deterministic merge/publication ordering and reuse the same CPU/live-memory resource authority.
...
- Cranelift 0.128 backend settings now expose deterministic tuning hooks directly in Molt (`regalloc_algorithm`, minimum function alignment, and libcall call-conv override).
...
- Keep benchmark and compile references synchronized with:
  ...
  - [Compile progress tracker](docs/benchmarks/compile_progress.md)
  - [Stdlib intrinsics audit](docs/spec/areas/compat/surfaces/stdlib/stdlib_intrinsics_audit.generated.md)
  - [Rust lowering program](docs/spec/areas/compat/plans/stdlib_lowering_plan.md)
