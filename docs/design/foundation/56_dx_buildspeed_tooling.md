...
| **sccache** | Wiring LANDED. `molt dx env` / `RunContext.dx_env` now resolve `SCCACHE_DIR=$MOLT_EXT_ROOT/.sccache` and `SCCACHE_CACHE_SIZE=10G` cross-platform; normal CLI Cargo builds install the same defaults before enabling `RUSTC_WRAPPER=sccache`. `tools/throughput_env.sh` is a POSIX compatibility wrapper over the Python DX resolver, not the cache-policy authority. | code-verified |
