...
| **Meta-gate idempotence check has false positives** (rustfmt/version nondeterminism makes a generator's double-run differ) | Phase 1 | The check runs the generator twice in ONE environment and diffs; nondeterminism is then a REAL generator bug (it would also fail the existing `--check` sync test intermittently) and is fixed in the generator (pin the rustfmt invocation and guarded stdin/stdout custody in `gen_op_kinds.py:57`), never by exempting the generator. |
