...
| 5 | native linker keep policy | `native_link_plan.py` + `native_link_command.py::_build_native_link_plan`; publication in `build_results.py::_finalize_native_link_candidate` | target-plan dead stripping plus generated export policy; archives are linked without unconditional whole-archive loading, so only referenced objects are pulled | the export root is still a policy input that must remain derived from the live `Export ∪ RuntimeEntrypoint` set |
