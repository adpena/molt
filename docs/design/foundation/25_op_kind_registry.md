...
2. **TIR SSA mapper and reverse backend spelling** — `kind_to_opcode` in `runtime/molt-tir/src/tir/ssa.rs:1902`, backed by `op_kinds_generated.rs:20`, maps a kind string → `OpCode`. `opcode_canonical_kind_table` is the generated reverse authority for backend op-name tails such as MLIR opaque lowering. Unknown input kinds deliberately fall back to `OpCode::Copy`, stashing the spelling in `_original_kind`, as the runtime backstop behind the generated registry.
