...
`tools/llvm_runtime_abi_audit.py --check` guards the preserved-Copy runtime-call ABI seam that the op-kind audit only identifies by symbol coverage. `MOLT_RUNTIME_INTRINSIC_SYMBOLS` is availability only; LLVM call signatures are owned by two disjoint authorities: exact declarations in `runtime/molt-backend-native/src/llvm_backend/runtime_imports/fixed.rs::FIXED_RUNTIME_IMPORTS`, and residual all-i64 fallback facts in `runtime/molt-backend-native/src/llvm_backend/runtime_imports/abi_facts.rs::CONSERVATIVE_RUNTIME_IMPORTS`. The gate derives emitted-but-unmapped frontend kinds, intersects them with `pub extern "C" fn molt_*` exports, requires boxed/i64 or void ABI facts, verifies each fallback fact against Rust export arity, parameter ABI, and return ABI after alias normalization, rejects duplicate facts, and keeps missing runtime exports out of the fixed table instead of preserving phantom declarations.
