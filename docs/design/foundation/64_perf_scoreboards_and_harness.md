...
- **Schema-contract gate (every phase):** `perf_schema.validate_cell/validate_board` runs
  ...
  `tools/ci_gate.py` Tier 1 includes the required `perf-scoreboard-contract` check, which
  runs `tests/tools/test_perf_causality.py`, `tests/tools/test_perf_schema.py`, and
  `tests/tools/test_perf_scoreboard.py` without a Rust rebuild.
...
Every PR that touches the perf plane must run: `perf-scoreboard-contract`, then
`--self-test --all-boards`, the projection-asymmetry test, and `structural_audit --check`
as the relevant phases land. The schema/tool contract is already a Tier 1 gate; the full
smoke benchmark gate remains Phase 3's runtime CI wiring.
