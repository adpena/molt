...
- WASM binary/artifact facts are no longer CLI pins. Artifact-local binary readers such as
  `_wasm_export_function_signatures` live in `molt.wasm_artifact`; runtime import
  signatures/results are generated ABI manifest facts and must flow through
  `molt.cli.wasm` manifest helpers rather than binary-derived duplicate authority.
