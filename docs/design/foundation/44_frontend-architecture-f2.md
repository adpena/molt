...
**Date:** 2026-06-06. Every file:line anchor verified against HEAD `dc6965d8d`. Current-code note, 2026-06-25: F1 move-only extraction has since reduced `src/molt/frontend/__init__.py` to 9,256 lines and 310 class methods, but the generator is still one shared-state MRO shell. The F2 target below remains the required end-state: data contracts and phase separation, not permanent mixins over the god object.
