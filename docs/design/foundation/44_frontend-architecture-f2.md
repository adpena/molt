...
| **super() dispatch** | static fold vs runtime path now asks `frontend.sema.classgraph.super_fold_is_sound` over sema-owned C3/static-MRO/reachability and local class-member facts; dynamic/decorated class-member surfaces fail closed, and imported class metadata is an explicit external input | `sema/classgraph.py` (`ClassFacts`, `super_fold_is_sound`), `calls.py` (the call site) | Next cut: Sema precomputes `ClassFacts.super_fold_sound: set[method]`; Lower reads it without re-running the predicate |
...
- `src/molt/frontend/sema/classgraph.py` — static class graph, local class-member facts, C3/static-MRO/reachability facts, and the zero-arg `super()` fold soundness predicate; next cut is precomputed fold-sound method sets instead of call-time predicate execution.
