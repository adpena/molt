...
- **Rustc `HIR → THIR → MIR` + query system** (rustc-dev-guide.rust-lang.org; MIT/Apache — ideas only): the query system computes a fact (e.g. `typeck`) **on demand, memoized, keyed by `DefId`**, and consumers *ask* for it rather than recomputing. **Borrowed:** `SemaResult` tables keyed by node id are the memoized-fact analog; `super_fold_is_sound` is now a sema-owned predicate, with the remaining step being a precomputed `ClassFacts.super_fold_sound` fact that Lower reads without running the predicate. **Diverge:** molt computes Sema eagerly per module (no lazy query engine — the module is small enough that eager is simpler and the DX is better; we reject importing a query framework, §5).
...
| **super() dispatch** | static fold vs runtime path now asks `frontend.sema.classgraph.super_fold_is_sound` over sema-owned C3/static-MRO/reachability facts; method metadata is still supplied by the walk until class facts are fully precomputed | `sema/classgraph.py` (`super_fold_is_sound`), `calls.py` (the call site) | Sema computes `ClassFacts.super_fold_sound: set[method]`; Lower reads it without re-running the predicate |
...
**Scope.** Create `frontend/sema/` with `scope.py` (the `ScopeInfo`/`ScopeTable` builder — lifts the closure-cell/free-var/nonlocal/global/comp-isolation analysis currently smeared across the `*Collector` nested classes and the 18 scope dicts), `classgraph.py` (lifts static class-graph construction, C3/static-MRO/reachability facts, and the zero-arg `super()` fold soundness predicate; the final ClassFacts precompute cut still needs method-owner/fold-sound sets instead of call-time class metadata), `constenv.py` (lifts `_collect_module_const_dicts` `__init__.py:2705`, the const-int facts, the refuse-to-fold decision), `legality.py` (lifts `_prescan_compile_warnings` `__init__.py:1450`). Each is **free functions over dataclasses** (the `cfg_analysis.py` shape). `SimpleTIRGenerator.__init__` *calls* Sema and stores the immutable `SemaResult`; the visit methods initially still read their old dicts, now *populated from* `SemaResult` (a shim layer).
...
The phase boundary is now partially real but incomplete. Parse is external (ast), and `frontend.sema` now owns module class-graph construction, C3/static-MRO/reachability facts, const-env collection, function-shape facts, and the zero-arg `super()` fold soundness predicate. The remaining gap is that Lower still supplies walk-built class method metadata to the sema predicate and still reads shimmed god-object dicts in many places. The only fully clean separation in the package remains `cfg_analysis.py` — and it operates on the *already-emitted* op stream, i.e. it is a *post-Lower* analysis, not a *pre-Lower* Sema. **Gap remains high:** the architecture has a Sema phase, but Lower is not yet a thin consumer of immutable ClassFacts/ScopeInfo.
...
- `src/molt/frontend/sema/classgraph.py` — static class graph, C3/static-MRO/reachability facts, and the zero-arg `super()` fold soundness predicate; next cut is precomputed ClassFacts instead of call-time method metadata.
- `src/molt/frontend/visitors/calls.py` — call lowering consumes `sema.classgraph.super_fold_is_sound`; still owns the large `visit_Call` dispatch body.
