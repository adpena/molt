...
**Status:** Design doc. F1 (move-only mixin split) is landed and has expanded beyond the original four-mixin snapshot into the current local-binding, midend-optimization, serialization, analysis, visitor, and statement-family mixin shell. F2 is the semantic decomposition that F1 deliberately deferred. No F2 implementation has landed. This doc is the program spec: end-state shape, the single-authority rule, the registry extension, the phasing with F1/M1 discipline, the per-principle scorecard, and the contention economics.

**Date:** 2026-06-06. Every file:line anchor verified against HEAD `dc6965d8d`. Current-code note, 2026-06-25: F1 move-only extraction has since reduced `src/molt/frontend/__init__.py` to 9,809 lines and 310 class methods, but the generator is still one shared-state MRO shell. The F2 target below remains the required end-state: data contracts and phase separation, not permanent mixins over the god object.
