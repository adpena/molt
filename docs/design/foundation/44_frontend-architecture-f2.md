...
**Status:** Design doc. F1 (move-only mixin split) is landed. F2 is the semantic decomposition that F1 deliberately deferred. No F2 implementation has landed. This doc is the program spec: end-state shape, the single-authority rule, the registry extension, the phasing with F1/M1 discipline, the per-principle scorecard, and the contention economics.

**Date:** 2026-06-06. Every file:line anchor verified against HEAD `dc6965d8d`.

**The verdict this engineers against** (supervisor, "engineered like Chris Lattner would?"): **NO, today.** `src/molt/frontend/__init__.py` is 27,071 lines, one class `SimpleTIRGenerator` with 538 `def`s (`__init__.py:211`), assembled from four MRO-mixins (`SerializationMixin, PatternMatchMixin, CallVisitorMixin, ClassDefVisitorMixin, ast.NodeVisitor` — `__init__.py:211-217`) that share its ~150 mutable instance fields. Semantic analysis (scope binding, static MRO/C3, super-fold soundness, IC index allocation, exception-edge insertion, const handling, augassign-kind selection) lives **inline in the visit/emit methods**, recomputed during the lowering walk. The cost is measured below in §6.
