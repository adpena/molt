...
**Status:** Design doc plus live F2 routing note. F1 (move-only mixin split) is landed and has expanded beyond the original four-mixin snapshot into the current local-binding, midend-optimization, serialization, analysis, visitor, async/generator, and statement-family mixin shell. F2 is the semantic decomposition that F1 deliberately deferred. F2b has begun: `frontend.sema.funcmeta` now owns `FunctionKind`, canonical function-kind normalization, yield/signature classification, and async-generator legality predicates consumed by lowering. This doc remains the program spec for the unfinished end-state: phase separation, single-authority facts, registry extension, explicit lowering contexts, and dissolution of the mixin-over-god-object shims.

**Date:** 2026-06-06. Every file:line anchor verified against HEAD `dc6965d8d`. Current-code note, 2026-06-25: F1 plus the first function-shape F2 authority cut has reduced `src/molt/frontend/__init__.py` to 8,458 lines and 289 class methods. The generator is still one shared-state MRO shell, but function-shape spelling and suspension classifiers now have one semantic home in `frontend.sema.funcmeta`. The F2 target below remains the required end-state: data contracts and phase separation, not permanent mixins over the god object.

**The verdict this engineers against** (supervisor, "engineered like Chris Lattner would?"): **NO, today.** At the original audit, `src/molt/frontend/__init__.py` was 27,071 lines, one class `SimpleTIRGenerator` with 538 `def`s (`__init__.py:211`), assembled from four MRO-mixins (`SerializationMixin, PatternMatchMixin, CallVisitorMixin, ClassDefVisitorMixin, ast.NodeVisitor` — `__init__.py:211-217`) that shared its ~150 mutable instance fields. Current F1 has moved more families out of the file, including `LocalBindingMixin` and `MidendOptimizationMixin`, but the architectural defect remains: semantic analysis (scope binding, static MRO/C3, super-fold soundness, IC index allocation, exception-edge insertion, const handling, augassign-kind selection) still lives **inline in the visit/emit methods**, recomputed during the lowering walk behind one shared generator state. The cost is measured below in §6.
...
| **super() dispatch** | static fold vs runtime path decided by a predicate *run during the walk* | `calls.py:122` (`_super_fold_is_sound`), `:1875` (the call site) | Sema computes `ClassFacts.super_fold_sound: set[method]`; Lower reads it |
...
**Scope.** Create `frontend/sema/` with `scope.py` (the `ScopeInfo`/`ScopeTable` builder — lifts the closure-cell/free-var/nonlocal/global/comp-isolation analysis currently smeared across the `*Collector` nested classes and the 18 scope dicts), `classgraph.py` (lifts `_c3_merge`/`_class_mro_names`/`_static_mro_names`/`_super_fold_is_sound` from `__init__.py:903-1085` and `calls.py:122`), `constenv.py` (lifts `_collect_module_const_dicts` `__init__.py:2705`, the const-int facts, the refuse-to-fold decision), `legality.py` (lifts `_prescan_compile_warnings` `__init__.py:1450`). Each is **free functions over dataclasses** (the `cfg_analysis.py` shape). `SimpleTIRGenerator.__init__` *calls* Sema and stores the immutable `SemaResult`; the visit methods initially still read their old dicts, now *populated from* `SemaResult` (a shim layer).
...
No phase boundary exists. Parse is external (ast), but Bind/Sema/Lower/Serialize are **one walk on one object**: scope binding, legality, class-graph, const-env, and emission all happen in the same `visit_X` pass. Evidence: `_prescan_compile_warnings` (legality, `__init__.py:1450`) and `_collect_module_const_dicts` (const-env, `:2705`) are *methods of the generator* run from `visit_Module` (`:2925/:2978`), interleaved with emission; `_super_fold_is_sound` (Sema, `calls.py:122`) is *run during* the call-lowering walk (`calls.py:1875`), not precomputed. The only clean separation in the package is `cfg_analysis.py` — and it operates on the *already-emitted* op stream, i.e. it is a *post-Lower* analysis, not a *pre-Lower* Sema. **Gap is maximal:** the architecture has no Sema phase at all.
...
Nothing semantic is testable without constructing the whole generator. To test the super-fold soundness predicate you must instantiate `SimpleTIRGenerator` (150 fields) and populate `module_class_bases`; to test scope binding you run a full lowering. The *only* isolation-testable unit is `cfg_analysis.py` (free functions over an `OpLike` Protocol — `cfg_analysis.py:7/44`) and `gen_op_kinds.py`'s output. Sema-as-free-functions (F2b) is exactly what makes `_super_fold_is_sound`, the C3 merge, and scope binding unit-testable on a bare AST. **Gap is maximal today.**
...
- `src/molt/frontend/visitors/calls.py` — `_super_fold_is_sound` (`:122`), call lowering (live-uncommitted at HEAD).
