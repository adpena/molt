...
**PEP 3118 completeness.** The N-D memoryview supports shapes, strides, formats, readonly state, direct released-view fail-closed behavior, and writable bytearray-backed mutation. `suboffsets` (Fortran-order indirect arrays), generic bytes-like consumer error precedence, exporter lock/count parity, and NumPy ndarray construction remain open.
...
**Doc 29 — Zero-Copy Spine (Buffer Protocol + DLPack + C-ABI Buffer Bridge).** Commission the next cut after the current typed strided storage/released-view foundation. Inputs: `ops_memoryview.rs` (current state), runtime buffer acquire/export, `molt_buffer_export` C struct, and `tensor.py` (no `__dlpack__`). Deliverable: (a) NumPy ndarray construction from the runtime-owned descriptor, (b) full exporter lock/count parity, (c) generic bytes-like consumers routed through typed buffer authority for released-view precedence, (d) `__dlpack__`/`__dlpack_device__` on `molt.gpu.Tensor`, and (e) suboffset/PEP 3118 completion. Benchmark lane: numpy `np.frombuffer(molt_bytes)` zero-copy roundtrip, PyTorch `torch.from_dlpack(tensor.__dlpack__())` roundtrip latency and memory copy count.
