...
**Refcount ordering.** `runtime/molt-codegen-abi/src/lib.rs` owns `MoltRefCount` beside `MoltFlags` through one generated implementation and separate custody policies. Refcounts use `Cell<u32>` under deterministic GIL and wasm32 and `AtomicU32` under explicit native free-threading, retaining the acquire fence on drop-to-zero. Native flags stay atomic for pre-GIL scheduler access. Backend/runtime link witnesses bind the exact mode, so cached generated objects cannot mix contracts.
...
**memoryview.** `/Users/adpena/Projects/molt/runtime/molt-runtime/src/object/ops_memoryview.rs` — substantive implementation. Supports 1-D and N-D shaped views (`alloc_memoryview` / `alloc_memoryview_shaped`), typed strided storage/export descriptors, `cast()` with C-contiguous guard, `tobytes()`, `tolist()` (recursive N-D), `count()`, `index()`, `hex()`, `toreadonly()`, and idempotent `release()` with a runtime released-view guard. Feature-gated via `builtin_memoryview` in `stdlib_server` and above.

`molt_buffer_export` exposes a `BufferExport` C struct (ptr, len, readonly, ndim, shape, strides, format, itemsize, owner/base, offset) for bytes/bytearray/memoryview objects through the runtime-owned typed strided descriptor. Released memoryviews fail closed with `ValueError` instead of exporting stale storage.
...
**cpython-abi buffer.** `PyObject_GetBuffer` routes through the same runtime buffer acquire/export path for bytes/bytearray/memoryview descriptors, and `PyBuffer_Release` releases the retained owner. The remaining gap is NumPy ndarray construction and full exporter lock/count parity, not a separate always-failing C-ABI lane.
...
**PEP 3118 completeness.** The N-D memoryview supports shapes, strides, formats, readonly state, direct released-view fail-closed behavior, writable bytearray-backed mutation, and released-view precedence for bytes/bytearray method arguments. `suboffsets` (Fortran-order indirect arrays), remaining bridge/builtin bytes-like consumer error precedence, exporter lock/count parity, and NumPy ndarray construction remain open.
...
**Doc 29 — Zero-Copy Spine (Buffer Protocol + DLPack + C-ABI Buffer Bridge).** Commission the next cut after the current typed strided storage/released-view foundation. Inputs: `ops_memoryview.rs` (current state), runtime buffer acquire/export, `molt_buffer_export` C struct, and `tensor.py` (no `__dlpack__`). Deliverable: (a) NumPy ndarray construction from the runtime-owned descriptor, (b) full exporter lock/count parity, (c) remaining bridge/builtin bytes-like consumers routed through typed buffer authority for released-view precedence, (d) `__dlpack__`/`__dlpack_device__` on `molt.gpu.Tensor`, and (e) suboffset/PEP 3118 completion. Benchmark lane: numpy `np.frombuffer(molt_bytes)` zero-copy roundtrip, PyTorch `torch.from_dlpack(tensor.__dlpack__())` roundtrip latency and memory copy count.
