...
- `/Users/adpena/Projects/molt/runtime/molt-passes/src/tir/module_phase.rs` — lines 115–179 (`run_module_pipeline`: call graph → inliner → slot-promotion → rebuild)
- `/Users/adpena/Projects/molt/runtime/molt-passes/src/tir/target_info.rs` — lines 246–311 (TargetInfo constructors; `BuildProfile` field exists but dev-fast vs release-fast never changes a pass decision)
