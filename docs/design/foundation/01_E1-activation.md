...
**Add** `pub fn lower_functions_to_tir_module(functions: &[FunctionIR], module_name: &str) -> (TirModule, Vec<bool>)` — iterates the slice, lowers body-owning functions for module optimization, and returns the aligned `is_extern` vec. Extern `FunctionIR` entries carry canonical target-neutral signature metadata rather than executable bodies: they are excluded from inlining/body passes, preserved for declaration emission, and validated through `FunctionIR::extern_signature`. Native machine carriers and the complete target-neutral source signature are frozen once from body owners before object partitioning in `NativeBackendModuleContext`; its one transient TIR snapshot authors both linkage rows and leaf-call-graph facts, avoiding a second O(IR) lift. Every batch ingest proves the consumer declaration's arity, closure role, value/void result, and execution-context policy against that frozen row before LLVM or Cranelift can register it. LLVM calls, trampolines, declarations, provider definitions, entry coercion, and returns all require the same row. Shared-stdlib cache admission streams the complete versioned `FunctionIR` contract plus exact linkage row into the SHA-256 `stdlib-partition-v2-exact-linkage-abi` manifest, so new IR fields and stale provider ABIs cannot bypass cold/warm cache admission.
