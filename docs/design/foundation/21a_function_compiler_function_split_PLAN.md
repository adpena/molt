...
## Executive finding: premise corrected; M1.1-M1.15 now landed
...
exception control, and M1.15 value transfer are landed as standalone `fc/`
handlers. Do not undo it.
Three load-bearing facts:

1. **`function_compiler` is already a directory module, partially extracted.**
   `runtime/molt-backend/src/native_backend/function_compiler.rs` (now **9,684 lines**,
   down from doc 21's 39,043) declares `mod fc;` (line 10) → `function_compiler/fc/`, a
   subtree of **38 already-extracted handler files** (`arith.rs`, `compare.rs`,
   ...
   and value-custody transfer families through `fc::<family>::handle_*`
   handlers as well.
...
**Move #1 = continue the function-extraction of `compile_func_inner`** (now ~3,521
lines, lines 3125-6645, with the planned large opcode-family clusters extracted).
Strictly move-only / zero-logic-change / no-API-widening.

## 1. Current structure map

### 1.1 `function_compiler.rs` (9,684 lines)
...
| **`compile_func_inner()`** | **3125-6645** | THE MONOLITH (~3,521 lines). Preanalysis destructure (3133-~3227, ~45 shared `let mut` locals), pre-passes (3228-4195), dispatch loop `for op_idx in 0..ops.len()` at **4196**, central `match op.kind.as_str()` at **4372**, per-op **epilogue** at **6076** (post-dispatch: dec_ref of loop-reassigned vars, drain-cleanup, deferred define). |
| `drain_dead_block_temps_for_suspend()` | 6672-6714 | trailing helper |
| `#[cfg(test)] mod tests` | 6717-9684 | 63 tests |
...
Remaining inline families, current as of the M1.15 landing:

No planned M1 opcode-family cluster remains inline. Residual inline clusters (constants, env/runtime probes, print/warn/newline, bridge-unavailable, and block_on mini-clusters) require a fresh structural contract before movement; do not split semantic ownership just to chase line count.
...
dispatch match reduced to thin delegating arms, epilogue at 6076), `compile_func`, the §1.4
...
3. **`continue`/`break`/epilogue fidelity (correctness-critical):** outer op-loop is UNLABELED
   (4196). `OpFlow::Continue` ⇒ caller `continue` (skips epilogue 6076+); `OpFlow::Proceed` ⇒
...
Stop-anywhere: M1.1-M1.15 removed the largest arithmetic/compare/unary/function-object,
coroutine, call, memory, ret/jump, structured control-flow, loop, subscript, and
sequence/iterator families plus complete dict mutation, exception control, and value-custody transfer and converted them into separate codegen units. Future work should start
...
## Critical files
- `runtime/molt-backend/src/native_backend/function_compiler.rs` (shell + `compile_func_inner` 3125-6645)
