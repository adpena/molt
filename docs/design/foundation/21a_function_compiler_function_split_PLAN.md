...
## Executive finding: premise corrected; M1.1-M1.14 now landed
...
`indexing`, M1.12 `sequence_ops`, M1.13 residual dict mutation, and M1.14
exception control are landed as standalone `fc/` handlers. Do not undo it.
Three load-bearing facts:

1. **`function_compiler` is already a directory module, partially extracted.**
   `runtime/molt-backend/src/native_backend/function_compiler.rs` (now **9,771 lines**,
   ...
   read/write, sequence/iterator, complete dict mutation, and exception-control
...
**Move #1 = continue the function-extraction of `compile_func_inner`** (now ~3,661
lines, lines 3125-6785, with the planned large opcode-family clusters extracted).
Strictly move-only / zero-logic-change / no-API-widening.

## 1. Current structure map

### 1.1 `function_compiler.rs` (9,771 lines)
...
| **`compile_func_inner()`** | **3125-6785** | THE MONOLITH (~3,661 lines). Preanalysis destructure (3133-~3227, ~45 shared `let mut` locals), pre-passes (3228-4195), dispatch loop `for op_idx in 0..ops.len()` at **4196**, central `match op.kind.as_str()` at **4372**, per-op **epilogue** at **6218** (post-dispatch: dec_ref of loop-reassigned vars, drain-cleanup, deferred define). |
| `drain_dead_block_temps_for_suspend()` | 6812-6854 | trailing helper |
| `#[cfg(test)] mod tests` | 6857-9771 | 61 tests |
...
Remaining inline families, current as of the M1.14 landing:

No planned M1 opcode-family cluster remains inline. Residual inline clusters (constants, env/runtime probes, print/warn/newline, bridge-unavailable, block_on, and RC/box/identity transfer mini-clusters) require a fresh structural contract before movement; do not split semantic ownership just to chase line count.
...
dispatch match reduced to thin delegating arms, epilogue at 6218), `compile_func`, the §1.4
...
3. **`continue`/`break`/epilogue fidelity (correctness-critical):** outer op-loop is UNLABELED
   (4196). `OpFlow::Continue` ⇒ caller `continue` (skips epilogue 6218+); `OpFlow::Proceed` ⇒
...
Stop-anywhere: M1.1-M1.14 removed the largest arithmetic/compare/unary/function-object,
coroutine, call, memory, ret/jump, structured control-flow, loop, subscript, and
sequence/iterator families plus complete dict mutation and exception control and converted them into separate codegen units. Future work should start
...
## Critical files
- `runtime/molt-backend/src/native_backend/function_compiler.rs` (shell + `compile_func_inner` 3125-6785)
