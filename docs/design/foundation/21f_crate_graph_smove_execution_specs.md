...
- **From `molt-tir/src/tir/` -> `molt-ir/src/tir/`** (the vocabulary; 21b Layer-0 list):
  ...
  generator input -- moves with it), `values.rs` (20), `blocks.rs` (176), `function.rs`
...
**Current vocabulary:** the former `effect_proof.rs` static module/class tag is
retired with the speculative frontend class cache. Module lookup effects follow
the shared operation semantics, not a caller-supplied name-based exemption.
`printer.rs` is TIR-only; LIR formatting lives in `lir_printer.rs` and must stay
