...
4. **The capture-analysis substrate is live** (`molt-passes/.../escape_analysis/`): it supplies conservative allocation/alias facts from generated operand contracts. It does not prove thread confinement, storage lifetime, or RC freedom. Designs 20/49 own the lifetime boundary; the former automatic placement and heap-RC stripping lanes are retired.
...
- `molt-passes/.../escape_analysis/` — capture obligations shared by alias analysis and SROA; no placement or release-erasure permission. `ArgEscape` requires a positive callee noncapture proof.
