...
- **Rule 3a — One authority per semantic fact.** Opcode vocabulary, conservative effects and positive local-operand contracts are generated from `op_kinds.toml`. Operand-, alias-, and CFG-dependent facts come from their shared analyses, never an opcode-only substitute. Escape-state diagnostics project `escape_analysis::analyze`; physical nonheap eligibility projects representation facts. The retired deferred-RC heap-exposure and SROA immediate tables must not be recreated as pass-local classifiers.
