...
- **Rule 3a — No semantic branch below the registry.** Every parity-relevant semantic fact (does an op throw? does it allocate a fresh owned ref? is it side-effecting? is it a fusion barrier? what is its result repr? what dunder protocol does it dispatch?) is owned by the **op-kind registry** (`runtime/molt-ir/src/tir/op_kinds.toml` -> generated classifier tables) — *not* by per-pass/per-backend `matches!`. Doc 00 Risk 7 is the live exemplar and several of these already landed (STATUS.md: `fusion_barrier_opcodes`, `refcount_heap_exposure_opcodes`, `i64_zero_divisor_guard_opcodes`, `literal_payload_opcodes`, `exception_label_attr_opcodes` are all now generated exhaustive classifiers). This arc completes the set for the *remaining* parity-relevant facts and adds the gate (FACT 5) that forbids new `matches!`-on-opcode for a semantic fact.
