...
>
> **(C) WHAT IS NOT DONE — the ONE remaining activation gate: a single CLEAN full ~796-file `tests/differential/basic` sweep (wired no-worse vs dormant) + its formal dormant diff, blocked PURELY by this environment's SIGURG process-kill behavior — a TOOLING limitation, NOT a drop-pass issue.** In this environment the molt daemon's tokio runtime delivers `SIGURG` that hard-kills any long-running multi-build harness: `molt_diff.py` dies (~file 25-200), standalone Python scripts die even with `signal(SIGURG, SIG_IGN)` + `nohup`, and bash task-wrappers die even with `trap '' URG`. Only (i) SHORT bash loops (<~3 min, e.g. the 14-file memory corpus + 9-file peel completed), (ii) SHORT `pytest -n` runs (compliance's 46 tests / 114 s completed), and (iii) single `molt build` commands survive. A `pytest`-driven full-corpus gate (`/tmp/rcf4/test_corpus_gate.py`, parametrized over all 796 files, `--junitxml` so the result survives a broken stdout pipe) was the closest viable harness — `pytest -n 6` ITSELF survives SIGURG (only its bash wrapper dies) — but a full sweep is ~60-75 min (≈5 s/file build, link-dominated) and did not finish within the session. **Activation prerequisite (de-risked, mechanical): run `/tmp/rcf4/test_corpus_gate.py` to completion WIRED (`MOLT_CORPUS_SESSION=agent-rcf4`, junitxml) AND DORMANT (`MOLT_CORPUS_SESSION=agent-rcf4d`), diff the two junitxml failure sets — every wired failure must already be a dormant failure (no-worse) with ZERO new `invalid object header before dec_ref`/`refcount underflow` UAFs. Given (B) found zero regressions across ~180 files + the exhaustive memory/compliance gates, this is expected to be clean; it just needs an environment where a 60-min run is not SIGURG-killed (or a pytest `--forked`/batched driver).** Once green: activation = flip `target_uses_tir_drop_insertion`'s `NativeCranelift => false` to `true` (ONE line; everything else is already wired behind the `drop_inserted` marker), commit the 30M leak repros into `tests/differential/memory/`, and record the `MOLT_PROFILE` leak-closure numbers above. The native-RC retirement (Findings #2/#3 — `tracked_*` registration gate, `arg_cleanup` gate, slot-store/load gates, `rc_coalescing` skip) auto-engages via the `drop_inserted` marker the moment the gate flips; no further native-backend edits are needed. **The `f2b2d1b32` restack already activated LLVM/WASM/Luau** (they qualify: value-keyed / 1:1-NaN-boxed-local / GC); only NativeCranelift stays gated pending (C).
