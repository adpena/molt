...
>
> **(A) Backend executable cache custody.** Commit `fdbb51329` added binary
> identity to the shared-stdlib, module and function object-cache keys. Its
> original path/mtime/size stamp was insufficient: same-size preserved-mtime
> rewrites could reuse old code. The September consolidation replaces that stamp
> with the shared stable executable-content authority for probe receipts,
> object-cache variants and daemon selection. Backend setup also carries the
> content-bound compiler fingerprint into the Rust TIR cache through the existing
> compile-local environment contract. Source identity still selects rebuilds;
> it is not a substitute for the binary's bytes. Exact-key sidecars/manifests
> govern reuse without destructive cache sweeps. Historical daemon logs and the
> original 18-test receipt describe the earlier fix, not current-source execution
...
- `runtime/molt-passes/src/tir/analysis/mod.rs` — add `AnalysisId::Liveness` to the enum and `ALL` array. Implement `Analysis for TirLiveness`.
- `runtime/molt-passes/src/tir/passes/liveness.rs` (new file) — implement `TirLiveness`:
...
- `runtime/molt-passes/src/tir/passes/mod.rs` — add `pub mod liveness;`.
...
- `runtime/molt-passes/src/tir/passes/drop_insertion.rs` (new file):
...
- `runtime/molt-passes/src/tir/passes/mod.rs` — add `pub mod drop_insertion;`.
- `runtime/molt-passes/src/tir/pass_manager.rs` — add two new pass slots at the end of `build_default_pipeline`:
...
| `runtime/molt-passes/src/tir/passes/liveness.rs` | Create | `TirLiveness` analysis — backward dataflow liveness |
| `runtime/molt-passes/src/tir/analysis/mod.rs` | Modify | Add `AnalysisId::Liveness`, register `TirLiveness` |
| `runtime/molt-passes/src/tir/passes/drop_insertion.rs` | Create | `DropInsertion` pass — core of this substrate |
| `runtime/molt-passes/src/tir/passes/mod.rs` | Modify | Add `pub mod liveness; pub mod drop_insertion;` |
| `runtime/molt-passes/src/tir/pass_manager.rs` | Modify | Append `drop_insertion` + `refcount_elim_post` to `build_default_pipeline` |
