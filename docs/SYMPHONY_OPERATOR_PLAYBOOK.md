...
export MOLT_SYMPHONY_TRUSTED_USERS=adpena,symphony
export MOLT_SYMPHONY_TRUSTED_MACHINES=
export MOLT_SYMPHONY_AUTOLAND_ENABLED=1
export MOLT_SYMPHONY_AUTOLAND_MODE=direct-main
export MOLT_SYMPHONY_AUTOLAND_COMMIT_MESSAGE="chore: sync all changes"
export MOLT_SYMPHONY_AUTOLAND_PR_AUTOMERGE=1
export MOLT_SYMPHONY_AUTOLAND_PR_BASE=main
export MOLT_SYMPHONY_CODEX_ARGS="-c model_reasoning_effort=low -c model_reasoning_summary=none -c hide_agent_reasoning=true -c model_verbosity=low -c tool_output_token_limit=6000 -c model_auto_compact_token_limit=120000"
```

For recurring runs, load `ops/linear/runtime/symphony.env` generated by bootstrap.
Workflow hooks are cross-platform by default and run through:
`uv run --python 3.12 python tools/symphony_hooks.py before_run|after_run`.
...
- Keep automated merge acceptance gated to trusted identities via `MOLT_SYMPHONY_AUTOMERGE_ALLOWED_AUTHORS`, `MOLT_SYMPHONY_TRUSTED_USERS`, and `MOLT_SYMPHONY_TRUSTED_MACHINES`.
