...
- **Out-of-band process sentinel**: use `python3 tools/process_sentinel.py --once --kill-all` to stop repo-scoped Molt build/test/bench process groups from prior sessions, or run it without `--once` as a lightweight live guard. It streams to stderr/stdout only, writes no artifacts by default, and hard-clamps the cumulative group ceiling below 58 GB.
