...
- **Runtime artifact stability**: runtime Cargo feature sets are stable for the selected stdlib profile and target. User import graphs and required WASM exports must not mutate the runtime Cargo command or fingerprint; they only affect user-code lowering, linking/export validation, and cache keys for user artifacts. WASM runtime rebuilds use Cargo `compiler-artifact` JSON as post-build provenance and never pre-delete shared candidate `.wasm`/`.a` artifacts to manufacture freshness. Reusable sidecars identify standalone WASM by exact bytes and static archives by ordered linkable-member names and contents, excluding only derived archive metadata and local Molt rustc CGU discriminators. Candidates without that canonical content identity are rebuilt instead of hydrated. A successful Cargo run that reports no matching `molt_runtime` artifact fails closed instead of reusing an old path guess.
