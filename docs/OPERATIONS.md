...
- **Standalone guarded commands**: use `python3 tools/memory_guard.py --max-rss-gb <gb> --max-total-rss-gb <gb> --stream stderr -- <command...>` for arbitrary local commands. `--stream` emits live samples without creating artifacts; `--samples-jsonl <path>` writes bounded samples and rotates once by default (`--samples-max-mb`, default 2 MB).
