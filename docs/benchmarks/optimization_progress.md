...
- Status summary: runtime observability counters + profile JSON emission are landed/validated; linked-WASM stabilization remains partial (43/45 in last captured tranche); compile-time recovery tightening is landed in targeted slices; docs + planning canon are synchronized around swarm gating and regression firewall policy; benchmark-harness interpreter/dependency mismatch (`packaging.markers` missing in uv-only lane) is resolved (`tools/bench.py` and `tools/bench_wasm.py` now use `uv run --python 3.12 python3`); remaining Wave 0 blocker is long-tail release build churn.
...
| 2026-03-04 | WASM/native perf triage + tooling hardening | Captured fresh native vs CPython + linked WASM baselines on a focused kernel set (`sum`, `attr_access`, `bytes_find`, `dict_ops`, `matrix_math`), plus warm reruns and wasm CPU profiles. Native geomean on this slice is faster than CPython (`0.577x` ratio) but with persistent compile overhead (warm median build `~17.9s` per script); WASM remains far behind (`10.6x` CPython geomean, `18.4x` slower than native geomean). Fixed `tools/wasm_profile.py` API drift against `bench_wasm` (`build_runtime_wasm`/`prepare_wasm_binary` kwargs), added regression coverage, and added `tools/bench_wasm.py --node-allow-tiering` throughput mode (deterministic mode remains default). | Complete (baseline + tooling lane) | `/Volumes/APDataStore/Molt/bench/results/native_vs_cpython_20260304.json`, `/Volumes/APDataStore/Molt/bench/results/native_vs_cpython_20260304_warm2.json`, `/Volumes/APDataStore/Molt/bench/results/wasm_release_20260304.json`, `/Volumes/APDataStore/Molt/bench/results/wasm_release_20260304_warm2.json`, `/Volumes/APDataStore/Molt/logs/wasm_profile/bench_attr_access_20260304/`, `/Volumes/APDataStore/Molt/logs/wasm_profile/bench_dict_ops_20260304/`, `tools/wasm_profile.py`, `tools/bench_wasm.py`, `tests/test_wasm_profile_tool.py`, `tests/test_bench_wasm_node_resolver.py` | Implement P0 architecture slices: persistent multi-script compile session, fast-locals/lazy-locals materialization, dict subscript specialization, and wasm call-path de-JS-bridging. |
