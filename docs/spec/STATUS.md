...
- Build target semantics are explicit for Python `3.12`, `3.13`, and `3.14`:
  ...
  `sys.version`, and `sys.hexversion` from the same stamped state. Malformed or
  non-string target-version config fails closed instead of falling back to
  another target. The default target remains Python `3.12`.
