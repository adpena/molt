...
- Runtime intrinsic metadata and resolver generation now have separate generated
  ...
  generated per-leaf intrinsic sub-registry, with the aggregate resolver delegating
...
- Runtime text leaf ownership is now active for `html` and `unicodedata`: the
  aggregate-runtime duplicate modules are deleted, `molt_html_*` and
...
- Runtime zoneinfo leaf ownership is now active: the aggregate-runtime duplicate
  module is deleted, `molt_zoneinfo_*` resolver arms are gated by the dep-backed
...
- WASM remains a supported target area, but same-contract parity with native is
  still incomplete.
- WASM browser numeric-kernel calls route through the split-runtime embed
...
- Backend daemon custody is identity-based and centralized in
  ...
  leaf-owned: the `molt-runtime` aggregate fallback module is deleted,
...
- Stdlib lowering audit: `920` modules audited; `41` intrinsic-backed; `875` intrinsic-partial; `1` policy-gate; `0` python-only.
