...
- Implemented: Orchestration bootstrap/runtime env defaults now include DSPy routing
  keys (`MOLT_REMOVED_DSPY_ENABLE`, `MOLT_REMOVED_DSPY_MODEL`,
  `MOLT_REMOVED_DSPY_API_KEY_ENV`) and docs now run `linear_hygiene` via
...
- Implemented: readiness now emits deterministic next-tranche action plans
  ...
  `logs/orchestration/readiness/next_tranche.{json,md}` to drive recursive execution.
...
- Implemented: recursive loop now records tool-promotion distillations under
  `/Volumes/APDataStore/orchestration/molt/state/tool_promotion/` and readiness now
...
- Implemented: readiness now emits `improvement_issue_sync` as a dry-run/apply
  ...
  default behavior.
- Implemented: launchd-managed Orchestration now uses a self-healing control plane:
  launchd keeps minimal supervisor logs under `~/Library/Logs/Molt/orchestration-launchd`,
...
- Implemented: Molt-specific runtime helpers now live under `moltlib`, including `moltlib.molt_db`, the runtime-native `moltlib.concurrency` surface, the streaming/websocket helper surface `moltlib.net`, and the ASGI adapter `moltlib.asgi`; `molt.molt_db`, `molt.net`, `molt.concurrency`, and `molt.asgi` remain as compatibility shims for existing imports, while the root `molt` package stays reserved for compiler/runtime core.
...
- CPython shim: minimal ASGI adapter for http/lifespan via `moltlib.asgi.asgi_adapter` (`molt.asgi.asgi_adapter` retained as a compatibility shim).
- `molt_accel` client/decorator expose before/after hooks, metrics callbacks (including payload/response byte sizes), cancel-checks with auto-detection of request abort helpers, concurrent in-flight requests in the shared client, optional worker pooling via `MOLT_ACCEL_POOL_SIZE`, and raw-response pass-through; timeouts schedule a worker restart after in-flight requests drain; wire selection honors `MOLT_WORKER_WIRE`/`MOLT_WIRE`; idempotent retry/backoff knobs are available per-call and via env (`MOLT_ACCEL_RETRY_ON_TIMEOUT`, `MOLT_ACCEL_RETRY_ON_BUSY`, `MOLT_ACCEL_RETRY_BACKOFF_MS`, `MOLT_ACCEL_RETRY_BACKOFF_MAX_MS`).
...
- C API: bootstrap `libmolt` C-extension surface is landed (`runtime/molt-runtime/src/c_api.rs` + `include/molt/molt.h`) for runtime/GIL, errors, scalar constructors/accessors (`none`/bool/int/float), object protocol (including bytes-name attribute helpers, iterator primitives `molt_object_get_iter`/`molt_iterator_next`, compare/contains bool helpers, and native capsule primitives `molt_capsule_new`, `molt_capsule_get_name_ptr`, `molt_capsule_get_pointer`, `molt_capsule_is_valid`, `molt_capsule_get_context`, `molt_capsule_set_context`, `molt_capsule_import`), numerics, sequence/mapping, array-to-container constructors, iterable-to-sequence materializers (`molt_sequence_to_list`, `molt_sequence_to_tuple`), buffer/bytes wrappers, plus type/module parity wrappers (`molt_type_ready`, module create/add/get APIs, runtime-owned module metadata/state registries, and `PyState_*` backing APIs). A CPython-style source-compat shim header is now available at `include/Python.h` / `include/molt/Python.h` (including partial `PyErr_*`, `PySequence_*`, list-materializing `PyMapping_Keys`/`PyMapping_Values`/`PyMapping_Items`, `PyMapping_HasKey`/`PyMapping_HasKeyString`, `PyDict_Keys`/`PyDict_Values`/`PyDict_Items`, `PyByteArray_FromStringAndSize`/`PyByteArray_AsString`/`PyByteArray_Size`, `PyArg_ParseTuple`, O(n + k) `PyArg_ParseTupleAndKeywords`, module-creation/access helpers such as `PyModule_New(Object)`, `PyModule_GetDef`, `PyModule_GetState`, `PyModule_SetDocString`, `PyModule_GetFilename(Object)`, `PyModule_FromDefAndSpec(2)`, `PyModule_ExecDef`, `PyState_*`, callback-backed `PyModule_AddFunctions`, expanded heap-type creation via `PyType_FromSpec(WithBases)` selected slot lowering + `METH_CLASS`/`METH_STATIC`, module-associated type APIs `PyType_FromModuleAndSpec`/`PyType_GetModule*`, runtime-backed `PyCapsule_*` helpers, `PyObject_Vectorcall`, `PyObject_Dir`, `PyObject_Format`, `PyObject_GetIter`, `PyIter_Next`, `PyObject_Length`, `PyObject_Size`, `PySequence_List`, `PySequence_Tuple`, basic lock/mutex shims, `PyTraceMalloc_Track`, `PyTraceMalloc_Untrack`, `PyNumberMethods`, `PyTuple_Type`/`PyTupleObject`, `PyType_Type`, `PyType_Check`, `PyVarObject`, `Py_SIZE`, `Py_uhash_t`, `Py_HUGE_VAL`, `PyUnicode_GET_LENGTH`, `PyUnicode_AsEncodedString`, `PyUnicode_FromWideChar`, and `Py_VISIT`).
- C API / NumPy source-compat: `include/numpy/*` now covers the initial public bootstrap plus a broader NumPy 2.x compile surface for DType metadata, array-method structs/flags, order/casting converters, descriptor promotion/adaptation helpers, writeback/buffer helpers, public array accessors, iterator/neighborhood structs, public DType aliases, `PyArray_GetItemFunc`, `PyArrayMapIterObject`, `PyArrayInterface`, `PyArrayNeighborhoodIterObject`, `PyArrayNeighborhoodIter_Next`, `PyArrayNeighborhoodIter_Reset`, `PyArray_Any`, `PyArray_BufferConverter`, `PyArray_ByteorderConverter`, `PyArray_CFloatDType`, `PyArray_CDoubleDType`, `PyArray_CLongDoubleDType`, exact `NPY_DEVICE`/`NPY_CORRELATEMODE` enums, tagged `enum NPY_TYPES`, LP64-correct integer/format sizing, `npy_dtype_info`, `frameobject.h`, top-level `arrayobject.h`, `pymem.h`, wrapper entry points (`PyArray_Converter`, `PyArray_CorrelatemodeConverter`, `PyArray_FromAny_int`, `PyArray_FromArrayAttr(_int)`, `PyArray_Empty_int`, `PyArray_DTypeOrDescrConverterRequired`, `PyArray_DTypeFromObjectStringDiscovery`, `PyArray_EquivTypenums`, `PyArray_Flatten`, `PyArray_CumSum`, `PyArray_CumProd`, `PyArray_GenericAccumulateFunction`, `PyArray_Diagonal`), dtype aliases (`PyArray_DoubleDType`, `PyArray_DatetimeDType`, `PyArray_FloatAbstractDType`), selected internal compile helpers (`PyArray_CopyObject`, `PyArray_PromoteDTypeSequence`, `PyArray_GetCastingImpl`, `PyArray_CastDescrToDType`, `PyArray_AssignRawScalar`, `PyArray_GetStridedCopyFn`, `PyArray_CastRawArrays`, `PyArray_PrepareTwoRawArrayIter`, `PyArray_LookupSpecial(_OnInstance)`), `ndarrayobject.h`-level `import_array*` wiring, `arrayscalars.h`, `numpyconfig.h`, `utils.h`, `NPY_UNUSED` / `NPY_TLS` / visibility helpers, and ufunc constructor/registration stubs (`PyUFunc_FromFuncAndData`, `PyUFunc_FromFuncAndDataAndSignature`, `PyUFunc_AddLoopsFromSpecs`). `include/molt/Python.h` now also covers `PyUnicode_InternFromString`, `PySys_GetObject`, a concrete `PyObject_HEAD`/`PyCodeObject` shape, and `_PyDict_GetItemWithError` aliasing, which is enough for real NumPy core string/sys bootstrap code such as `numpy/_core/src/multiarray/npy_static_data.c` to compile under `clang -fsyntax-only -Wno-sign-compare`. Archived extension scans on 2026-03-06 now report NumPy `2.4.2` at `256` missing `Py*` symbols (coverage `0.604`) and pandas `3.0.1` at `0` missing symbols (coverage `1.000`). Real `clang -fsyntax-only` validation now passes NumPy limited-API examples (`limited_api1.c`, `limited_api_latest.c`) plus every checked pandas `_libs/src` translation unit except `parser/tokenizer.c`, whose remaining failure is a source/build-env `inttypes.h` formatting dependency rather than a `libmolt` header gap. Runtime execution of many ndarray kernels remains explicitly unavailable, and the next NumPy core frontier is now concentrated in `NpyAuxData` lifecycle helpers, richer `PyUFuncObject` field shapes, and `pythoncapi-compat`/thread-state closure rather than the earlier public-header/bootstrap gaps.
- C API / NumPy source-compat (latest 2026-03-06 tranche): the bounded NumPy contract now explicitly includes the upstream-shipped public headers `numpy/arrayobject.h`, `numpy/dtype_api.h`, `numpy/npy_2_compat.h`, `numpy/npy_cpu.h`, `numpy/npy_math.h`, `numpy/numpyconfig.h`, and `numpy/utils.h`, plus upstream-derived overlays such as `numpy/_public_dtype_api_table.h`, `numpy/halffloat.h`, `numpy/npy_2_complexcompat.h`, `numpy/npy_3kcompat.h`, `numpy/npy_endian.h`, `numpy/npy_no_deprecated_api.h`, `numpy/npy_os.h`, `numpy/random/bitgen.h`, and `numpy/random/distributions.h`. Internal-source compatibility now also has a compile-focused `templ_common.h` bridge, `PyArray_ArrFuncs`/dtype-flag/source-shape alignment, `NPY_SELECTKIND`, `NPY_DATETIME_NUMUNITS`, and prototype-only internal-build lanes for NumPy-owned functions such as `PyArray_DescrFromScalar`, `PyArray_DescrFromTypeObject`, `PyArray_Return`, and `PyArray_Scalar`. Archived extension scans now report NumPy `2.4.2` at `420/647` supported (`227` missing, coverage `0.649`, `34` contract headers consulted) and pandas `3.0.1` at `160/160` (`0` missing, coverage `1.000`). Real `clang -fsyntax-only` validation now passes NumPy `scalarapi.c` in addition to the earlier limited-API and `ufunc_type_resolution.c` probes, while the next checked frontier has moved to deeper internal closure in `descriptor.c` (`npy_packed_static_string`, `PyDict_Merge`, `PyArray_DescrNewFromType`, `PyArray_DescrConverter2`, `PyDataType_ISDATETIME`, `PyMemoryView_Type`) rather than the earlier public-header/bootstrap gaps.
...
- Cancellation tokens are available with request-scoped defaults and task-scoped
  ...
  `moltlib.concurrency.cancelled()` remain available (`molt.concurrency` stays as
...
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): scan-driven CPython header parity sprint expanded `include/molt/Python.h` with high-impact wrappers/macros for type/ref helpers, tuple/list/dict fast helpers, memory allocators, thread/GIL shims, compare/call builders (`PyObject_CallFunction*`, `Py_BuildValue` subset), capsule/buffer helpers, module/capsule import helpers (`PyImport_ImportModule`, `PyCapsule_Import`), `PyArg_UnpackTuple`, iter/number/float helpers, `PyObject_Dir`, `PyObject_Format`, `PyObject_GetIter`, `PyObject_Length`, `PyObject_Size`, `PyTraceMalloc_Track`, `PyTraceMalloc_Untrack`, `PyTupleObject`, `PyVarObject`, `Py_SIZE`, `Py_uhash_t`, `Py_HUGE_VAL`, `PyUnicode_AsEncodedString`, `PyUnicode_FromWideChar`, and set/complex checks; the latest capsule hardening pass now routes `PyCapsule_New`/`PyCapsule_GetName`/`PyCapsule_GetPointer`/`PyCapsule_IsValid`/`PyCapsule_GetContext`/`PyCapsule_SetContext`/`PyCapsule_Import` through runtime-native `molt_capsule_*` wrappers instead of header-local dict emulation. Initial NumPy compatibility headers (`include/numpy/*`) are now wired into extension scans and CI extension matrix lanes with added DType/scalar helper stubs, a new `include/numpy/dtype_api.h` lane, broader NumPy 2.x DType/method/type structs, order/casting/converter helpers, descriptor promotion/adaptation helpers, iterator/neighborhood structs, and array metadata/accessor macros (`PyArray_BASE`, `PyArray_DTYPE`, `PyArray_IS*`, `PyDataType_NAMES/FIELDS/ISUNSIZED`, `PyUFunc_Type`, `PyUFunc_None`, `PyArray_NewLikeArray`, `PyArray_View`, `PyArray_CanCastTypeTo`, `PyArray_CanCastArrayTo`, `PyArray_OrderConverter`, `PyArray_PromoteTypes`, `PyArray_SafeCast`, `PyArray_Newshape`, `PyArray_CopyConverter`, `PyArray_DTypeFromObject`, `PyArray_DTypeOrDescrConverterOptional`, `PyArray_ExtractDTypeAndDescriptor`, `PyArray_OverflowMultiplyList`, `PyArray_DiscardWritebackIfCopy`, `PyArray_CheckFromAny`, `PyArray_CopyAnyInto`, `PyArrayInterface`, `PyArrayNeighborhoodIterObject`, `PyArrayNeighborhoodIter_Next`, `PyArrayNeighborhoodIter_Reset`, `PyArray_Any`, `PyArray_BufferConverter`, `PyArray_ByteorderConverter`, `PyArray_CFloatDType`, `PyArray_CDoubleDType`, `PyArray_CLongDoubleDType`, `PyUFunc_AddLoopsFromSpecs`), plus a minimal `datetime.h` shim lane (`PyDateTimeAPI`/`PyDateTime_IMPORT`/basic checks); additional scan signal hardening filters local `#define Py*`/`static Py*` helpers and current archived scans now reduce NumPy/Pandas missing `Py*` counts (NumPy `2.4.2`, pandas `3.0.1`) from `1074/27` to `734/13`.
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): follow-on NumPy header tranche tightened exact converter/type shapes with `NPY_DEVICE`, `NPY_CORRELATEMODE`, `npy_dtype_info`, `PyArray_DoubleDType`, `PyArray_DatetimeDType`, `PyArray_FloatAbstractDType`, `PyArray_Converter`, `PyArray_CorrelatemodeConverter`, `PyArray_FromAny_int`, `PyArray_FromArrayAttr(_int)`, `PyArray_Empty_int`, `PyArray_DTypeOrDescrConverterRequired`, `PyArray_DTypeFromObjectStringDiscovery`, `PyArray_EquivTypenums`, `PyArray_Flatten`, `PyArray_CumSum`, `PyArray_CumProd`, `PyArray_GenericAccumulateFunction`, `PyArray_Diagonal`, and `PyTime_Check`, with corresponding `clang` smoke coverage. Archived scans on 2026-03-05 further improved NumPy/Pandas missing `Py*` counts from `734/13` to `702/12`.
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): the 2026-03-06 libmolt burndown tightened real-source NumPy/pandas compile coverage with scanner hardening for project-shared/header-local `Py*` names, `ndarrayobject.h`-level `import_array*` support, `arrayscalars.h`, `numpyconfig.h`, `utils.h`, datetime/int-format/type aliases (`npy_float64`, `NPY_INT32_FMT`, `NPY_MIN_INT64`, `NPY_MAXDIMS`, `NPY_INT64`), `NPY_UNUSED` / `NPY_TLS` / visibility helpers, scalar-token-compatible `PyArray_IsScalar`, `PyDataType_C_METADATA`/datetime metadata structs, `structmember.h` aliases, `PyUnicode_InternFromString`, `PySys_GetObject`, and `Py_T_*` / `Py_READONLY` macros plus fail-fast `PyMember_GetOne` / `PyMember_SetOne` stubs. Archived scans now report NumPy `2.4.2` at `256` missing `Py*` symbols (coverage `0.604`) and pandas `3.0.1` at `0`, while `clang -fsyntax-only` now passes NumPy `limited_api1.c` / `limited_api_latest.c`, NumPy `numpy/_core/src/multiarray/npy_static_data.c` (with `-Wno-sign-compare` to ignore upstream warning noise), and pandas `date_conversions.c`, `pd_datetime.c`, `io.c`, `pd_parser.c`, vendored NumPy datetime sources, and vendored `ujson` sources. The remaining checked pandas blocker is `parser/tokenizer.c` unless the compile command force-includes `inttypes.h`, and the next NumPy core frontier is now split between generated-build inputs such as `numpy/_core/src/common/config.h` / `npy_cpu_dispatch_config.h` and deeper internal-header closure (`npy_3kcompat.h`, `frameobject.h`, richer `PyUFuncObject` field shapes, `NPY_OFF_T_PYFMT`).
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): follow-on libmolt C-API reconciliation aligned the shipped header/runtime/spec surface for mapping/dict helpers by upgrading `include/molt/Python.h` with real `PyDict_DelItem` / `PyDict_DelItemString` wrappers plus `PyMapping_HasKey` / `PyMapping_HasKeyString`, making the runtime/native `PyMapping_Keys` / `PyMapping_Values` / `PyMapping_Items` and `PyDict_Keys` / `PyDict_Values` / `PyDict_Items` symbols materialize CPython-shaped lists instead of internal dict-view objects, and syncing the C-API matrix away from stale `Missing` markers for already-landed `PySequence_DelItem`, `PyMapping_Check`, and `PyByteArray_*` coverage.
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): the next 2026-03-06 libmolt burndown tranche added LP64-correct NumPy integer sizing (`npy_int64`/`NPY_INT64`, `PyInt64ArrType_Type`, `PyIntpArrType_Type`), `NPY_SIZEOF_OFF_T`/pointer-size config metadata, tagged `enum NPY_TYPES`, `PyArray_ArrFuncs` `getitem`/`setitem` slots, a repo `frameobject.h`, top-level `arrayobject.h` / `pymem.h` forwarding, guarded separation between extension-only and NumPy-internal DType/ufunc helper names, and a concrete `PyObject`/`PyCodeObject` header shape in `include/molt/Python.h`. Targeted smoke coverage remains green (`6` extension-header tests passing), `ufunc_type_resolution.c` now advances into `NpyAuxData` + `PyUFuncObject` layout gaps instead of earlier public-header collisions, and `scalarapi.c` now stops in `pythoncapi-compat`/thread-state compatibility rather than missing `frameobject.h` or LP64 sizing metadata.
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): the latest 2026-03-06 libmolt burndown added generated NumPy config bridge headers (`_numpyconfig.h`, `config.h`, `npy_cpu_dispatch_config.h`, `numpy/npy_cpu.h`), `Python.h` coverage for `inttypes.h`, `PY_VERSION_HEX`, `PyErr_BadInternalCall`, selected weakref/unicode/private-helper shims, and NumPy-core `PYTHONCAPI_COMPAT` suppression so vendored `pythoncapi-compat` no longer blocks `_MULTIARRAYMODULE` / `_UMATHMODULE` syntax-only probes. The same tranche added `NpyAuxData` lifecycle macros, `NPY_MAXARGS`, and upstream-shaped legacy `PyUFunc_Loop1d` / `PyUFuncObject` source layouts. Archived scans now report NumPy `2.4.2` at `239` missing `Py*` symbols (coverage `0.631`) and pandas `3.0.1` at `0`; `clang -fsyntax-only` now also passes pandas `parser/tokenizer.c` and NumPy `numpy/_core/src/umath/ufunc_type_resolution.c`. The remaining checked NumPy frontier is now explicitly split between private/generated build artifacts (`arraytypes.h` in `conversion_utils.c`) and deeper internal source-shape gaps in `scalarapi.c` (`PyArray_DiscoverDTypeAndShape`, `PyArray_AssignFromCache`, `PyArray_NewFromDescr*`, `npy_dtype_info`, `NPY_NSCALARKINDS`, and `PyArrayScalar_VAL` macro-family coverage).
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): the follow-on 2026-03-06 libmolt burndown promoted additional upstream-shipped NumPy contract headers (`numpy/arrayobject.h`, `numpy/dtype_api.h`, `numpy/npy_2_compat.h`, `numpy/npy_cpu.h`, `numpy/npy_math.h`, `numpy/numpyconfig.h`, `numpy/utils.h`) and added upstream-derived overlay headers (`numpy/_public_dtype_api_table.h`, `numpy/halffloat.h`, `numpy/npy_2_complexcompat.h`, `numpy/npy_3kcompat.h`, `numpy/npy_endian.h`, `numpy/npy_no_deprecated_api.h`, `numpy/npy_os.h`, `numpy/random/bitgen.h`, `numpy/random/distributions.h`) plus a compile-focused `templ_common.h` bridge for generated-source probes. The same tranche aligned `PyArray_ArrFuncs`, dtype flags, `NPY_SELECTKIND`, `NPY_DATETIME_NUMUNITS`, arrayscalar/source shapes, and internal-build prototype-only lanes for NumPy-owned functions. Archived scans now report NumPy `2.4.2` at `227` missing `Py*` symbols (coverage `0.649`, `420/647`, `34` contract headers consulted) and pandas `3.0.1` at `0`; `clang -fsyntax-only` now also passes NumPy `numpy/_core/src/multiarray/scalarapi.c` while keeping `numpy/_core/src/umath/ufunc_type_resolution.c` and pandas `parser/tokenizer.c` green. The checked NumPy frontier is now centered on deeper internal closure in `descriptor.c` (`npy_packed_static_string`, `PyDict_Merge`, `PyArray_DescrNewFromType`, `PyArray_DescrConverter2`, `PyDataType_ISDATETIME`, `PyMemoryView_Type`) instead of the earlier public-header/bootstrap gaps.
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): defined the libmolt extension ABI/header contract in `0217` and wired tooling to report it. `include/molt/molt.h` is now the explicit stable ABI tier, `Python.h` plus the small legacy forwarding headers are documented as bounded source-compat facades, NumPy headers are documented as ecosystem overlays, `molt extension scan` now consults an explicit contract header list instead of an open-ended recursive crawl, and `molt extension build` records the contract in `extension_manifest.json`. Private/generated upstream headers remain outside the contract by policy.
...
- Implemented(backend, owner:compiler, milestone:TL2, priority:P2, status:partial): `--target luau` is now an explicit preview backend contract instead of a silent best-effort lane. The backend rejects lowered output that still contains unsupported comment/stub markers, CLI help now exposes `luau` in `--target`/`--emit`, backend-daemon retries preserve the `is_luau` target flag, and targeted backend tests now pin the bootstrap tree-shake plus fail-fast preview markers. Broader Python parity and a validated Luau subset spec remain follow-on work.
...
- Implemented(c-api, owner:runtime, milestone:SL3, priority:P1, status:partial): scan-driven CPython/NumPy header parity sprint expanded `include/molt/Python.h` with high-impact wrappers/macros for type/ref helpers, tuple/list/dict fast helpers, memory allocators, thread/GIL shims, compare/call builders (`PyObject_CallFunction*`, `Py_BuildValue` subset), capsule/buffer and module/capsule import helpers, `PyArg_UnpackTuple`, iter/number/float helpers, capsule context helpers, `PyErr_Print`/normalization/thread-save-restore coverage, and `ModuleNotFoundError`/`IOError` shims; NumPy compatibility headers under `include/numpy/*` now include broader dtype/type-object exports, `PyDataType_*` and `PyDataMem_*` helpers, array flag/conversion/copy/setup shims, upstream-shaped internal include wrappers (`npy_common.h`, `dtype_api.h`, `__multiarray_api.h`, `__ufunc_api.h`, `npy_2_compat.h`, `npy_math.h`), fail-fast `PyUFunc_*` constructor/registration surface, broader NumPy 2.x DType/method/type structs, order/casting/converter helpers, descriptor promotion/adaptation helpers, array metadata/accessor macros, and a minimal `datetime.h` shim lane. C-source sdist scans on 2026-03-05 reduced NumPy `2.4.2` missing `Py*` tokens from `1193` to `873` while holding pandas `3.0.1` at `28`, and grew the exported header surface from `416` to `770` tokens; heavy ndarray allocation/ownership, transfer machinery, and real ufunc execution remain explicit fail-fast gaps pending runtime backing.
...
- TODO(offload, owner:runtime, milestone:SL1, priority:P1, status:partial): broaden Django test-client chaos matrix (timeouts/busy/cancel paths) + non-demo handler coverage; baseline/offload parity and retry-on-restart coverage are now landed. `molt_accel` ships as an optional dependency group (`pip install .[accel]`) with a packaged default exports manifest so the decorator can fall back to `molt-worker` in PATH when `MOLT_WORKER_CMD` is unset. A demo app scaffold lives in `demo/`.
- TODO(offload, owner:runtime, milestone:SL1, priority:P1, status:partial): compile entrypoints into molt_worker.
- TODO(offload, owner:runtime, milestone:SL1, priority:P1, status:partial): finalize accel retry/backoff policy rollout across non-demo handler coverage.)
