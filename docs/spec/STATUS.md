...
- C API / NumPy source-compat: `include/numpy/*` now covers the initial public bootstrap plus a broader NumPy 2.x compile surface for DType metadata, array-method structs/flags, order/casting converters, descriptor promotion/adaptation helpers, writeback/buffer helpers, public array accessors, iterator/neighborhood structs, public DType aliases, `PyArray_GetItemFunc`, `PyArrayMapIterObject`, `PyArrayInterface`, `PyArrayNeighborhoodIterObject`, `PyArrayNeighborhoodIter_Next`, `PyArrayNeighborhoodIter_Reset`, `PyArray_Any`, `PyArray_BufferConverter`, `PyArray_ByteorderConverter`, `PyArray_CFloatDType`, `PyArray_CDoubleDType`, `PyArray_CLongDoubleDType`, selected internal compile helpers (`PyArray_CopyObject`, `PyArray_PromoteDTypeSequence`, `PyArray_GetCastingImpl`, `PyArray_CastDescrToDType`, `PyArray_AssignRawScalar`, `PyArray_GetStridedCopyFn`, `PyArray_CastRawArrays`, `PyArray_PrepareTwoRawArrayIter`, `PyArray_LookupSpecial(_OnInstance)`), and ufunc constructor/registration stubs (`PyUFunc_FromFuncAndData`, `PyUFunc_FromFuncAndDataAndSignature`, `PyUFunc_AddLoopsFromSpecs`). Current archived extension scans improved from NumPy/pandas missing `Py*` counts of `906/26` to `875/13` (`NumPy 2.4.2` coverage `0.377 -> 0.398`, pandas `3.0.1` coverage `0.850 -> 0.925`) while runtime execution of many ndarray kernels remains explicitly unavailable and the residual NumPy misses are now concentrated in deeper private internals such as `PyArrayIdentityHash*`, `PyArray_AddLegacyWrapping_CastingImpl`, `PyArray_AssignZero`, `PyArray_CopyAsFlat`, `PyArray_DeviceConverterOptional`, and `PyArray_DiscoverDTypeFromScalarType`.
