...
- Implemented: `pathlib` now routes core path algebra and file-open behavior through Rust intrinsics (`molt_path_join`, `molt_path_isabs`, `molt_path_dirname`, `molt_path_splitext`, `molt_path_abspath`, `molt_path_parts`, `molt_path_parents`, `molt_path_relative_to`, `molt_path_with_name`, `molt_path_with_suffix`, `molt_path_expanduser`, `molt_path_match`, `molt_path_glob`, `molt_file_open_ex`); targeted differential lane (`os`/`time`/`traceback`/`pathlib`/`threading`) ran `24/24` green with RSS caps enabled.
