...
- `molt_tir_fact_graph` JSON with schema version, function name, per-value
  producers/consumers, fact provenance, and summary counts. Schema v3 requires
  `source_site` fields on producers, consumers, and facts, carries `source_file`
  when frontend source identity is available, records `event_id` on event facts,
  and emits allocation/ownership facts generated from the TIR op-kind authority
  rather than command-local classifiers.
