...
| G1 | `docs-gates` | `uv run --python 3.12 python3 tools/update_status_blocks.py --check && uv run --python 3.12 python3 tools/bench_report.py --manifest bench/results/docs_manifest.json --check --update-status-doc && uv run --python 3.12 python3 tools/check_docs_architecture.py && uv run --python 3.12 python3 -m pytest -q tests/test_update_status_blocks.py tests/test_bench_report.py tests/test_check_docs_architecture.py tests/test_compatibility_contract_docs.py` |
...
| G3 | `diff-basic-stdlib` | `MOLT_DIFF_MEASURE_RSS=1 MOLT_DIFF_RLIMIT_GB=10 MOLT_DIFF_TIMEOUT=180 MOLT_DIFF_FAILURES=ir_probe_failures.txt uv run --python 3.12 python3 -u tests/molt_diff.py tests/differential/basic tests/differential/stdlib && uv run --python 3.12 python3 tools/check_molt_ir_ops.py --require-probe-execution --probe-rss-metrics rss_metrics.jsonl --failure-queue ir_probe_failures.txt` |
...
- GitHub Actions remains required for cheap required gates plus hosted build/release validation across supported environments; do not remove release-relevant hosted lanes just because local verification exists.
- The dedicated `docs-gates` job owns documentation architecture enforcement and generated-doc freshness checks.
- `tools/dev.py lint` should continue to include the same docs architecture checks for local developer workflows.
...
- Dedicated IR probe hardening is a required CI gate after the differential lanes:
...
- Run curated `molt-diff` parity lanes against CPython 3.12+ (minimum 3.12):
  `tests/differential/basic/` and `tests/differential/stdlib/`.
