...
- Implemented: wasm-side `molt_db` shims consume the response stream and surface results as bytes/Arrow IPC.
...
- Implemented: browser host adapter in `wasm/browser_host.js` that forwards `db_query`/`db_exec` via fetch or a JS adapter with cancellation polling.
