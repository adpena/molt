...
| G3 | Tier 0/1 differential parity (basic lane) | `MOLT_DIFF_MEASURE_RSS=1 MOLT_DIFF_RLIMIT_GB=10 MOLT_DIFF_TIMEOUT=180 MOLT_DIFF_FAILURES=ir_probe_failures.txt uv run --python 3.12 python3 -u tests/molt_diff.py tests/differential/basic && uv run --python 3.12 python3 tools/check_molt_ir_ops.py --require-probe-execution --probe-rss-metrics rss_metrics.jsonl --failure-queue ir_probe_failures.txt` | Exit 0; diff parity green with RSS recorded, and required IR probes executed with `status=ok` and absent from failure queue. |
