...
| G5 | CPython parity regression lane (periodic/pre-release) | `python tools/proof_plan.py --run-family nightly_regrtest --receipt proof-receipts/g5.json` | Exit 0; exact commit-pinned CPython source, expected skips only, summary/JUnit evidence, and receipt succeed. |
