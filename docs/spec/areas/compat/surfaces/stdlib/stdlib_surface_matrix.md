...
| tkinter | Capability-gated | Partial | P3 | SL3 | stdlib | Phase-0 intrinsic-backed `_tkinter` + `tkinter`/`tkinter.ttk` wrappers are landed, and runtime now includes an opt-in native Tcl/Tk backend path (`molt_tk_native`) for interpreter-backed command dispatch, callback/event pumping, and Tk lifecycle operations. Focused differential coverage (`tests/differential/stdlib/tkinter_phase0_core_semantics.py`) validates import/error-shape/capability-gate contracts without requiring a GUI backend. Broader tkinter submodule parity and universally reliable native Tk toolchain enablement remain pending. |
