...
in [docs/spec/areas/compat/plans/stdlib_lowering_plan.md](docs/spec/areas/compat/plans/stdlib_lowering_plan.md).
...
| array | Stdlib | Partial | P1 | SL1 | runtime | Rust intrinsics cover constructor, typecode/itemsize, indexing, slice get/set/delete, repeat, bytes/list round-trip, and mutation methods; remaining work is deterministic layout plus full buffer-protocol parity. (TODO(stdlib-compat, owner:runtime, milestone:SL1, priority:P1, status:partial): finish deterministic array layout and full buffer-protocol parity.) |
...
| statistics | Stdlib | Partial | P2 | SL2 | stdlib/runtime | Intrinsic-backed function surface (`mean`, `fmean`, `stdev`, `variance`, `pvariance`, `pstdev`, `median`, `median_low`, `median_high`, `median_grouped`, `mode`, `multimode`, `quantiles`, `harmonic_mean`, `geometric_mean`, `covariance`, `correlation`, `linear_regression`) with shim-level `StatisticsError` mapping; `NormalDist.samples` is version-gated for Python 3.12+ parity through `molt_statistics_normal_dist_samples` in both lanes (`gauss` route for 3.12/3.13, runtime-owned inverse-CDF route for 3.14+ with seeded/unseeded uniform draws). Remaining parity is broader 3.12+ API/PEP coverage and edge-case message alignment. (TODO(stdlib-compat, owner:stdlib, milestone:SL2, priority:P2, status:partial): complete full statistics 3.12+ API/PEP parity beyond function surface lowering.) |
...
| os | Capability-gated | Partial | P2 | SL3 | stdlib/runtime | Intrinsic-backed shim with ~40 intrinsic-backed APIs: env access gated via `env.read`/`env.write` with runtime-backed `os.environ` state (`molt_env_snapshot`/`molt_env_set`/`molt_env_unset`) and dedicated `os.putenv`/`os.unsetenv` intrinsics (`molt_env_putenv`/`molt_env_unsetenv`) matching CPython-visible mapping separation; path helpers (`basename`/`split`/`relpath`/`expandvars`/`realpath`) + fs helpers (`exists`/`isdir`/`isfile`/`unlink`/`remove`/`access`/`link`/`truncate`/`removedirs`/`walk`/`scandir`/`utime`) + fd ops (`close`/`pipe`/`dup`/`dup2`/`read`/`write`/`lseek`/`ftruncate`/`isatty`/`fdopen`/`get_inheritable`/`set_inheritable`/`sendfile`) + process/signal ops (`kill`/`waitpid`/`getpgrp`/`setpgrp`/`setsid`/`getppid`/`getuid`/`getgid`/`geteuid`/`getegid`/`getlogin`/`getloadavg`/`umask`) + system info (`cpu_count`/`uname`/`get_terminal_size`/`devnull`/`sysconf`/`sysconf_names`/`F_OK`/`R_OK`/`W_OK`/`X_OK`). `mkdir`/`makedirs` mode handling lowers through Rust intrinsics (`molt_path_mkdir(path, mode)` / `molt_path_makedirs(path, mode, exist_ok)`), and `stat`/`lstat`/`fstat`/`rename`/`replace` now lower through dedicated Rust intrinsics (`molt_os_stat`, `molt_os_lstat`, `molt_os_fstat`, `molt_os_rename`, `molt_os_replace`) with thin `os.stat_result` shaping in the shim. `chdir` is intrinsic-backed. `path.commonpath`/`path.commonprefix`/`path.getatime`/`path.getctime`/`path.getmtime`/`path.getsize`/`path.samefile`/`path.realpath` are intrinsic-backed. `urandom` remains capability-gated by `rand`. Remaining: richer stat metadata fidelity and long-tail POSIX parity. |
| sys | Capability-gated | Partial | P2 | SL3 | stdlib/runtime | Intrinsic-backed shim with significantly expanded coverage (~20 new intrinsics in 2026-02-25 sprint): argv/version/version_info/path/modules (synced from runtime module cache) + recursion limits; stdio and bootstrap state are intrinsic-only via `molt_sys_bootstrap_payload` (`sys.path` + module roots + `PYTHONPATH`/`VIRTUAL_ENV` site-packages/`PWD`/stdlib-root/include-cwd policy) with no host-Python fallback path; `sys.version` + `sys.version_info` are stamped unconditionally by the compiled target-version bootstrap before user code/importlib gates run, and runtime stdlib version decisions read `RuntimeState.sys_version_info` instead of ambient env (`MOLT_SYS_VERSION_INFO`/`MOLT_SYS_VERSION` remain explicit differential lane overrides). Metadata attrs `hexversion`, `api_version`, `abiflags`, and `implementation` lower through dedicated runtime intrinsics (`molt_sys_hexversion`, `molt_sys_api_version`, `molt_sys_abiflags`, `molt_sys_implementation_payload`), and `sys.flags` is now intrinsic-backed through `molt_sys_flags_payload` with shim-side payload validation and CPython-ordered sequence fields. Platform metadata intrinsics now cover `maxsize`, `maxunicode`, `byteorder`, `prefix`, `exec_prefix`, `base_prefix`, `base_exec_prefix`, `platlibdir`, `stdlib_module_names`, `builtin_module_names`, and `orig_argv`. Info struct intrinsics now cover `float_info`, `int_info`, `hash_info`, and `thread_info`. Interpreter hooks `intern`, `getsizeof`, `displayhook`, `excepthook`, and `copyright` are intrinsic-backed. `sys.exc_info()` reads the active exception context; compiled argv is runtime-sourced; host info remains capability-gated via `env.read` (`sys._getframe` still partial). |
...
| signal | Capability-gated | Partial | P3 | SL3 | stdlib/runtime | Intrinsic-backed signal handling gated by `process.signal`. Core signal constants (`SIG_DFL`, `SIG_IGN`, `SIGABRT`, `SIGFPE`, `SIGILL`, `SIGINT`, `SIGSEGV`, `SIGTERM`, `SIGHUP`, `SIGQUIT`, `SIGALRM`, `SIGPIPE`, `SIGUSR1`, `SIGUSR2`, `SIGCHLD`, `SIGCONT`, `SIGSTOP`, `SIGTSTP`, `SIGTTIN`, `SIGTTOU`) plus 12 new constant intrinsics (`SIGBUS` through `SIGSYS`) and core functions (`signal`, `getsignal`, `raise_signal`, `alarm`, `pause`, `set_wakeup_fd`, `valid_signals`, `Signals`, `Handlers`, `Sigmasks`). POSIX function intrinsics added: `strsignal`, `pthread_sigmask`, `pthread_kill`, `sigpending`, `sigwait`. Remaining: full platform-specific signal parity and wasm capability gating. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): close remaining signal platform-specific parity.) |
...
| subprocess | Capability-gated | Partial | P3 | SL3 | stdlib/runtime | Intrinsic-backed process spawn via `molt_process_spawn_ex` Rust intrinsic with expanded `Popen` surface: `start_new_session`, `process_group` params, `pid` property, `send_signal` method, `wait`/`communicate`/`poll`/`returncode`. Convenience functions `check_call`, `getstatusoutput`, `getoutput` are intrinsic-backed. `proc.spawn` capability gating enforced. Remaining: full Popen stdin/stdout/stderr pipe lifecycle, `check_output`, `DEVNULL`/`PIPE`/`STDOUT` constant fidelity, and broader platform parity. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): close remaining subprocess Popen/pipe lifecycle parity.) |
...
| asyncio | Capability-gated | Partial | P2 | SL3 | stdlib/runtime | Shim exposes `run`/`sleep`, `EventLoop`, `Task`/`Future`, `create_task`/`ensure_future`/`current_task`, `wait`/`wait_for`/`gather`, stream/socket adapters, pipe transports (`connect_read_pipe`/`connect_write_pipe` via 11 new pipe transport intrinsics), and Transport/Protocol base classes; 42 new Rust intrinsics for Future/Event/Lock/Semaphore/Queue state machines; all 97 bare `except` blocks eliminated; WASM capability gating for 6 I/O operations; TaskGroup/Runner cancellation fanout is intrinsic-backed (`molt_asyncio_cancel_pending` + gather path), synchronization waiter hot paths lower through Rust intrinsics (`molt_asyncio_waiters_notify`, `molt_asyncio_waiters_notify_exception`, `molt_asyncio_waiters_remove`, `molt_asyncio_barrier_release`), task/future callback transfer + event waiter teardown are intrinsic-backed (`molt_asyncio_future_transfer`, `molt_asyncio_event_waiters_cleanup`), task registry + event-waiter token maps are runtime-owned, running/event-loop state is intrinsic-backed, TaskGroup done-callback fanout + ready-queue dispatch are intrinsic-backed, coroutine predicates route through inspect intrinsics, and TLS execution is runtime-owned for client and server lanes. 3.13 version-gated APIs (`as_completed` async iter, `Queue.shutdown`) and 3.14 version-gated APIs (`get_event_loop` `RuntimeError`, child watcher removal, policy deprecation) added. Advanced loop APIs and full submodule parity remain pending. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): asyncio pending parity) |
| _asyncio | Stdlib | Partial | P3 | SL3 | stdlib/runtime | Intrinsic-backed running/event-loop hooks (`_get_running_loop`/`_set_running_loop`/`get_running_loop`, intrinsic-backed `get_event_loop` path via runtime loop/policy state) plus C-accelerated task surface functions expanded in 2026-02-25 sprint: `current_task`, `_enter_task`, `_leave_task`, `_register_task`, `_unregister_task` backed by 4 new Rust intrinsics with runtime-owned task-state management. Remaining: broader CPython C-accelerated helper parity (`_task_step_impl`, `_task_result`). (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): implement remaining `_asyncio` C-accelerated helper parity.) |
...
| _thread | Capability-gated | Partial | P3 | SL3 | stdlib/runtime | Fully rewritten on existing thread intrinsics (2026-02-25 sprint): `allocate_lock`, `LockType`, `start_new_thread`, `exit`, `get_ident`, `get_native_id`, `_count`, `stack_size`, `interrupt_main`, `TIMEOUT_MAX`, and `error` are all intrinsic-backed with no host-Python fallback. WASM target raises `NotImplementedError` for thread operations by design. Remaining: broader CPython `_thread` surface parity (daemon threads, excepthook). (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): close remaining _thread surface parity.) |
...
| concurrent.futures | Capability-gated | Partial | P3 | SL3 | stdlib/runtime | Intrinsic-backed executor interfaces: `ThreadPoolExecutor`, `ProcessPoolExecutor`, `Future`, `wait`, `as_completed`, `Executor` base class, and `BrokenExecutor`/`CancelledError`/`TimeoutError` exceptions are intrinsic-backed. Verified intrinsic-complete (2026-02-25). Remaining: broader CPython edge-case parity and full lifecycle fidelity. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): close remaining concurrent.futures lifecycle/edge-case parity.) |
...
| doctest | Stdlib | Partial | P3 | SL3 | stdlib | Stub that rejects eval/exec/compile usage for now. (Policy-deferred note: doctest parity that depends on dynamic execution (`eval`/`exec`/`compile`) remains intentionally unsupported for now; revisit only behind explicit capability gating after utility analysis, performance evidence, and explicit user approval.) |
...
| runpy | Capability-gated | Partial | P3 | SL3 | stdlib | `fs.read` gating with runtime-lowered path resolution (`molt_runpy_resolve_path`) and intrinsic-backed `run_path` execution (`molt_runpy_run_path`). Differential coverage now asserts transient `sys.argv[0]` swap+restore semantics for `runpy.run_path(...)` and `runpy.run_module(... alter_sys=True ...)`, plus `sys.modules` restoration in the `alter_sys` alias lane. Full runpy package/code-object execution parity remains pending. |
...
| tkinter | Capability-gated | Partial | P3 | SL3 | stdlib | Expanded intrinsic-backed `_tkinter` + `tkinter` surface is landed with 10 Rust intrinsics wired (event parsing, Tcl list/dict conversion, hex color validation, option normalization); all strict mode violations resolved. Runtime-backed core command dispatch, broad headless `tkinter.ttk` lowering (Treeview, `ttk::style`, Notebook/Panedwindow/container/widget subcommands, and `ttk::notebook::enableTraversal`), and dialog-command intrinsic lanes (`commondialog`, `messagebox`, `filedialog`, `simpledialog`) with deterministic headless behavior when native Tk is unavailable. Runtime also includes an opt-in native Tcl/Tk backend path (`molt_tk_native`). 3.13 version-gated APIs (`tk_busy_*`, `PhotoImage.copy_replace`) and 3.14 version-gated APIs (`trace_variable` deprecation) added. `tkinter.tix` imports only for 3.12; raises `ModuleNotFoundError` for 3.13+ (CPython-aligned removal). 100% submodule coverage achieved. Focused differential coverage (`tests/differential/stdlib/tkinter_phase0_core_semantics.py`) validates import/error-shape/capability-gate contracts plus runtime headless core/ttk semantics probes without requiring a GUI backend. Remaining gaps: broader tkinter submodule behavioral parity and consistently reliable native Tk toolchain availability across hosts. |
...
- Update [docs/spec/areas/compat/surfaces/stdlib/stdlib_surface_matrix.md](docs/spec/areas/compat/surfaces/stdlib/stdlib_surface_matrix.md) and [docs/ROADMAP.md](docs/ROADMAP.md) as each module lands.
...
- TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): capability-gated I/O (`io`, `os`, `sys`, `pathlib`); `os` and `sys` significantly advanced in 2026-02-25 sprint.
- TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): network/process gating (`socket`, `ssl`, `subprocess`, `asyncio`); `subprocess` advanced in 2026-02-25 sprint.
