...
| tkinter | Capability-gated | Partial | P3 | SL3 | stdlib | Expanded intrinsic-backed `_tkinter` + `tkinter` surface is landed: runtime-backed core command dispatch, broad headless `tkinter.ttk` lowering (Treeview, `ttk::style`, Notebook/Panedwindow/container/widget subcommands, and `ttk::notebook::enableTraversal`), and dialog-command intrinsic lanes (`commondialog`, `messagebox`, `filedialog`, `simpledialog`) with deterministic headless behavior when native Tk is unavailable. Runtime also includes an opt-in native Tcl/Tk backend path (`molt_tk_native`) for interpreter-backed command dispatch, callback/event pumping, and Tk lifecycle operations. Version-gated absence is explicit: `tkinter.tix` imports only for 3.12 and raises `ModuleNotFoundError` for 3.13+ (CPython-aligned removal). Focused differential coverage (`tests/differential/stdlib/tkinter_phase0_core_semantics.py`) validates import/error-shape/capability-gate contracts plus runtime headless core/ttk semantics probes (`tkinter:runtime_core_semantics`, `tkinter.ttk:runtime_semantics`) without requiring a GUI backend. Remaining gaps: broader tkinter submodule behavioral parity and consistently reliable native Tk toolchain availability across hosts. |
