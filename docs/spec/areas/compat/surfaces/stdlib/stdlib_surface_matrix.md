...
| tkinter | Capability-gated | Partial | P3 | SL3 | stdlib | Phase-0 intrinsic-backed `_tkinter` + `tkinter`/`tkinter.ttk` wrappers are landed, and runtime includes an opt-in native Tcl/Tk backend path (`molt_tk_native`) for interpreter-backed command dispatch, callback/event pumping, and Tk lifecycle operations. Headless Rust lowering now covers major `ttk` command families (Treeview, `ttk::style`, Notebook/Panedwindow/container/widget subcommands, and `ttk::notebook::enableTraversal`). Version-gated absence is explicit: `tkinter.tix` imports only for 3.12 and raises `ModuleNotFoundError` for 3.13+ (CPython-aligned removal). Focused differential coverage (`tests/differential/stdlib/tkinter_phase0_core_semantics.py`) validates import/error-shape/capability-gate contracts and runtime headless `ttk` semantics (`tkinter.ttk:runtime_semantics`) without requiring a GUI backend. Broader tkinter submodule parity and universally reliable native Tk toolchain enablement remain pending. |
