...
| `PyType_FromSpecWithBases` | Heap-type creation from `PyType_Spec` + bases | Partial | Header shim supports `Py_tp_doc`, `Py_tp_methods`, `Py_tp_base`, `Py_tp_bases`, `Py_tp_new`, selected call/dunder slots (`Py_tp_call`, `Py_tp_iter`, `Py_tp_iternext`, `Py_tp_repr`, `Py_tp_str`), and selected numeric/sequence slots (`Py_nb_add`, `Py_nb_subtract`, `Py_nb_multiply`, `Py_sq_concat`). Descriptor slots (`Py_tp_getset`/`Py_tp_members`, `PyDescr_NewGetSet`, `PyDescr_NewMember`) fail closed in the source-compat header and require the `cpython-abi` tier, where the real descriptor protocol authority lives. Unsupported slot IDs still raise immediately. |
