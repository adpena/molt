...
| textwrap | Stdlib | Partial | P3 | SL3 | stdlib/runtime | Intrinsic-backed `TextWrapper.wrap`/`TextWrapper.fill` and `indent` via `molt_textwrap_wrap`/`molt_textwrap_fill`/`molt_textwrap_indent`; differential coverage now exercises width-driven wrap/fill semantics, default `indent` behavior, and option-contract probes for `drop_whitespace`/`expand_tabs`/`break_on_hyphens` (currently rejected by the intrinsic-backed wrapper constructor). Full CPython `TextWrapper` option surface (including indent/predicate hooks) remains pending. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P3, status:partial): close remaining textwrap option/edge-case parity.) |
