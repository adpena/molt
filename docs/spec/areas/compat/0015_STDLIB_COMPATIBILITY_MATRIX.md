...
| bisect | Stdlib | Complete | P1 | SL1 | stdlib/runtime | Intrinsic-backed `bisect_left`/`bisect_right` + `insort_left`/`insort_right` with `key` support; aliases `bisect`/`insort` and `_bisect` parity. |
...
| colorsys | Stdlib | Supported | P3 | SL3 | stdlib/runtime | Intrinsic-backed RGB/YIQ/HLS/HSV conversions with full API surface. |
