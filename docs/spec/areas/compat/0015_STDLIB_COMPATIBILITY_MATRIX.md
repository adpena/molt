...
| statistics | Stdlib | Partial | P2 | SL2 | stdlib/runtime | Intrinsic-backed `mean` + `stdev` (`StatisticsError` mapping in shim); broader module coverage pending. (TODO(stdlib-compat, owner:stdlib, milestone:SL2, priority:P2, status:partial): expand statistics coverage beyond mean/stdev.) |
...
| queue | Stdlib | Partial | P3 | SL3 | stdlib/runtime | Intrinsic-backed `Queue`/`SimpleQueue` core operations (`put`/`get`, timeout handling, `task_done`/`join`, `Empty`/`Full`) landed; remaining parity includes `LifoQueue`, `PriorityQueue`, and full edge-case/API coverage. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): complete queue parity beyond core FIFO support) |
