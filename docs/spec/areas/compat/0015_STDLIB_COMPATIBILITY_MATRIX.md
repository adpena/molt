...
| bisect | Stdlib | Full | P1 | SL1 | stdlib | Intrinsic-backed `bisect`/`_bisect` (`bisect_left`/`bisect_right`, `insort_left`/`insort_right`) with `key` support; aliases `bisect`/`insort`. |
