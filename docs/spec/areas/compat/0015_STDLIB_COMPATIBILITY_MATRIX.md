...
| contextlib | Stdlib | Partial | P2 | SL2 | stdlib/runtime | `contextmanager`/`ContextDecorator` + `ExitStack`/`AsyncExitStack`, `asynccontextmanager`, `aclosing`, `suppress`, `redirect_stdout`/`redirect_stderr`, `nullcontext`, and `closing` are runtime-intrinsic-backed (including `ExitStack.callback` + `ExitStack.enter_context` and async enter/exit plumbing). `AbstractContextManager` and full edge-case parity remain pending. (TODO(stdlib-compat, owner:stdlib, milestone:SL2, priority:P2, status:partial): contextlib pending parity) |
...
| importlib.util | Stdlib | Partial | P2 | SL3 | stdlib/runtime | `find_spec`/`module_from_spec`/`spec_from_file_location` for filesystem sources; sys bootstrap/path resolution and spec/cache shaping lower through runtime payload intrinsics (`molt_importlib_bootstrap_payload`, `molt_importlib_find_spec_payload`, `molt_importlib_cache_from_source`, `molt_importlib_spec_from_file_location_payload`). meta_path/zip/extension loaders pending. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): importlib.util parity for non-filesystem loaders) |
| importlib.machinery | Stdlib | Partial | P2 | SL3 | stdlib | `ModuleSpec` + filesystem `SourceFileLoader`; package/module shaping lowers via `molt_importlib_source_loader_payload`, file reads lower via `molt_importlib_read_file`, and restricted module source execution lowers via `molt_importlib_exec_restricted_source` (no Python-side fallback parser). Extension/zip/namespace loaders pending. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): importlib.machinery loader/finder parity) |
| importlib.resources | Capability-gated | Partial | P2 | SL3 | stdlib/runtime | Filesystem-backed `files`/`open_text`/`read_binary`/traversable APIs; package root + namespace discovery lower via `molt_importlib_resources_package_payload`, traversable stat/listdir payloads lower via `molt_importlib_resources_path_payload`, and direct read/open helpers use intrinsic file IO (`molt_importlib_read_file`). zip/loader resource readers pending. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): importlib.resources loader/zip parity) |
| importlib.metadata | Capability-gated | Partial | P2 | SL3 | stdlib/runtime | `distribution`/`version`/`entry_points` via dist-info scan; bootstrap + scan lower via `molt_importlib_bootstrap_payload` and `molt_importlib_metadata_dist_paths`, aggregated entry points lower via `molt_importlib_metadata_entry_points_payload`, file reads lower via `molt_importlib_read_file`, and metadata header + dependency/extra/entry-point payload parsing lowers via `molt_importlib_metadata_payload` (requires `fs.read`). Remaining gaps are advanced metadata/version semantics. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): importlib.metadata parity) |
...
| time | Capability-gated | Partial | P3 | SL3 | stdlib/runtime | `monotonic`/`perf_counter` + `process_time` + `sleep` + `get_clock_info`; `time`/`time_ns` gated by `time.wall` (or legacy `time`); `localtime`/`gmtime`/`strftime` + `struct_time` + `asctime`/`ctime` + `timezone`/`daylight`/`altzone`/`tzname` + `mktime` wired to runtime intrinsics. WASM localtime/timezone currently uses UTC (TODO(wasm-parity, owner:runtime, milestone:RT2, priority:P1, status:partial): wire local timezone + locale on wasm hosts). (TODO(stdlib-compat, owner:stdlib, milestone:SL2, priority:P2, status:partial): expand time module surface (`timegm`) + deterministic clock policy) |
...
| traceback | Stdlib | Partial | P3 | SL3 | stdlib | `format_exc`/`format_tb`/`format_list`/`format_stack`/`print_exception`/`print_list`/`print_stack`, `extract_tb`/`extract_stack`, `StackSummary.extract`/`StackSummary.from_list`, and `TracebackException.from_exception`; exception chain formatting (`__cause__`/`__context__`/`__suppress_context__`) is runtime-lowered via `molt_traceback_format_exception`, extract/list frame shaping routes through `molt_traceback_payload`, and `TracebackException.from_exception` component extraction is runtime-lowered via `molt_traceback_exception_components`. Best-effort frames (CPython traceback objects or Molt payload tuples); full parity pending. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P3, status:partial): traceback pending parity) |
