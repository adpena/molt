...
| --- | --- | --- | --- | --- | --- | --- |
| builtins | Core | Partial | P0 | SL1 | runtime/frontend | Importable module binds supported builtins (including function objects for allowlisted builtins); missing names raise `AttributeError`. Current `str` method tranche is intrinsic-backed for `lower`/`upper`/`capitalize`/`swapcase` plus predicates `isalpha`/`isalnum`/`isdecimal`/`isdigit`/`isnumeric`/`islower`/`isupper`/`isspace`/`istitle`/`isprintable`/`isascii` (covered by `tests/differential/basic/str_case_methods_extended.py` and `tests/differential/basic/str_predicates_surface.py`). |
...
| operator | Core-adjacent | Full | P1 | SL1 | stdlib/runtime | Full CPython 3.12+ operator/_operator surface (math/bitwise/comparison, in-place ops, concat/getitem/setitem/delitem, length_hint/countOf, and callable getter types). |
...
| statistics | Stdlib | Partial | P2 | SL2 | stdlib/runtime | Intrinsic-backed function surface (`mean`, `fmean`, `stdev`, `variance`, `pvariance`, `pstdev`, `median`, `median_low`, `median_high`, `median_grouped`, `mode`, `multimode`, `quantiles`, `harmonic_mean`, `geometric_mean`, `covariance`, `correlation`, `linear_regression`) with shim-level `StatisticsError` mapping; `NormalDist.samples` is version-gated for Python 3.12+ parity (`gauss` route for 3.12/3.13 via `molt_statistics_normal_dist_samples`, inverse-CDF route for 3.14+ via `molt_statistics_normal_dist_inv_cdf`). Remaining parity is broader 3.12+ API/PEP coverage and edge-case message alignment. (TODO(stdlib-compat, owner:stdlib, milestone:SL2, priority:P2, status:partial): complete full statistics 3.12+ API/PEP parity beyond function surface lowering.) |
...
| abc | Stdlib | Partial | P3 | SL3 | stdlib | `ABCMeta`/`ABC` + `abstractmethod` with intrinsic-backed `_abc` registry/caches (`register`, `__instancecheck__`, `__subclasscheck__`, cache token + reset helpers). Focused parity tracking now includes invalid `__subclasshook__` return-contract differential coverage (non-`bool`/non-`NotImplemented` should raise CPython-shaped `AssertionError`) via `tests/differential/stdlib/abc_subclasshook_return_contract.py`. Remaining: cache invalidation + broader subclasshook corner cases. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P3, status:partial): close remaining abc edge-case parity around subclasshook/cache invalidation.) |
| _abc | Stdlib | Partial | P3 | SL3 | stdlib | Intrinsic-backed `_abc` exports cache token/init/register/instancecheck/subclasscheck/get_dump/reset_registry/reset_caches via direct intrinsic bindings; no host-Python fallback path. Remaining: CPython edge-case parity for cache/version corner cases. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P3, status:partial): close `_abc` edge-case cache/version parity.) |
...
| weakref | Stdlib | Partial | P3 | SL3 | stdlib | Runtime-backed weakrefs + proxies + WeakKey/ValueDictionary + WeakSet + WeakMethod + finalize + getweakrefcount/refs (runtime registry-backed, including callback refs and no-callback dedupe semantics); `ReferenceType.__callback__` is intrinsic-backed (`molt_weakref_callback`) with CPython-style alive/dead visibility. Shutdown finalize behavior is intrinsic-backed via the `atexit` callback stack runner (LIFO-order preserving against user callbacks), honors `finalize.atexit`, matches callback-list rewrite corners (`atexit._clear()`/`atexit.unregister()`), and now has focused differential coverage for `finalize.detach()`/`peek()`/`alive`/idempotent-call state transitions. |
...
| json | Stdlib | Partial | P1 | SL2 | stdlib | Shim supports `loads`/`dumps`/`load`/`dump` with parse hooks, indent, separators, and `allow_nan`, plus `JSONEncoder`/`JSONDecoder` and `JSONDecodeError` details. `_json`, `json.decoder`, `json.encoder`, `json.scanner`, `json.tool`, and version-gated `json.__main__` now match CPython API-gap digests (including 3.12/3.13/3.14 gating for `json.tool` and `json.__main__`) via intrinsic-first wrappers. Runtime fast-path + deeper semantic parity remain pending. |
| imghdr | Stdlib | Partial | P2 | SL3 | stdlib/runtime | 3.12-only module lowered with intrinsic-backed header detection (`molt_imghdr_detect`) and CPython 3.12 surface (`what`, `test`, `testall`, `test_*`, `tests`, `PathLike`, `warnings`); importlib now gates `imghdr` absent on 3.13/3.14 (PEP 594 removal). Differential evidence: `imghdr_basic.py`, `imghdr_version_gate.py`, and API-gap probe `cpython312plus_api_gap_top_level_imghdr_f49c9279.py` under 3.12/3.13/3.14. |
...
| os | Capability-gated | Partial | P2 | SL3 | stdlib/runtime | Intrinsic-backed shim: env access gated via `env.read`/`env.write` with runtime-backed `os.environ` state (`molt_env_snapshot`/`molt_env_set`/`molt_env_unset`) and dedicated `os.putenv`/`os.unsetenv` intrinsics (`molt_env_putenv`/`molt_env_unsetenv`) matching CPython-visible mapping separation; path helpers (`basename`/`split`/`relpath`/`expandvars`) + fs helpers (`exists`/`isdir`/`isfile`/`unlink`/`remove`) + fd ops (`close`/`pipe`/`dup`/`read`/`write`/`get_inheritable`/`set_inheritable`). `mkdir`/`makedirs` mode handling lowers through Rust intrinsics (`molt_path_mkdir(path, mode)` / `molt_path_makedirs(path, mode, exist_ok)`), and `stat`/`lstat`/`fstat`/`rename`/`replace` now lower through dedicated Rust intrinsics (`molt_os_stat`, `molt_os_lstat`, `molt_os_fstat`, `molt_os_rename`, `molt_os_replace`) with thin `os.stat_result` shaping in the shim. `urandom` remains capability-gated by `rand`. Remaining: full process/signal long-tail parity and richer stat metadata fidelity. |
| sys | Capability-gated | Partial | P2 | SL3 | stdlib/runtime | Minimal shim: argv/version/version_info/path/modules (synced from runtime module cache) + recursion limits; stdio and bootstrap state are intrinsic-only via `molt_sys_bootstrap_payload` (`sys.path` + module roots + `PYTHONPATH`/`VIRTUAL_ENV` site-packages/`PWD`/stdlib-root/include-cwd policy) with no host-Python fallback path; `sys.version` + `sys.version_info` are stamped by the toolchain intrinsic and honor `MOLT_SYS_VERSION_INFO`/`MOLT_SYS_VERSION` lane overrides end-to-end for differential version gating. Metadata attrs `hexversion`, `api_version`, `abiflags`, and `implementation` lower through dedicated runtime intrinsics (`molt_sys_hexversion`, `molt_sys_api_version`, `molt_sys_abiflags`, `molt_sys_implementation_payload`), and `sys.flags` is now intrinsic-backed through `molt_sys_flags_payload` with shim-side payload validation and CPython-ordered sequence fields. `sys.exc_info()` reads the active exception context; compiled argv is runtime-sourced; host info remains capability-gated via `env.read` (`sys._getframe` still partial). |
...
| stat | Stdlib | Full | P3 | SL3 | stdlib/runtime | Intrinsic-backed `stat` constants/mode helpers cover CPython 3.12+ file-type constants (`S_IFSOCK`/`S_IFLNK`/`S_IFREG`/`S_IFBLK`/`S_IFDIR`/`S_IFCHR`/`S_IFIFO`/`S_IFDOOR`/`S_IFPORT`/`S_IFWHT`), permission/set-id aliases (`S_ENFMT`, `S_IREAD`, `S_IWRITE`, `S_IEXEC`), `ST_*` indexes, BSD/macOS UF/SF flags, Windows `FILE_ATTRIBUTE_*`, and helpers (`S_IFMT`, `S_IMODE`, `S_ISDIR`, `S_ISREG`, `S_ISCHR`, `S_ISBLK`, `S_ISFIFO`, `S_ISLNK`, `S_ISSOCK`, `S_ISDOOR`, `S_ISPORT`, `S_ISWHT`, `filemode`) via `molt_stat_constants` + `molt_stat_*` intrinsics. Version policy is explicit: 3.12 baseline exports only 3.12 constants; 3.13/3.14-only constants (`UF_SETTABLE`/`UF_TRACKED`/`UF_DATAVAULT`/`SF_SETTABLE`/`SF_RESTRICTED`/`SF_FIRMLINK`/`SF_DATALESS`/`SF_SUPPORTED`/`SF_SYNTHETIC`) are exported only for `sys.version_info >= (3, 13)` with intrinsic payload slots zeroed for 3.12. Cross-version differential evidence is green under 3.12/3.13/3.14 for the stat slice (`stat_basic.py`, `stat_helpers_intrinsic.py`, `stat_filemode_constants.py`, `stat_api_surface_versioned.py`) plus sys/version propagation regression (`sys_stat_version_gate.py`). |
...
| selectors | Capability-gated | Partial | P3 | SL3 | stdlib/runtime | CPython-shaped `SelectSelector`/`PollSelector`/`EpollSelector`/`KqueueSelector`/`DevpollSelector` now route through intrinsic-backed `select` object backends (`molt_select_selector_*`) instead of Python async wait fan-out. Focused differential coverage now asserts register/modify/unregister semantics, timeout normalization (`timeout <= 0` non-blocking), event-mask filtering to registered interests, and `DefaultSelector` readiness on a supported backend. Remaining: full OS-specific flag/error fidelity, fd inheritance corners, and broader wasm/browser host parity. |
...
| http.cookies | Stdlib | Partial | P3 | SL3 | stdlib/runtime | Intrinsic-backed `SimpleCookie`/`Morsel` subset via `molt_http_cookies_parse` + `molt_http_cookies_render_morsel` covering `__setitem__`/`__getitem__`, `load("a=1; b=2")`, `output()`, items iteration, and morsel attrs (`path`, `secure`, `httponly`, `max-age`, `expires`). Remaining: full CPython quoting/escaping semantics (`value_encode`/`value_decode`), reserved-attr breadth beyond subset, and full parser edge-case parity. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): complete http.cookies quoting/attribute/parser parity beyond intrinsic-backed subset.) |
...
| urllib.response | Stdlib | Partial | P3 | SL3 | stdlib/runtime | CPython-style `addbase`/`addclosehook`/`addinfo`/`addinfourl` classes are now implemented, with Molt handle-backed response bridging through `molt_urllib_request_response_*` intrinsics (`read`/`read1`/`readinto`/`readinto1`/`readline`/`readlines`, `readable`/`writable`/`seekable`/`tell`/`seek`, `close`/`drop`, and metadata/header accessors). Closed-read semantics are CPython-shaped by response kind (`data:` closed operations raise `ValueError`; HTTP closed reads return EOF/zero and remain non-seekable). Python shim remains responsible only for wrapper object shaping and method dispatch. Remaining: full parity for edge-case file-wrapper delegation and legacy urllib integration corners. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P3, status:partial): close remaining urllib.response file-wrapper and integration edge parity.) |
...
| pickle | Stdlib | Partial | P2 | SL2 | stdlib/runtime | Intrinsic-backed core dumps/loads for protocols `0..5` (`molt_pickle_dumps_core`, `molt_pickle_loads_core`) with thin shim wiring in `pickle.py` (`dump`/`dumps`/`load`/`loads`, `Pickler`, `Unpickler`), including protocol-`2+` memo opcodes, LONG/EXT decoding, reducer/build paths, and persistent hooks. Remaining parity gaps: out-of-band buffers, full reducer tuple/copyreg edge semantics, complete class-surface error parity, and broader CPython 3.12+ type/PEP semantics. (TODO(stdlib-compat, owner:stdlib, milestone:SL2, priority:P2, status:partial): close remaining pickle CPython 3.12+ parity gaps before intrinsic-backed promotion.) |
...
| atexit | Stdlib | Partial | P3 | SL3 | stdlib/runtime | Core exit-callback registry wiring and shutdown drain execution are intrinsic-backed (no import-only stub path for compiled runs), including LIFO ordering, callback continuation after failures, version-gated stderr reporting parity (`Exception ignored in atexit callback: ...` on 3.12 vs `... callback ...:` on 3.13+), and custom `sys.unraisablehook` parity via `UnraisableHookArgs` payload shape (`err_msg`/`object` version semantics aligned to CPython 3.12+). Weakref finalizer callbacks execute through the same callback stack runner, preserving CPython order against user callbacks. |
...
| importlib.resources | Capability-gated | Partial | P2 | SL3 | stdlib/runtime | `files`/`open_text`/`read_binary`/traversable APIs with package root + namespace discovery lowered via `molt_importlib_resources_package_payload`, traversable stat/listdir payloads lowered via `molt_importlib_resources_path_payload` (filesystem + zip/whl/egg namespace/resource roots), and consolidated files()-path arbitration lowered via `molt_importlib_resources_files_payload`; direct read/open helpers use intrinsic file IO (`molt_importlib_read_file`) for filesystem + archive entries. Loader reader bootstrap is intrinsic-backed via `molt_importlib_resources_module_name`/`molt_importlib_resources_loader_reader` (including fallback from `module.__spec__.loader` to `module.__loader__`), with custom loader reader contract support through dedicated runtime intrinsics (`molt_importlib_resources_reader_roots`, `molt_importlib_resources_reader_contents`, `molt_importlib_resources_reader_resource_path`, `molt_importlib_resources_reader_is_resource`, `molt_importlib_resources_reader_open_resource_bytes`, `molt_importlib_resources_reader_child_names`). `importlib.resources._functional` string-anchor path-name operations lower through dedicated runtime intrinsics (`molt_importlib_resources_*_from_package_parts`) with thin shim-level encoding/warning behavior. Helper modules (`importlib.resources.abc`, `importlib.resources._common`, `importlib.resources._adapters`, `importlib.resources.simple`) route joinpath/import/mode/leaf-name helpers through `molt_importlib_resources_joinpath`, `molt_importlib_import_required`, `molt_importlib_resources_open_mode_is_text`, and `molt_importlib_resources_package_leaf_name`. Zip archive-member paths are explicitly tagged in runtime payloads so `resource_path()` stays filesystem-only across direct + traversable + roots fallback lanes while `open_resource()` remains intrinsic-backed for archive reads. |
| importlib.metadata | Capability-gated | Partial | P2 | SL3 | stdlib/runtime | `distribution`/`version`/`entry_points` via dist-info scan; bootstrap + scan lower via `molt_importlib_bootstrap_payload` and `molt_importlib_metadata_dist_paths`, bulk cache hydration lowers through `molt_importlib_metadata_distributions_payload`, aggregated/filtered entry points lower via `molt_importlib_metadata_entry_points_payload`/`molt_importlib_metadata_entry_points_select_payload`, distribution name canonicalization lowers via `molt_importlib_metadata_normalize_name`, file reads lower via `molt_importlib_read_file`, and metadata header + dependency/extra/entry-point payload parsing lowers via `molt_importlib_metadata_payload` (requires `fs.read`). `files()` RECORD parsing and `packages_distributions()` top-level provider aggregation now lower through dedicated runtime payload intrinsics (`molt_importlib_metadata_record_payload`, `molt_importlib_metadata_packages_distributions_payload`). Helper submodules route hot helpers through runtime intrinsics (`molt_functools_lru_cache`, `molt_functools_update_wrapper`, `molt_itertools_filterfalse`, `molt_operator_truth`, `molt_operator_eq`, `molt_operator_lt`, `molt_importlib_import_required`). Remaining gaps are advanced metadata/version semantics. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): importlib.metadata parity) |
| queue | Stdlib | Partial | P3 | SL3 | stdlib/runtime | Intrinsic-backed `Queue`/`SimpleQueue` core operations plus intrinsic-backed `LifoQueue` and `PriorityQueue` constructors/ordering (`molt_queue_lifo_new`, `molt_queue_priority_new`) are landed. Differential coverage now includes queue-family probes for `Queue.shutdown`/`ShutDown` surface/behavior across interpreter versions, `Queue.put/get(block=False, timeout=...)` timeout-ignore semantics with state-driven `Full`/`Empty`, `SimpleQueue.get(block=False, timeout=...)` empty-path behavior, and `_queue` import/surface sanity (`Empty`, `SimpleQueue`). Remaining parity is edge-case/API alignment (task accounting corner cases, comparator/error-path fidelity, complete `_queue` lowering, and broader behavioral coverage). (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): complete queue edge-case/API parity) |
...
| textwrap | Stdlib | Partial | P3 | SL3 | stdlib/runtime | Intrinsic-backed `TextWrapper.wrap`/`TextWrapper.fill` and `indent` via `molt_textwrap_wrap_ex`/`molt_textwrap_fill_ex`/`molt_textwrap_indent_ex`, including option-surface wiring (`drop_whitespace`, `expand_tabs`, `break_on_hyphens`, `max_lines`, placeholder behavior) plus predicate-aware `indent`. Differential coverage asserts width/fill parity plus option-sensitive behavior paths. Remaining work is full CPython edge-case parity and module-surface completeness outside the currently lowered wrappers. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P3, status:partial): close remaining textwrap edge-case/module-surface parity.) |
...
| cgi | Stdlib | Partial | P3 | SL3 | stdlib/runtime | 3.12 import-only allowlist stub; importlib gates `cgi` absent for Python 3.13+ as part of the PEP 594 removed-module set (`aifc`, `audioop`, `cgi`, `cgitb`, `chunk`, `crypt`, `imghdr`, `mailcap`, `msilib`, `nis`, `nntplib`, `ossaudiodev`, `pipes`, `sndhdr`, `spwd`, `sunau`, `telnetlib`, `uu`, `xdrlib`). Form parsing parity remains pending on the 3.12 path. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P3, status:partial): cgi 3.12-path parity) |
...
| glob | Capability-gated | Partial | P3 | SL3 | stdlib/runtime | Intrinsic-backed `glob`/`iglob`/`has_magic` via `molt_glob` + `molt_glob_has_magic` (`fs.read` gated for filesystem expansion) now covers `root_dir`, `recursive` + `**` recursion gating, `include_hidden`, trailing-separator directory matching, pathlike `root_dir` forwarding, bytes-pattern outputs (`list[bytes]`), CPython-style mixed bytes/str `TypeError` guards, and intrinsic `dir_fd` traversal for native hosts (Linux `/proc`/`/dev/fd`, Apple `fcntl(F_GETPATH)`, Windows handle-path resolution). Results preserve CPython-like filesystem traversal order (no forced sort). Wasm host split: server/WASI lanes support `dir_fd` when fd-path resolution exists; browser-like wasm hosts raise explicit `NotImplementedError` for relative `dir_fd` globbing. Remaining parity is focused on non-UTF8 bytes + host-specific traversal-order edge behavior. (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P3, status:partial): close remaining non-UTF8 bytes/traversal-order edge parity.) |
| fnmatch | Stdlib | Complete | P3 | SL3 | stdlib/runtime | Runtime-owned wildcard engine via `molt_fnmatch`/`molt_fnmatchcase` with intrinsic-backed `filter` and `translate`; supports `*`, `?`, bracket classes, class negation, range matching, bytes patterns, and normcase parity. |
...
| token | Stdlib | Supported | P3 | SL3 | stdlib/runtime | Intrinsic-backed module initialization (`molt_import_smoke_runtime_ready`) + Rust object payload bootstrap via `molt_token_payload_312` (runtime-backed 3.12 payload source); exports CPython 3.12 token constants, `tok_name`, `EXACT_TOKEN_TYPES`, helper predicates (`ISTERMINAL`/`ISNONTERMINAL`/`ISEOF`), and `__all__` parity. Focused differential coverage: `tests/differential/stdlib/token_core_api.py`. |
...
| smtplib | Capability-gated | Partial | P3 | SL3 | stdlib | Capability-gated SMTP shim with baseline `SMTP.sendmail` parity slice; wasm thread-dependent server paths fail fast deterministically (`NotImplementedError: threads are unavailable in wasm`) rather than emitting secondary artifacts. Remaining: full CPython client/session/error-state parity. |
...
| zipimport | Capability-gated | Partial | P3 | SL3 | stdlib | Minimal zipimporter for store/deflate+zip64 zipfile entries; `fs.read` gated. Runtime-heavy wasm regression now explicitly asserts CPython-compatible failure shape for root-importer `pkg.mod` lookup (`ZipImportError`). (TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P3, status:partial): implement zipimporter bytecode/cache parity + broader archive support.) |
...
- Runtime/IR: Current shim uses list-backed `deque` + dict-subclass parity for `Counter`/`defaultdict`; top-level `_collections.OrderedDict` now routes through runtime intrinsics.
