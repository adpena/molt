...
**Audience:** Molt compiler/runtime engineers, library authors, AI agents
...
## 11. Agent instructions (LLM-friendly)

When implementing OpenMP-inspired parallelism:
1. Create task: `tools/new-agent-task.sh parfor-v0`
2. Write a design note:
   - safety rules and what is forbidden
   - reduction semantics
3. Implement minimal primitives:
   - `ParFor` IR node + lowering to sequential loop first
4. Add thread pool runtime later behind a feature flag
5. Add tests:
   - correctness in sequential mode
   - correctness with parallel runtime
   - forced “fallback to sequential” test
6. Add benchmarks:
   - `range` fill
   - map kernel
   - reduction kernel
