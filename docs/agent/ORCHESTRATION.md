...
- **✅ UPDATE 2026-07-06 (late, orchestrator): R73.1 + R73.2 LANDED; WITNESS FRONTIER MOVED; NEW METABUG ARC; CRASH-RESILIENCE LESSON.**
  - **R73.1 + R73.2 LANDED (origin/main da862df81, historical).** R73.1 introduced shared content-addressed `runtime.wasm` caching and memory-bounded Cargo jobs. R73.2 established standalone Cython regeneration without `scipy._cyutility` imports. Its implicit provisioning policy is superseded: compilation now validates explicitly prepared tools and never installs Cython. Follow the current [extension pipeline](../spec/areas/tooling/0215_MOLT_EXTENSION_BUILD_PIPELINE.md), not the historical auto-provisioning procedure. The "scipy._cyutility unsolved structural gap" note below remains superseded by standalone regeneration.
