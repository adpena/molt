...
- DX wrappers prefer healthy external artifact roots before the internal disk when configured (`prefer_external_artifacts`, `MOLT_PREFER_EXTERNAL_ARTIFACTS=1`, or `tools/run_context_env.py --prefer-external-artifacts`). On this Windows workstation, the attached 2 TB SSD volume label `APDataStore` is the preferred build/artifact/toolchain tier and resolves to `D:\Molt` when healthy; the default managed toolchain root is `D:\Molt\target-root`. RunContext drops stale inherited `E:\Molt` derived roots and `E:\molt-target`/`E:\Molt\target-root` defaults in external-preferred DX sessions, while non-legacy explicit toolchain roots remain operator-owned. APDataStore is exFAT, so backend cache publication must route through the canonical lock+rename/copy fallback when hard links are unavailable; do not disable cache, reroute to `E:`, or hand-copy artifacts to work around hard-link errors. Windows DX env defaults `UV_LINK_MODE=copy` unless the operator explicitly overrides it, avoiding repeated uv hard-link fallback warnings on exFAT. Other Windows volumes are not default discovery targets; use one only through an explicit `MOLT_EXTERNAL_ARTIFACT_ROOTS` override. Preserve inherited `E:` roots only with `MOLT_PRESERVE_LEGACY_ARTIFACT_ROOTS=1`. POSIX defaults prefer `/Volumes/APDataStore/Molt` before `/Volumes/VertigoDataTier/Molt`; override with `MOLT_EXTERNAL_ARTIFACT_ROOTS` and tune health gating with `MOLT_EXTERNAL_MIN_FREE_GB`. Set `MOLT_REQUIRE_EXTERNAL_ARTIFACTS=1` only for maintainer/agent lanes that must fail closed instead of falling back.
- `MOLT_ALLOW_C_DRIVE_ARTIFACTS=1` is an explicit emergency override for
  ...
  or benchmark runs.
- Explicit non-legacy canonical env vars remain authoritative: if an operator sets `MOLT_EXT_ROOT`, `CARGO_TARGET_DIR`, `MOLT_CACHE`, `TMPDIR`, or related roots, wrappers must derive only missing defaults and must not overwrite the explicit value. Stale inherited legacy roots under `E:\Molt`, `E:\molt-target`, or `E:\Molt\target-root` are dropped in external-preferred DX sessions unless `MOLT_PRESERVE_LEGACY_ARTIFACT_ROOTS=1` is set deliberately.
...
- The executable classifier authority for this boundary is
  ...
  process-sampling change that touches Codex/Claude/app-server/node-repl
  identity must update that classifier and the host-control matrix tests before
  changing kill behavior. Do not add ad hoc name/path matching elsewhere.
...
- Outside Crash Recovery Structural Stability Mode, default to multi-hour autonomous
