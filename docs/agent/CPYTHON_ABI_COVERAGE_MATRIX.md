...
| `PyUnicode_AsUTF8` | FAITHFUL ✗→PARTIAL/DIVERGENT | high | cython/numpy (ubiquitous C-string access) | Returns interned UTF-8 pointer; BadArgument+NULL on non-str. DOWNGRADE: NO NUL-termination guarantee — CPython always appends '\0' (len+1 malloc); molt returns exactly len bytes (alloc_bytes_like reserves no terminator slot), so data[len] is a heap OVER-READ whenever 32+len hits a size-class boundary (len=8/16/24/... or >65504) — and virtually all callers treat the result as a C string (PyArg 's', printf '%s', fopen). Also lone-surrogate handling diverges. Borrowed/no-steal contract correct. |
| `PyUnicode_Check` | FAITHFUL ✗→PARTIAL/DIVERGENT | high | cython/numpy | Address-compares ob_type to PyUnicode_Type. DOWNGRADE: this is CheckExact semantics — CPython PyUnicode_Check = PyType_FastSubclass(...,UNICODE_SUBCLASS), TRUE for subclasses. numpy.str_ subclasses str: its ob_type != &PyUnicode_Type so molt returns 0 vs CPython 1 (mis-fires the header kwargs 'keywords must be strings' guard). Lone identity-only holdout vs sibling _Check subtype walks. |
...
| `PyUnicode_FromString` | FAITHFUL ✗→PARTIAL/DIVERGENT | high | cython/numpy/witness | Real; OOM fail-closed. DOWNGRADE: passes raw NUL-terminated bytes straight to alloc_str with NO UTF-8 validation; CPython = DecodeUTF8Stateful strict. Invalid UTF-8 char* (Latin-1/OS-encoded/binary) -> silent malformed str vs CPython UnicodeDecodeError+NULL. Also auto-interns identifier/1-char strings (identity divergence: FromString does not intern in CPython). |
| `PyUnicode_FromStringAndSize` | FAITHFUL ✗→PARTIAL/DIVERGENT | high | cython/numpy/witness | Real alloc via runtime hook; fails-closed NULL+MemoryError on OOM. DOWNGRADE: alloc_str copies bytes verbatim with ZERO UTF-8 validation; CPython decodes strict UTF-8. Invalid UTF-8 (b'\xff', lone continuation, overlong, byte surrogates) -> molt returns a MALFORMED non-NULL str (silent wrong answer) where CPython raises UnicodeDecodeError. size<0 -> NULL w/o SystemError; s==NULL,size==0 -> NULL (CPython returns empty singleton). |
...
| `PyBytes_CheckExact` | MISSING | med | cython/numpy (exact-type guard) | Not an exported symbol; must be an inline macro in include/ overlay. |
...
| `PyUnicode_CheckExact` | MISSING | med | cython/numpy (exact-type guard) | Not exported as a symbol; must be an inline macro in the include/ overlay. Verify header supplies it (molt has no str subclasses, so ==Check). |
...
| `PyBytes_Check` | FAITHFUL (claimed) | high | cython/numpy | Address compare to PyBytes_Type. |
...
| `PyUnicode_AsUTF8AndSize` | FAITHFUL (claimed) | high | cython/numpy | Single handle resolution; NULL sentinel always carries TypeError. (NOTE: shares the AsUTF8 NUL-termination caveat.) |
