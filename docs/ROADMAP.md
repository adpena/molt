...
- TODO(type-coverage, owner:runtime, milestone:TC3, priority:P2, status:partial): buffer protocol + memoryview layout. Runtime-owned typed strided storage/export now backs bytes/memoryview construction, `molt_buffer_export/acquire`, and the CPython ABI `PyObject_GetBuffer` projection; release-state/lifetime enforcement and NumPy ndarray construction remain.
