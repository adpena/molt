...
Ten-item parity plan details live in `docs/spec/0015_STDLIB_COMPATIBILITY_MATRIX.md` (section 3.1).
...
- TODO(language-roadmap, owner:frontend, milestone:LF1): `with` lowering for async/multi-context managers + try/finally lowering in IR.
- TODO(language-roadmap, owner:runtime, milestone:LF1): exception propagation + suppression semantics for context manager exit paths.
- TODO(language-roadmap, owner:stdlib, milestone:LF1): `contextlib.contextmanager` lowering and generator-based manager support.
...
- TODO(language-roadmap, owner:stdlib, milestone:LF3): expand `io`/`pathlib` to buffered + streaming wrappers with capability gates.
