...
- Implemented: import-path bootstrap wiring now reaches `runpy`/`importlib.machinery` + `importlib.util` filesystem resolution surfaces via runtime payload intrinsics (`molt_runpy_resolve_path`, `molt_importlib_read_file`, `molt_importlib_bootstrap_payload`, `molt_importlib_cache_from_source`, `molt_importlib_find_in_path`, `molt_importlib_spec_from_file_location`) with no Python `os.path` fallback in those code paths. Note: `molt_importlib_source_loader_payload` and `molt_importlib_spec_from_file_location_payload` were granular precursors now superseded by `molt_importlib_source_exec_payload` and `molt_importlib_spec_from_file_location` respectively.
...
- Implemented: `sys` metadata attrs `hexversion`, `api_version`, `abiflags`, and `implementation` now lower through dedicated runtime intrinsics (`molt_sys_hexversion`, `molt_sys_api_version`, `molt_sys_abiflags`, `molt_sys_implementation_payload`) with shim-side payload validation.
...
- Implemented: importlib zip source loader discovery/execution now lowers through Rust intrinsics (`molt_importlib_find_spec_payload` zip payload fields + `molt_importlib_zip_source_exec_payload`) and `importlib.machinery.ZipSourceLoader`; extension/sourceless loader execution now has capability-gated intrinsic execution lanes (`molt_importlib_exec_extension`, `molt_importlib_exec_sourceless`) with deterministic candidate probing (`*.molt.py` + `*.py`) before explicit `ImportError`, including continue-on-probe behavior when an earlier candidate hits restricted-shim parser limits.
