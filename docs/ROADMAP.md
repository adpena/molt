...
- TODO(tooling, owner:tooling, milestone:TL2, priority:P1, status:planned): add function-level object caching so unchanged functions can be relinked without recompiling whole scripts.
- TODO(tooling, owner:tooling, milestone:TL2, priority:P1, status:planned): add batch compile server mode for diff runs to amortize backend startup and reduce per-test compile overhead.
...
- TODO(type-coverage, owner:runtime, milestone:TC3, priority:P2, status:partial): buffer protocol + memoryview layout. Runtime-owned typed strided storage/export now backs bytes/memoryview construction, direct released-view fail-closed behavior, `molt_buffer_export/acquire`, the CPython ABI `PyObject_GetBuffer` projection, and bytes/bytearray method argument consumers through checked bytes-like access. Remaining bridge/builtin bytes-like consumers, exporter lock/count parity, and NumPy ndarray construction remain.
...
- Implemented: import-path bootstrap wiring reaches `runpy`, `importlib.machinery`, and `importlib.util` through the canonical compiled-module importer (`molt_runpy_run_module`, `molt_runpy_run_path`, `molt_module_import`) plus runtime path/spec payloads. Runpy no longer owns a second path intrinsic or source interpreter.
- Implemented: `importlib.util.find_spec` now routes all spec resolution through `molt_importlib_find_spec_orchestrate` (covering meta_path, path_hooks, namespace packages, cache-key computation, and `path_importer_cache` reuse). Granular precursors `molt_importlib_find_spec_payload`, `molt_importlib_find_spec_from_path_hooks`, `molt_importlib_existing_spec`, `molt_importlib_parent_search_paths`, `molt_importlib_finder_signature`, `molt_importlib_path_importer_cache_signature`, `molt_importlib_coerce_search_paths`, `molt_importlib_search_paths`, `molt_importlib_namespace_paths`, `molt_importlib_runtime_state_payload`, and `molt_importlib_runtime_state_view` are dead — all handled inside the orchestrator — and are pending removal from the manifest.
- Implemented: `importlib.resources` namespace package path discovery and `importlib.metadata` dist-info path scanning now lower through runtime intrinsics (`molt_importlib_metadata_dist_paths`), and `importlib.metadata` text reads now use intrinsic file IO (`molt_importlib_read_file`) instead of Python-side file-open fallbacks.
- Implemented: `importlib.resources` traversable stat/listdir shaping now lowers through runtime payload intrinsic (`molt_importlib_resources_path_payload`) with archive-member tagging (zip/whl/egg), and `open_text`/`open_binary`/`read_text`/`read_binary` paths use intrinsic-backed reads (`molt_importlib_read_file`) without Python file-open fallback.
- Implemented: `importlib.metadata` name/version/header and entry-point parsing now lower through runtime payload intrinsic (`molt_importlib_metadata_payload`), aggregated entry-point discovery/filtering now lowers through `molt_importlib_metadata_entry_points_filter_payload` (with group/name/value filter support), and distribution-name canonicalization is runtime-owned via `molt_importlib_metadata_normalize_name`. Note: `molt_importlib_metadata_entry_points_payload` (unfiltered) is dead — superseded by `molt_importlib_metadata_entry_points_filter_payload`.
- Implemented: `importlib.machinery.SourceFileLoader` and runpy execute compiler-emitted module bodies through one transactional module cache/table authority; the former restricted source mini-interpreter and its ABI surface are deleted.
- Implemented: `importlib.machinery` module/package detection for loader execution now lowers through `molt_importlib_module_spec_is_package` instead of Python-side spec-shaping probes.
- Implemented: `runpy.run_module` and `runpy.run_path` use transactional compiled-module execution with deterministic namespace metadata, package `__main__`, `init_globals`, `sys.modules`, and `sys.argv[0]` restoration.
...
- Implemented: `importlib.import_module` now dispatches through public runtime intrinsic `molt_importlib_import_module(name, package)` (no Python `__import__` fallback path in the stdlib shim); that wrapper performs public API validation/relative-name resolution before delegating to the shared Rust import transaction core. Frontend literal/static-call folding is refused after `importlib.import_module` is rebound through `importlib` or any module alias.
...
- TODO(stdlib-compat, owner:stdlib, milestone:SL1, priority:P1, status:partial): `collections` (`deque`, `Counter`, `defaultdict`) parity. Guarded native full-stdlib evidence now covers `tests/differential/stdlib/collections_defaultdict_missing.py` and `tests/differential/stdlib/collections_parity.py`; remaining work is broader CPython regrtest/differential coverage and target parity.
...
- TODO(stdlib-compat, owner:stdlib, milestone:SL2, priority:P1, status:partial): complete the remaining `gc` public API beyond the recovered runtime cycle collector (`get_objects`/`get_referrers`/`get_referents`, stats/callbacks/freeze/generation parity).
...
- TODO(stdlib-compat, owner:runtime, milestone:SL3, priority:P1, status:planned): implement the Windows native WinSock ABI authority as one target slice (constants, sockaddr storage, resolver calls, socket ownership, SSL fd/winsock handle custody, and async poller readiness). Until then, Windows native `stdlib_net` routes through explicit no-net intrinsics rather than compiling a partial POSIX-shaped socket layer.
...
- Implemented: `os.mkdir`/`os.makedirs` now lower `mode` through Rust intrinsics (`molt_path_mkdir(path, mode)` and `molt_path_makedirs(path, mode, exist_ok)`), including `__index__` exception-propagation parity.
- Implemented: `sys` metadata attrs `hexversion`, `api_version`, platform-gated `abiflags`, and `implementation` now lower through dedicated runtime intrinsics (`molt_sys_hexversion`, `molt_sys_api_version`, `molt_sys_abiflags`, `molt_sys_implementation_payload`) with shim-side payload validation; Windows targets match CPython by omitting `sys.abiflags`.
- Implemented: `weakref.ReferenceType.__callback__` is now intrinsic-backed (`molt_weakref_callback`) and matches CPython alive/dead callback visibility.
- Implemented: `os.stat`/`os.lstat`/`os.fstat`/`os.rename`/`os.replace` now lower through dedicated Rust intrinsics (`molt_os_stat`, `molt_os_lstat`, `molt_os_fstat`, `molt_os_rename`, `molt_os_replace`) with thin shim-side `os.stat_result` shaping.
- Implemented: `sys.flags` is now intrinsic-backed through `molt_sys_flags_payload` with shim-side payload validation and CPython-ordered sequence-field shaping.
- Implemented: weakref finalize detach/peek/alive/idempotence edge semantics now have focused differential coverage (`weakref_finalize_detach_peek_edges.py`) on top of intrinsic-backed finalize/atexit behavior.
...
- Implemented: importlib zip source loader discovery/execution lowers through Rust intrinsics (`molt_importlib_find_spec_payload` zip payload fields + `molt_importlib_zip_source_payload`) and `importlib.machinery.ZipSourceLoader`; source, zip-source, extension-shim, and sourceless-shim execution share the canonical compiler-emitted module transaction, while native extension execution remains capability-gated through the module-object loader transaction. The obsolete dict-only extension/sourceless execution ABIs are deleted.
- TODO(stdlib-compat, owner:stdlib, milestone:SL3, priority:P2, status:partial): full importlib native extension and pyc execution parity where no compiler-emitted module body exists.
- Implemented: `importlib.resources` custom loader reader bootstrap (`module` name resolution + `get_resource_reader` dispatch) and reader contract surfaces now lower through dedicated runtime intrinsics (`molt_importlib_resources_module_name`, `molt_importlib_resources_loader_reader`, `molt_importlib_resources_reader_roots`, `molt_importlib_resources_reader_contents`, `molt_importlib_resources_reader_resource_path`, `molt_importlib_resources_reader_is_resource`, `molt_importlib_resources_reader_open_resource_bytes`, `molt_importlib_resources_reader_child_names`) in addition to intrinsic source/zip/sourceless loader readers; `resource_path()` now remains filesystem-only across all reader fallback branches (direct `resource_path`, traversable, and roots payload lanes), while archive members stay byte-openable through intrinsic resource reads.
...
- Implemented: `atexit` core callback registry/shutdown-drain execution is intrinsic-backed (no import-only stub lane), including callback continuation after failures, version-gated stderr callback+traceback reporting parity (3.12 vs 3.13+ punctuation shape), custom `sys.unraisablehook` payload parity (`UnraisableHookArgs` shape/version semantics), and weakref-finalizer runner ordering parity with user callbacks (including `_clear`/`unregister` callback-list rewrites).
...
- Implemented: `stat`/`textwrap`/`urllib.parse` core lanes are now intrinsic-backed (`molt_stat_*`, `molt_textwrap_*`, `molt_urllib_*`) with Python shims reduced to thin wrapper/result-shaping logic; `urllib` package root now requires a runtime intrinsic and is no longer Python-only.
