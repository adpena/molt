...
- TODO(type-coverage, owner:runtime, milestone:TC3, priority:P2, status:partial): buffer protocol + memoryview layout. Runtime-owned typed strided storage/export now backs bytes/memoryview construction, direct released-view fail-closed behavior, `molt_buffer_export/acquire`, and the CPython ABI `PyObject_GetBuffer` projection; generic bytes-like consumers still need released-memoryview precedence routed through typed buffer authority. Exporter lock/count parity and NumPy ndarray construction remain.
