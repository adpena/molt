...
- Implemented: `importlib.util.find_spec` now lowers meta/path finder execution and namespace package discovery through runtime intrinsic payloads (`molt_importlib_find_spec_payload` + `molt_importlib_find_in_path`), including namespace-to-submodule resolution across package search roots and extension-module spec discovery.
- Implemented: `importlib.util.find_spec` path-hook finder caching now lowers through runtime intrinsic payloads with explicit `sys.path_importer_cache` wiring, so finder reuse is runtime-owned and no Python fallback cache path is needed.
- Implemented: importlib runtime state probing (`sys.modules`, `meta_path`, `path_hooks`, `path_importer_cache`) now lowers through `molt_importlib_runtime_state_payload`, removing Python-side `getattr(sys, ...)` state probing in `importlib.__init__` and `importlib.util`.
...
- Implemented: `importlib.metadata` name/version/header and entry-point parsing now lower through runtime payload intrinsic (`molt_importlib_metadata_payload`), aggregated entry-point discovery/filtering now lowers through `molt_importlib_metadata_entry_points_payload` + `molt_importlib_metadata_entry_points_select_payload`, and distribution-name canonicalization is runtime-owned via `molt_importlib_metadata_normalize_name`.
