...
// ---------------------------------------------------------------------------
// Embedded production monitoring dashboard (serves at /dashboard)
// ---------------------------------------------------------------------------
    ...
    <script>
        ...
        async function refresh() {
            try {
                ...
                const mv = d.model_variant || '\\u2014';
                const dv = d.device || '\\u2014';
                const wa = d.wasm_available ? 'yes' : 'no';
                const sc = d.status === 'ready' ? 'status-ok' : 'status-warn';
                document.getElementById('model-info').innerHTML =
                    '<div class="metric"><span>Status</span><span class="' + sc + '">' + d.status + '</span></div>' +
                    '<div class="metric"><span>Variant</span><span>' + mv + '</span></div>' +
                    '<div class="metric"><span>Device</span><span>' + dv + '</span></div>' +
                    '<div class="metric"><span>WASM</span><span>' + wa + '</span></div>';
                ...
                document.getElementById('perf-info').innerHTML =
                    ...
                    '<div class="metric"><span>p95 latency</span><span>' + (lat.p95_ms || 0) + 'ms</span></div>';
            } catch (e) {
        ...
        async function checkAssets() {
            ...
            for (var i = 0; i < assets.length; i++) {
                var a = assets[i];
                try {
                    var r = await fetch(BASE + a.url, { method: 'HEAD', headers: { Origin: 'https://freeinvoicemaker.app' } });
                    var size = parseInt(r.headers.get('content-length') || '0');
                    var sizeStr = size > 1e6 ? (size/1e6).toFixed(1) + ' MB' : (size/1e3).toFixed(1) + ' KB';
                    html += '<div class="metric"><span>' + a.name + '</span><span class="' + (r.ok ? 'status-ok' : 'status-err') + '">' + (r.ok ? sizeStr : r.status) + '</span></div>';
                } catch (e) { html += '<div class="metric"><span>' + a.name + '</span><span class="status-err">Error</span></div>'; }
            }
        ...
        async function checkEndpoints() {
            ...
            for (var i = 0; i < eps.length; i++) {
                var ep = eps[i];
                try {
                    var opts = { method: ep.method, headers: { Origin: 'https://freeinvoicemaker.app' } };
                    if (ep.body) { opts.headers['Content-Type'] = 'application/json'; opts.body = ep.body; }
                    var start = performance.now();
                    var r = await fetch(BASE + ep.path, opts);
                    var ms = Math.round(performance.now() - start);
                    html += '<div class="metric"><span>' + ep.name + '</span><span class="' + (r.ok ? 'status-ok' : 'status-warn') + '">' + r.status + ' (' + ms + 'ms)</span></div>';
                } catch (e) { html += '<div class="metric"><span>' + ep.name + '</span><span class="status-err">Error</span></div>'; }
            }
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    // -----------------------------------------------------------------------
    // Production monitoring dashboard: live health, perf, assets, endpoints.
    // -----------------------------------------------------------------------
