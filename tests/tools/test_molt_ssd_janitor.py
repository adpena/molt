...
def test_resolve_root_allows_artifact_root_from_linked_worktree(
    ...
    assert jan._resolve_root(str(root), force=True) == root
...
def test_gather_protects_registered_and_fresh(tmp_path: Path) -> None:
    root = tmp_path / "Molt"
    ...
    fresh_tmp.mkdir()
    old_tmp.mkdir()
    # Session targets remain outside this legacy janitor's authority even when old.
    ...
    plan = jan._gather(
        root,
        classes=("tmp", "worktrees"),
        ages={"tmp": 3, "scratch": 21, "worktrees": 7, "cargo": 30},
        min_idle_hours=6.0,
        cache_cap_gb=30.0,
    ...
    assert old_session.resolve() not in selected
    # Never the registered worktree, any session target, or a fresh entry.
...
def test_session_class_is_rejected_in_favor_of_canonical_disk_guard() -> None:
    args = argparse.Namespace(all=False, classes="sessions")
    with pytest.raises(SystemExit, match="unknown class"):
        jan._select_classes(args)
