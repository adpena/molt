...
def test_proof_queue_diagnoses_pact_witness_missing_fixture(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    db = tmp_path / "proof_queue.sqlite3"
    log_path = tmp_path / "pact-witness-missing-fixture.log"
    fixture_path = (
        tmp_path / "collab" / "pact" / "pact_witness_kernel" / "lstar_sample.npz"
    )
    conn = proof_queue._connect(db)
    proof_queue._insert_run(
        conn,
        run_id="pact-witness-missing-fixture-run",
        logical_id="pact-witness-acceptance",
        reason="prove missing witness fixture diagnosis",
        ...
        log_path=log_path,
        summary_json=tmp_path / "pact-witness-missing-fixture.memory_guard.json",
    )
    log_path.write_text(
        "Successfully built output.wasm\n"
        "Successfully linked output_linked.wasm\n"
        f"missing Pact fixture: {fixture_path}\n"
        "proof_queue finished status=failed exit_code=1 elapsed=2403.484s\n",
        encoding="utf-8",
    )
    proof_queue._update_run(
        conn, "pact-witness-missing-fixture-run", status="failed", returncode=1
    )
    ...
    assert (
        proof_queue.main(
            [
                ...
                "evidence",
                "--run-id",
                "pact-witness-missing-fixture-run",
            ]
        )
        == 0
    )
    ...
    assert "lstar_sample.npz" in diagnostics[0]["summary"]
    assert "pact-witness-oracle" in diagnostics[0]["next_action"]
    assert "collab/pact/" in diagnostics[0]["scopes"]
