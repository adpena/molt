...
def _terminalize_synthetic_run(
    ...
    *,
    ...
    if executed:
        row = conn.execute(
            "SELECT command_envelope_json FROM proof_runs WHERE run_id = ?", (run_id,)
        ).fetchone()
        assert row is not None
        envelope = json.loads(row[0])
        requested_toolchains = envelope["toolchains"]
        toolchains = {
        ...
        receipt_context["platform_process_custody"] = {
            ...
            "prelaunch_sha256": supervisor_custody._canonical_payload_sha256(
                platform_images
            ),
...
def _write_synthetic_guarded_execution(
    ...
    toolchains = {"python": synthetic_python_toolchain(result_path.parent)}
    toolchains, compact_custody, v3 = custody_factory(
        result_path.parent,
        toolchains,
        nonce=request["execution_nonce"],
    ...
    receipt_context = {
        ...
        "execution_nonce_sha256": hashlib.sha256(
            request["execution_nonce"].encode()
        ).hexdigest(),
        "source_commit": "b" * 40,
        "source_tree": "d" * 40,
        "source_tree_state": "clean",
    ...
    receipt_context = execution_receipt_details.compact_context(
        receipt_context, cas_root=result_path.parent / "custody-cas"
    )
    receipt_context["execution_custody_sha256"] = (
...
def test_proof_command_envelope_is_typed_and_command_derived_cross_platform(
    ...
    assert envelope["schema"] == command_admission.ENVELOPE_SCHEMA
...
def test_opaque_or_ambiguous_commands_are_rejected_before_execution(
    ...
    rejected = command_admission.admission_envelope(command)
...
def test_guarded_exec_delegation_is_exact_and_generic_binary_is_typed() -> None:
    with pytest.raises(ValueError, match="unknown proof executable kind"):
    ...
    disguised_command = [
        "python",
        "eviltools/guarded_exec.py",
        "--",
        "node",
        "--version",
    ]
    disguised = command_admission.envelope_for_command(disguised_command)
    with pytest.raises(ValueError, match="relative guarded_exec path is not canonical"):
        command_identity._bind_delegated_command(
            disguised,
            list(disguised_command),
            cwd=state.ROOT,
            env=os.environ,
        )
...
def test_guarded_exec_binds_canonical_script_and_delegated_binary_by_content() -> None:
    ...
    envelope = command_admission.envelope_for_command(command)
    exact = list(command)
    guarded_exec, delegated = command_identity._bind_delegated_command(
        envelope,
        exact,
        cwd=state.ROOT,
        env=os.environ,
    ...
    assert os.path.normcase(exact[3]) == os.path.normcase(os.path.abspath(node))
    assert guarded_exec is not None and command_identity._content_identity_available(
        guarded_exec
    )
    assert delegated is not None and command_identity._content_identity_available(
        delegated
    )


@pytest.mark.parametrize("mode", ["relative", "absolute", "module"])
def test_guarded_exec_spellings_share_one_recursive_binding_authority(
    mode: str,
) -> None:
    node = shutil.which("node")
    assert node is not None
    canonical = (state.ROOT / "tools" / "guarded_exec.py").resolve()
    if mode == "module":
        target = ["-m", "tools.guarded_exec"]
    elif mode == "absolute":
        target = [str(canonical)]
    else:
        target = ["tools/guarded_exec.py"]
    command = [sys.executable, *target, "--", node, "--version"]
    envelope = command_admission.envelope_for_command(command)
    assert envelope["guarded_exec"]["mode"] == (
        "module" if mode == "module" else "script"
    )
    exact = command_identity._exact_command(envelope, cwd=state.ROOT, env=os.environ)
    guarded, delegated = command_identity._bind_delegated_command(
        envelope, exact, cwd=state.ROOT, env=os.environ
    )
    assert str(canonical) in exact
    assert os.path.normcase(exact[-2]) == os.path.normcase(os.path.abspath(node))
    assert command_identity._content_identity_available(guarded or {})
    assert command_identity._content_identity_available(delegated or {})
...
@pytest.mark.parametrize(
    "option",
    [
        "--default-index=https://example.invalid/simple",
        "--env-file=.env",
...
def test_uv_input_escape_options_are_rejected_by_one_semantics_authority(
    option: str,
) -> None:
    with pytest.raises(ValueError, match="non-hermetic"):
        command_admission.envelope_for_command(["uv", "run", option, "python", "-V"])


def test_non_python_command_requires_a_closed_toolchain_registration() -> None:
    with pytest.raises(ValueError, match="unknown proof executable kind"):
        command_admission.envelope_for_command(
...
def test_luau_runtime_command_owns_required_runner_without_ambient_discovery() -> None:
    plan = proof_plan.ProofPlan.load()
    command = next(
        item for item in plan.commands if item.id == "rust.test.compiler-authorities"
    )
    envelope = command_admission.envelope_for_command(command.argv)
    assert envelope["kind"] == "proof-plan"
    assert "lune" in envelope["process_closure"]["toolchains"]
    assert "--include-ignored" in command.argv
    assert "--no-fail-fast" in command.argv
    config_index = command.argv.index("--config")
    assert command.argv[config_index + 1] == (
        "profile.dev-fast.package.cranelift-codegen.opt-level=0"
    )
    assert config_index < command.argv.index("--")
    assert {"cargo", "lune", "node", "wasm-ld"} <= set(envelope["toolchains"])
    compiler_only = [arg for arg in command.argv if arg != "--include-ignored"]
    compiler_envelope = command_admission.envelope_for_command(compiler_only)
    assert "lune" not in compiler_envelope["toolchains"]
...
def test_registered_process_spawning_toolchain_owns_declared_closure(
    command: list[str],
) -> None:
    envelope = command_admission.envelope_for_command(command)
    assert envelope["process_closure"] == {
...
def test_registered_toolchain_capability_queries_remain_true_leaves(
    command: list[str],
) -> None:
    envelope = command_admission.envelope_for_command(command)
...
def test_delegated_cargo_target_is_the_rust_linker_selection_authority() -> None:
    command = [
        sys.executable,
        "-m",
        "tools.guarded_exec",
...
@pytest.mark.parametrize(
    ("executable", "toolchain"), [("node", "node"), ("wasm-tools", "wasm-tools")]
)
def test_registered_direct_toolchain_commands_cannot_fall_back_to_an_empty_kind(
    executable: str, toolchain: str
) -> None:
    envelope = command_admission.envelope_for_command([executable, "--version"])
...
def test_uv_directory_and_project_must_stay_inside_admitted_source_root(
    ...
    outside.mkdir()
    escaped = command_admission.envelope_for_command(
        ["uv", "run", "--directory", str(outside), "python", "-V"]
    )
    with pytest.raises(ValueError, match="escapes admitted source root"):
        command_admission._execution_source_paths(escaped, cwd=root)


def test_uv_symlinked_source_selector_cannot_escape_admitted_root(
    tmp_path: Path,
) -> None:
    root = tmp_path / "source"
    outside = tmp_path / "outside"
    ...
    envelope = command_admission.envelope_for_command(
        ["uv", "run", "--directory", "linked", "python", "-V"]
    )
    with pytest.raises(ValueError, match="escapes admitted source root"):
...
def test_node_tool_identity_binds_runtime_versions_configuration_and_global_paths(
    ...
    command_identity._validate_toolchain_identity(plan, "node", identity)
    assert identity["runtime"] == runtime_payload
...
def test_quint_tool_identity_binds_resolved_node_package_tree(
    ...
    def fake_run(
        ...
        del cwd, env, timeout
        argv = list(command)
        if "@informalsystems/quint" in argv:
    ...
    monkeypatch.setattr(command_identity, "_run_captured", fake_run)
    plan = proof_plan.ProofPlan.load()
    before = command_identity._tool_identity(
        plan,
        "quint",
        {"python": None},
        [sys.executable],
...
def test_execution_environment_authority_covers_path_rust_and_wrapper_family() -> None:
    ...
    normalized, policies = normalize_cargo_environment(base)
    execution_env, contract = (
        execution_environment._deterministic_execution_environment(
            normalized, override_names=[]
        )
    )
    ...
    assert variables["CARGO_INCREMENTAL"]["redacted"] is True
    assert "value" not in variables["CARGO_INCREMENTAL"]
    for name in (
        "RUSTC_WRAPPER",
    ...
    assert "UV_INDEX_URL" not in variables
    assert variables["UV_CACHE_DIR"]["redacted"] is True
    changed_env = {**execution_env, "RUSTUP_TOOLCHAIN": "nightly"}
    changed = execution_environment._execution_environment_authority(
        changed_env,
        applied_cargo_policies=policies,
        fingerprint_key=fingerprint_key,
        contract=contract,
...
@pytest.mark.parametrize(
    ("name", "value"),
    [
        ("UNCLASSIFIED_BEHAVIOR", "1"),
        ("MOLT_BAD-NAME", "1"),
        ("PYTHONPATH", "C:/untracked/source"),
        ("PYTHONHOME", "C:/untracked/python"),
        ("PYTHONSTARTUP", "C:/untracked/startup.py"),
        ("PYTHONBREAKPOINT", "untracked.debugger"),
        ("PYTEST_ADDOPTS", "-p malicious"),
        ("UV_INDEX_URL", "https://example.invalid/simple"),
        ("MOLT_MEMORY_GUARD_STATE_ROOT", "C:/unowned/control"),
        ("MOLT_API_TOKEN", "super-secret-value"),
        ("MOLT_ENDPOINT", "https://user:password@example.invalid"),
        ("MOLT_ENDPOINT", "https://bearer-token@example.invalid"),
    ],
)
def test_environment_override_policy_rejects_unbound_or_secret_behavior(
    name: str, value: str
) -> None:
    assert (
        execution_environment.environment_override_policy_error({name: value})
        is not None
    )


def test_deterministic_environment_omits_ambient_injection_and_binds_all_passed() -> (
    None
):
    inherited = {
        "PATH": "C:/tools",
        ...
        "UNCLASSIFIED_BEHAVIOR": "active",
        "MOLT_AUTH_TOKEN": "secret",
    }
    selected, contract = execution_environment._deterministic_execution_environment(
        inherited, override_names=["MOLT_TRACE_IMPORT_STAGE"]
    )
    assert selected == {"PATH": "C:/tools", "MOLT_TRACE_IMPORT_STAGE": "1"}
    assert contract["passed_names"] == ["MOLT_TRACE_IMPORT_STAGE", "PATH"]
    assert contract["override_names"] == ["MOLT_TRACE_IMPORT_STAGE"]
...
def test_environment_override_policy_rejects_case_ambiguous_names() -> None:
    error = execution_environment.environment_override_policy_error(
        {"MOLT_TRACE_CAPI": "1", "molt_trace_capi": "0"}
    )
    assert error is not None
    assert "case-ambiguous" in error
...
def test_environment_pair_parser_rejects_duplicate_names_before_collapsing(
    pairs: list[str],
) -> None:
    with pytest.raises(SystemExit, match="duplicate environment override names"):
        policy._env_overrides_from_pairs(pairs)


def test_uv_custody_flags_cannot_be_spoofed_by_payload_arguments() -> None:
    command = [
        "uv",
        "run",
        "python",
    ...
    error = policy._proof_command_policy_error(command)
    assert error is not None
    assert (
        "missing --active, --project ., --python 3.12, --no-sync, --no-config" in error
    )
...
@pytest.mark.parametrize(
    ...
    [
        ([sys.executable, "-c", "pass"], {"MOLT_API_TOKEN": "do-not-persist"}),
        ([sys.executable, "--token", "do-not-persist"], {}),
        ([sys.executable, "https://bearer-token@example.invalid"], {}),
    ],
)
def test_secret_input_is_rejected_before_database_log_or_evidence_projection(
    tmp_path: Path,
    ...
    rc, run_id = runner._queue_one(
        args,
        logical_id="secret-rejection",
        reason="prove secrets never enter custody storage",
        command=command,
        resource_family="python-tests",
        contention_key="secret-rejection",
        scopes=["tools/proof_queue_pkg/guarded_execution.py"],
        env_overrides=env_overrides,
        initial_notes=["secret rejection fixture"],
    )
    assert rc == 2
    assert run_id is None
    assert not args.db.exists()
    assert not args.logs_root.exists()
...
def test_python_probe_verifies_declared_record_hash_against_installed_bytes(
    ...
    module = purelib / "custody_demo.py"
    module.write_bytes(b"original\n")
    dist_info = purelib / "custody_demo-1.0.dist-info"
    dist_info.mkdir()
    ...
    (dist_info / "RECORD").write_text(
        ...
        "custody_demo-1.0.dist-info/METADATA,,\n"
    ...
    outside = tmp_path.parent / f"{tmp_path.name}-record-escape.py"
    outside.write_bytes(b"original\n")
    escaped_relative = os.path.relpath(outside, purelib).replace(os.sep, "/")
    (dist_info / "RECORD").write_text(
        f"{escaped_relative},,\n"
        "custody_demo-1.0.dist-info/METADATA,,\n"
        "custody_demo-1.0.dist-info/RECORD,,\n",
...
def test_python_probe_classifies_owned_record_symlinks_and_rejects_external_escape(
    ...
    target = purelib / "custody_symlink_target.py"
    target.write_bytes(b"owned\n")
    linked = purelib / "custody_symlink.py"
    try:
        os.symlink(target, linked)
    except OSError as exc:
        pytest.skip(f"symlink creation unavailable: {exc}")
    dist_info = purelib / "custody_symlink-1.0.dist-info"
    dist_info.mkdir()
    (dist_info / "METADATA").write_text(
        "Metadata-Version: 2.1\nName: custody-symlink\nVersion: 1.0\n",
        encoding="utf-8",
    )
    declared = (
...
def test_python_probe_ignores_ambient_ownership_and_uses_admitted_pep610_root(
    ...
    source = project / "src"
    metadata_root = source / "custody_ambiguous.egg-info"
    metadata_root.mkdir(parents=True)
    (metadata_root / "PKG-INFO").write_text(
        "Metadata-Version: 2.1\nName: custody-ambiguous\nVersion: 1.0\n",
        encoding="utf-8",
    )
    ...
    direct_metadata = purelib / "custody_ambiguous-1.0.dist-info"
    direct_metadata.mkdir()
    (direct_metadata / "METADATA").write_text(
        "Metadata-Version: 2.1\nName: custody-ambiguous\nVersion: 1.0\n",
        encoding="utf-8",
    )
    (direct_metadata / "top_level.txt").write_text(
        "custody_ambiguous\n", encoding="utf-8"
    )
    (direct_metadata / "direct_url.json").write_text(
        json.dumps(
            {
    ...
    (direct_metadata / "RECORD").write_text(
        "custody_ambiguous-1.0.dist-info/METADATA,,\n"
        "custody_ambiguous-1.0.dist-info/top_level.txt,,\n"
        "custody_ambiguous-1.0.dist-info/direct_url.json,,\n"
...
def test_python_probe_does_not_follow_ambient_source_metadata_symlinks(
    ...
    _initialize_clean_git_repo(project)
    source = project / "src"
    (source / "custody_symlink.py").parent.mkdir(parents=True, exist_ok=True)
    (source / "custody_symlink.py").write_text("VALUE = 1\n", encoding="utf-8")
    metadata_root = source / "custody_symlink.egg-info"
    metadata_root.mkdir()
    (metadata_root / "PKG-INFO").write_text(
        "Metadata-Version: 2.1\nName: custody-symlink\nVersion: 1.0\n",
        encoding="utf-8",
    )
    (metadata_root / "top_level.txt").write_text("custody_symlink\n", encoding="utf-8")
    external = project / "external-metadata.txt"
    external.write_text("outside metadata root\n", encoding="utf-8")
    try:
        os.symlink(external, metadata_root / "escaped.txt")
    except OSError as exc:
        pytest.skip(f"symlink creation unavailable: {exc}")
    environment = {**os.environ, "PYTHONPATH": str(source)}
    completed = run_custody_subject_process(
...
def test_execution_request_nonce_invalidates_stale_result_and_transcripts(
    tmp_path: Path,
) -> None:
    command = [sys.executable, "-c", "pass"]
    envelope = command_admission.envelope_for_command(command)
    ...
    stale_paths = [
        log.with_suffix(".execution-request.json"),
        log.with_suffix(".execution.json"),
        log.with_suffix(".execution.stdout.bin"),
        log.with_suffix(".execution.stderr.bin"),
        log.with_suffix(".memory_guard.json"),
    ]
    for path in stale_paths:
        path.write_text("stale", encoding="utf-8")
    request_path, result_path, persisted, first_nonce = runner._write_execution_request(
    ...
    assert request["execution_nonce"] == first_nonce
    assert not result_path.exists()
    assert not result_path.with_suffix(".stdout.bin").exists()
    assert not result_path.with_suffix(".stderr.bin").exists()
    _, _, _, second_nonce = runner._write_execution_request(
        row=row,  # type: ignore[arg-type]
        command=command,
        repo_root=tmp_path,
        resource_family="python-tests",
...
def test_guard_receipt_rejects_replay_substitution_and_dirty_terminal_state(
    ...
    for mutation, message in [
        ...
        ({"windows_job_cleanup": {"completed": False}}, "cleanup is incomplete"),
        (
            {
                "windows_job_cleanup": {
                    "completed": True,
        ...
        (
            ...
            "sampling enforcement is incomplete",
        ),
    ]:
        summary.write_text(json.dumps({**base, **mutation}), encoding="utf-8")
...
def test_execution_context_rehashes_nonce_custody_and_transcript_artifacts(
    ...
    assert_execution_context_rejects_substitutions(tmp_path, synthetic_receipt_custody)


def test_synthetic_receipt_verifier_rejects_unissued_requests(
    tmp_path: Path,
    synthetic_receipt_custody: ReceiptCustodyFactory,
) -> None:
    del synthetic_receipt_custody
    with pytest.raises(AssertionError, match="unissued synthetic native verification"):
        runner._COMMANDS.run(
            [
                str(tmp_path / "unknown.bin"),
                "verify",
                "--policy",
                str(tmp_path / "unknown-policy.json"),
                "--receipt",
                str(tmp_path / "unknown-receipt.json"),
            ],
            check=False,
            capture_output=True,
            text=True,
...
def test_transcript_receipt_streams_hash_and_structured_counts(tmp_path: Path) -> None:
    transcript = tmp_path / "stdout.bin"
    transcript.write_bytes(b"alpha\n12 passed, 2 skipped in 1.00s\nomega\n")
    identity = command_identity._transcript_identity(transcript)
    assert identity["test_counts"] == {"passed": 12, "skipped": 2}
    assert identity["structured_test_output"] is True
...
def test_guarded_receipt_rejects_stable_dirty_source(
    ...
    assert "source-dirty-prelaunch" in custody_payload["ineligible_reasons"]
...
def test_live_custody_detects_mutate_execute_restore_transient(
    ...
    context = record["receipt_context"]
    assert context["execution_custody_session"] == {
    ...
    live_events = custody_cas.read_ref(
        context["live_input_custody"]["event_artifact"],
        expected_root=tmp_path / "custody-cas",
    )
...
def test_python_leaf_blocks_cargo_and_node_children_before_launch(
    ...
    assert rc == 0
    context = record["receipt_context"]
    receipt = execution_receipt_details.expand_context(
        context, cas_root=tmp_path / "custody-cas"
    )["child_process_custody"]["receipt"]
...
def test_python_bootstrap_parser_preserves_interpreter_options_and_payload() -> None:
    ...
    assert invocation.interpreter_options == (
        "-S",
        "-E",
        "-I",
        "-X",
    ...
    assert invocation.target == "print('owned')"
    assert invocation.arguments == ("--payload-flag",)
...
@pytest.mark.parametrize(
    ("mode", "target", "expected_argv0"),
    [
        ("command", "import json,sys; print(json.dumps(sys.argv))", "-c"),
        ("stdin", None, "-"),
    ],
)
def test_python_bootstrap_command_and_stdin_argv_semantics(
    ...
    bootstrap = Path(command_admission.__file__).with_name(
    ...
    completed = run_custody_subject_process(
        ...
        env={
            ...
            if not key.startswith("MOLT_PROOF_CHILD_CUSTODY")
        },
    )
    assert completed.returncode == 0, completed.stderr
    assert json.loads(completed.stdout) == [expected_argv0, "payload"]


def test_python_bootstrap_module_and_script_main_semantics(tmp_path: Path) -> None:
    package = tmp_path / "sample_package"
    package.mkdir()
    (package / "__init__.py").write_text("", encoding="utf-8")
    probe = (
        "import json,sys; print(json.dumps({"
        "'argv':sys.argv,'name':__name__,'package':__package__,"
        "'spec':__spec__.name if __spec__ else None}))"
    )
    ...
    assert module_payload["argv"][1:] == ["payload"]
    assert module_payload["name"] == "__main__"
    ...
    assert script_payload["argv"] == [str(script), "payload"]
...
def test_python_bootstrap_pytest_disables_source_cache_exactly_once(
    ...
    test_file.write_text("def test_owned():\n    assert True\n", encoding="utf-8")
    envelope = {"python": {"kind": "direct"}}
    ...
    already_disabled = command_admission._python_bootstrap_command(
        envelope,
        [
            sys.executable,
            "-m",
...
def test_python_bootstrap_installs_custody_under_isolated_startup(
    ...
    _initialize_clean_git_repo(repo)
    marker = tmp_path / "bootstrap-bypass-marker"
    marker_code = repr(f"from pathlib import Path; Path({str(marker)!r}).touch()")
    code = "\n".join(
        [
            "import os,sys; blocked=[]",
            "attempts=["
            "('system',lambda:os.system('echo bypass')) ,"
            f"('exec',lambda:os.execv(sys.executable,[sys.executable,'-c',{marker_code}])) ,"
            "('spawn',lambda:os.spawnv(os.P_WAIT,sys.executable,"
    ...
    receipt = execution_receipt_details.expand_context(
        record["receipt_context"], cas_root=tmp_path / "custody-cas"
    )["child_process_custody"]["receipt"]
    assert receipt["broker_complete"] is True
    assert any(
    ...
    assert capture["telemetry"]["receipt_context_bytes"] <= 64 * 1024
    assert capture["telemetry"]["receipt_context_bytes"] == len(
...
def test_real_minimal_cargo_link_has_single_prearm_selection_and_compact_custody(
    ...
    provision_target = Path(
        context["process_supervisor"]["provision_telemetry"]["build_target_dir"]
    )
    ...
    full_capture = toolchain_capture.load_capture(
        context["toolchain_capture"]["artifact"],
        cas_root=execution_path.parent / "custody-cas",
    )
    full_toolchains = full_capture["toolchains"]
    assert (
    ...
    event = context["process_supervisor"]["event_artifact"]
...
def test_python_leaf_blocks_exec_replacement_before_launch(
    ...
    script = (
        "import os,sys; blocked=False; "
        "\ntry: os.execv(sys.executable,[sys.executable,'-c',"
        + repr(f"from pathlib import Path; Path({str(marker)!r}).touch()")
        + "])"
...
def test_node_leaf_rejects_shell_mediated_spawn(tmp_path: Path) -> None:
    ...
    server = execution_custody.ChildCustodyEventServer("node", policy_payload)
    env = dict(os.environ)
    env.update(server.environment())
    env[execution_custody.CHILD_POLICY_ENV] = json.dumps(policy_payload)
...
def test_unavailable_executable_and_requirements_hashes_are_never_evidence(
    ...
    executable = command_identity._executable_identity(Path(sys.executable))
    overlay = command_identity._file_identity(requirements)
    assert command_identity._content_identity_available(executable) is False
    assert command_identity._content_identity_available(overlay) is False


def test_proof_queue_durable_state_uses_external_custody_root(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_guarded_execution_rejects_source_owned_outputs_before_arming(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    repo = tmp_path / "source"
    _initialize_clean_git_repo(repo)
    result = repo / "logs" / "proof-execution.json"
    result.parent.mkdir()
    command = [sys.executable, "-c", "pass"]
    request = tmp_path / "source-output.request.json"
    request.write_text(
        json.dumps(
            {
                "schema": command_admission.EXECUTION_SCHEMA,
                "run_id": "source-output-unit-run",
    ...
    with pytest.raises(ValueError, match="outputs must be outside effective source"):
        guarded_execution.execute_guarded_request(request)
    assert not result.exists()
    assert not result.with_suffix(".supervisor-policy.json").exists()
    assert not result.with_suffix(".supervisor-receipt.json").exists()
    assert not (result.parent / "custody-cas").exists()
...
def test_connect_never_rewrites_queued_or_terminal_admission_envelopes(
    ...
    monkeypatch.setattr(command_admission, "admission_envelope", drifted)
...
def test_proof_queue_exec_records_passed_run(
    ...
    rc = cli.main(
        [
            ...
            "python:queue-smoke",
            "--env",
            "MOLT_PROOF_QUEUE_TEST=queue-ok",
            "--note",
            ...
            str(custody_python),
            "-c",
            "import os; print(os.environ['MOLT_PROOF_QUEUE_TEST'])",
        ]
    )
    ...
    assert receipt_context["python_interpreters"]["proof_command"]["executable"] == str(
        custody_python
    )
...
def test_proof_queue_run_id_executes_only_selected_queued_row(
    ...
    for run_id, marker in (("queued-a", "A"), ("queued-b", "B")):
        scheduling._insert_run(
            ...
            logical_id=run_id,
            reason=f"run {marker}",
            command=[str(custody_python), "-c", f"print('{marker}')"],
            cwd=repo,
            resource_family="python",
...
def test_proof_queue_cargo_lane_records_guarded_uv_envelope(
    ...
    assert command[16:18] == ["-p", "molt-runtime"]
    ...
    assert authority == command_admission.envelope_for_command(command)
    ...
    platform_images = process_image_capture.platform_auxiliary_images(
        authority["process_closure"]["descendants"]
    )
    receipt_context["platform_process_custody"] = {
        "schema": process_image_capture.PROCESS_IMAGE_SCHEMA,
        "prelaunch": platform_images,
    ...
    assert requested_toolchains == ("python", "uv", "cargo", "rustc", "git")
...
def test_proof_queue_submit_run_executes_queued_row_in_place(
    ...
    assert (
        'env_override_names=["MOLT_PROOF_QUEUE_TEST", "RUSTUP_TOOLCHAIN"]'
        in queued_log_text
    )
    assert "queued-ok" not in queued_log_text
    ...
    dsl.write_text(
        "\n".join(
            [
                ...
                f'command = [{str(custody_python)!r}, "-c", "print(\'child\')"]',
                "",
                "[[proof]]",
                'id = "parent-proof"',
                ...
                'resource_family = "python"',
                'contention_key = "python:parent-child"',
                f'command = [{str(custody_python)!r}, "-c", "print(\'parent\')"]',
            ]
        ),
        encoding="utf-8",
    )
    ...
    dsl.write_text(
        "\n".join(
            [
                ...
                '  "--python", "3.12", "--no-sync", "--no-config", "python", "-c", "print(1)",',
                "]",
            ]
        ),
        encoding="utf-8",
...
def test_proof_queue_named_spec_locked_environment_authority_is_generic(
    ...
    spec = {
        ...
        "env_overrides": {"MOLT_LOCKED_INPUT": "attested"},
        "locked_env": ["MOLT_LOCKED_INPUT"],
    }
    args = SimpleNamespace(env=["molt_locked_input=user"], print_spec=True)
    ...
    assert exc.value.code == (
        ...
        "locked environment custody: MOLT_LOCKED_INPUT"
    )
...
def test_proof_queue_native_molt_run_is_queue_native(tmp_path: Path) -> None:
    ...
    assert command[10:14] == ["-m", "molt.cli", "run", "tmp/probe.py"]
...
def test_proof_queue_pact_witness_acceptance_admits_staged_extension_roots(
    ...
    artifact_root = tmp_path / "artifacts"
    expected_seals = [
        _pact_witness_fixture_root(artifact_root, package)
...
def test_proof_queue_pact_witness_acceptance_fails_when_canonical_scipy_is_absent(
    ...
    artifact_root = tmp_path / "artifacts"
    durable_numpy = _pact_witness_fixture_root(artifact_root, "numpy")
...
def test_proof_queue_pact_witness_acceptance_rejects_incomplete_scipy_set(
    ...
    artifact_root = tmp_path / "artifacts"
    durable_numpy = _pact_witness_fixture_root(artifact_root, "numpy")
    durable_scipy = _pact_witness_fixture_root(artifact_root, "scipy")
    numpy_identity = _write_current_scientific_seal(durable_numpy, package="numpy")
...
def test_proof_queue_pact_witness_roots_accept_artifact_specific_manifests(
    ...
    monkeypatch.setenv("MOLT_EXT_ROOT", str(tmp_path / "artifacts"))
    durable_scipy = _pact_witness_fixture_root(artifact_root, "scipy")
    scipy_identity = _write_current_scientific_seal(durable_scipy)
    assert numpy_identity is not None and scipy_identity is not None
    _patch_pact_expected_identities(
        monkeypatch,
        {"numpy": numpy_identity, "scipy": scipy_identity},
