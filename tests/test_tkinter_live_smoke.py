...
def _live_repeating_after_smoke_script(expected_platform: str) -> str:
    platform_gate = _platform_gate_source(expected_platform)
    return textwrap.dedent(
        f"""
        import sys
        ...
        label.pack()
        status = {{"ticks": 0, "quit_called": False}}

        def _tick():
            status["ticks"] += 1
            label.config(text=f"tick {{status['ticks']}}")
            if status["ticks"] >= 30:
                status["quit_called"] = True
                root.quit()
                return
        ...
        ok = status["ticks"] >= 30 and status["quit_called"] is True
        print("OK" if ok else "FAIL")
        print(f"DETAIL:platform={{sys.platform}} status={{status!r}}")
        if not ok:
            raise SystemExit(3)
...
def _run_live_repeating_after_smoke(expected_platform: str) -> None:
    _require_live_smoke_prereqs(expected_platform)
    base_tmp = EXT_ROOT / "tmp"
    base_tmp.mkdir(parents=True, exist_ok=True)
    run_dir = Path(
        tempfile.mkdtemp(
            prefix=f"tk_repeating_after_{expected_platform}_", dir=str(base_tmp)
        )
    )
    script_path = run_dir / "live_repeating_after_smoke.py"
    out_dir = run_dir / "out"
    output = out_dir / f"tk_repeating_after_{expected_platform}_molt"
    script_path.write_text(_live_repeating_after_smoke_script(expected_platform))
    ...
    assert build.returncode == 0, (
        f"live tkinter repeating-after smoke build failed for {expected_platform}\n"
        f"stdout:\n{build.stdout}\n\nstderr:\n{build.stderr}"
    )
    ...
    if first_line.startswith("SKIP:"):
        pytest.skip(first_line)
    assert run.returncode == 0, (
        f"live tkinter repeating-after smoke run failed for {expected_platform}\n"
        f"stdout:\n{run.stdout}\n\nstderr:\n{run.stderr}"
    )
    assert any(line == "OK" for line in run.stdout.splitlines()), (
        f"live tkinter repeating-after smoke did not report success for {expected_platform}\n"
        f"stdout:\n{run.stdout}\n\nstderr:\n{run.stderr}"
    )
...
@pytest.mark.skipif(sys.platform != "darwin", reason="macOS-only live Tk smoke")
def test_tkinter_live_repeating_after_smoke_macos() -> None:
    _run_live_repeating_after_smoke("darwin")


@pytest.mark.skipif(
    not sys.platform.startswith("linux"), reason="Linux-only live Tk smoke"
)
def test_tkinter_live_repeating_after_smoke_linux() -> None:
    _run_live_repeating_after_smoke("linux")


@pytest.mark.skipif(sys.platform != "win32", reason="Windows-only live Tk smoke")
def test_tkinter_live_repeating_after_smoke_windows() -> None:
    _run_live_repeating_after_smoke("win32")
