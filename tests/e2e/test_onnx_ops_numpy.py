...
def numpy_conv2d(x, w, bias=None, stride=1, padding=0, groups=1):
    ...
    if groups == 1:
        ...
        for kh in range(KH):
            for kw in range(KW):
                for c in range(C):
                    ...
                    for oh in range(OH):
                        for ow in range(OW):
                            cols[:, col_idx, oh*OW+ow] = x[:, c, oh*stride+kh, ow*stride+kw]
        ...
        for g in range(groups):
            ...
            out[:, g*oc_per_group:(g+1)*oc_per_group] = numpy_conv2d(x_g, w_g, stride=stride)
...
def numpy_batch_norm(x, scale, bias, mean, var, eps=1e-5):
    ...
    return (x - mean.reshape(1,-1,1,1)) / np.sqrt(var.reshape(1,-1,1,1) + eps) * scale.reshape(1,-1,1,1) + bias.reshape(1,-1,1,1)
...
def test_conv2d_vs_onnxruntime():
    ...
    conv_node = make_node('Conv', ['X', 'W', 'B'], ['Y'], kernel_shape=[3,3], pads=[1,1,1,1])
    ...
    print(f'Conv2d: max_diff = {max_diff:.2e} ({"PASS" if max_diff < 1e-4 else "FAIL"})')
...
def test_batch_norm_vs_onnxruntime():
    ...
    bn_node = make_node('BatchNormalization', ['X','scale','bias','mean','var'], ['Y'], epsilon=1e-5)
    ...
    max_diff = np.abs(our_out - ort_out).max()
    print(f'BatchNorm: max_diff = {max_diff:.2e} ({"PASS" if max_diff < 1e-4 else "FAIL"})')
    return max_diff < 1e-4
...
def test_bn_folding_correctness():
    ...
    print(f'BN folding: max_diff = {max_diff:.2e} ({"PASS" if max_diff < 1e-5 else "FAIL"})')
