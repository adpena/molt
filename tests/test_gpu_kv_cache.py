...
def test_turboquant_cache_builds_runtime_shadow_metadata():
    from molt.gpu.kv_cache import TurboQuantAttentionKVCache
    from molt.gpu.tensor import Tensor
    from molt.gpu.turboquant import TurboQuantCodec
    ...
    assert cache._runtime_mse_signs.shape == (8,)
    assert cache._runtime_qjl_signs.shape == (8,)
    assert cache._runtime_key_mse_weight_rows.shape == (1, 1, 2, 8)
...
def test_turboquant_attention_reference_uses_runtime_shadow_metadata():
    ...
    baseline = cache._attention_reference(q, None, 1.0)
...
def test_native_turboquant_attention_uses_webgpu_backend(tmp_path: Path) -> None:
    root = Path(__file__).resolve().parents[1]
    probe = tmp_path / "gpu_turboquant_webgpu_native.py"
    probe.write_text(
        "from molt.gpu.kv_cache import TurboQuantAttentionKVCache\n"
        "from molt.gpu.tensor import Tensor\n"
        "from molt.gpu.turboquant import TurboQuantCodec\n"
