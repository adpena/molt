...
def _run_molt(source: str, env_overrides: dict[str, str] | None = None,
    ...
    try:
        ...
        return subprocess.CompletedProcess(
            args=["molt", "run", path], returncode=-1,
...
def _run_python(source: str, timeout: int = 10) -> subprocess.CompletedProcess:
    ...
    try:
        ...
        return subprocess.CompletedProcess(
            args=[sys.executable, path], returncode=-1,
            stdout="", stderr="TIMEOUT",
        )
    finally:
        os.unlink(path)
...
def test_resource_env_vars_set():
    ...
    m = CapabilityManifest(resources=ResourceLimits(
        max_memory=1048576, max_duration=5.0,
        max_allocations=1000, max_recursion_depth=50,
    ))
    env = m.to_env_vars()
