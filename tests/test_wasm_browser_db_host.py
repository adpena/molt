...
def test_browser_kernel_facade_roundtrips_float32_typed_arrays(tmp_path: Path) -> None:
    if shutil.which("node") is None:
        pytest.skip("node is required for browser kernel typed-array test")
    if shutil.which("cargo") is None:
        pytest.skip("cargo is required for browser kernel typed-array test")

    root = Path(__file__).resolve().parents[1]
    src = tmp_path / "browser_typed_array_forward.py"
    src.write_text(
        "def forward(raw: bytes, n: int):\n"
        "    print(n)\n"
        "    return raw\n",
        encoding="utf-8",
    )
    ...
    try:
        base_url = f"http://127.0.0.1:{server.server_address[1]}"
        browser_host_uri = (root / "wasm" / "browser_host.js").as_uri()
        script = tmp_path / "run_browser_kernel_typed_array.mjs"
        script.write_text(
            f"""
import {{ loadMoltKernel }} from '{browser_host_uri}';
...
const kernel = await loadMoltKernel({{
  ...
  runtimeUrl: `${{baseUrl}}/molt_runtime.wasm`,
  preferLinked: false,
  exportName: 'browser_typed_array_forward__forward',
  resultType: 'float32',
}});
