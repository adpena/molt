...
def test_run_native_partial_link_command_ignores_sysroot_for_relocatable_merge(
    tmp_path: Path, monkeypatch
) -> None:
    input_a = tmp_path / "a.o"
    input_b = tmp_path / "b.o"
    input_a.write_bytes(b"a")
    input_b.write_bytes(b"b")
    output_path = tmp_path / "merged.o"
    ...
    assert captured_link_cmd[0] == "clang"
    assert "-Wl,-r" in captured_link_cmd
    assert str(sysroot) not in captured_link_cmd
    assert "-o" in captured_link_cmd
    assert str(output_path) in captured_link_cmd
    assert captured_link_cmd[-2:] == [str(input_a), str(input_b)]
