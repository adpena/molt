...
def test_default_store_layout_is_project_scoped() -> None:
    ...
    assert paths.resolve_symphony_parent_root(env) == Path(
        "/Volumes/APDataStore/symphony"
    )
    assert paths.resolve_symphony_store_root(env) == Path(
        "/Volumes/APDataStore/symphony/molt"
    )
    assert paths.symphony_log_root(env) == Path(
        "/Volumes/APDataStore/symphony/molt/logs"
    )
    assert paths.symphony_state_root(env) == Path(
        "/Volumes/APDataStore/symphony/molt/state"
    )
    assert paths.symphony_workspace_root(env) == Path(
        "/Volumes/APDataStore/symphony/molt/sessions/workspaces"
    )
    assert paths.symphony_tool_promotion_events_file(env) == Path(
        "/Volumes/APDataStore/symphony/molt/state/tool_promotion/events.jsonl"
    )
    assert paths.symphony_tool_promotion_distillations_dir(env) == Path(
...
def test_is_within_detects_cross_project_escape() -> None:
    ...
    assert (
        paths.is_within(Path("/Volumes/APDataStore/symphony/molt/logs"), parent) is True
    )
