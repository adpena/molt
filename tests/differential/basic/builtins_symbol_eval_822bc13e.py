"""Purpose: CPython 3.12+ builtins semantic probe for symbol `eval`."""
...
def _eval_expr_case() -> object:
    return SYMBOL("sum(x * x for x in range(6))")


def _eval_locals_case() -> object:
    glb = {"base": 10}
    loc = {"delta": 7}
    return SYMBOL("base + delta", glb, loc)


_record("target_eval_expr_case", _eval_expr_case)
_record("target_eval_locals_case", _eval_locals_case)
