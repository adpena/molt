...
"""Purpose: differential coverage for WebSocket masked text frame."""
...
def server() -> None:
    ...
    reply = build_frame(b"ack:" + payload, opcode=1, mask=False)
    conn.sendall(reply)
    conn.close()
    srv.close()
...
frame = build_frame(b"hello", opcode=1, mask=True)
sock.sendall(frame)
_fin, opcode, masked, payload = read_frame(sock)
sock.close()
t.join(timeout=1.0)

print(server_seen[0])
print(masked, opcode, payload.decode("ascii", errors="replace"))
