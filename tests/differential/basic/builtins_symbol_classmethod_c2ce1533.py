"""Purpose: CPython 3.12+ builtins semantic probe for symbol `classmethod`."""
import builtins
import contextlib
import hashlib
import inspect
...
def _classmethod_case() -> object:
    class FX:
        factor = 3

        @SYMBOL
        def convert(cls, amount: int) -> int:
            return cls.factor * amount

    return FX.convert(7)


_record("target_classmethod_case", _classmethod_case)
