"""Purpose: CPython 3.12+ builtins semantic probe for symbol `breakpoint`."""
import builtins
import contextlib
import hashlib
import inspect
...
def _breakpoint_hook_case() -> object:
    hits: list[str] = []

    def _hook(*args: object, **kwargs: object) -> None:
        hits.append(f"args={len(args)} kwargs={len(kwargs)}")
...
_record("target_breakpoint_hook_case", _breakpoint_hook_case)
