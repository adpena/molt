"""Purpose: CPython 3.12+ builtins semantic probe for symbol `globals`."""
import builtins
import contextlib
import hashlib
import inspect
...
def _globals_case() -> object:
    g = SYMBOL()
    subset = {k: type(v).__name__ for k, v in g.items() if k in {"NAME", "SYMBOL", "_record"}}
    return subset


_record("target_globals_case", _globals_case)
