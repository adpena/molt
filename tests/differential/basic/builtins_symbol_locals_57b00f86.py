"""Purpose: CPython 3.12+ builtins semantic probe for symbol `locals`."""
import builtins
import contextlib
import hashlib
import inspect
...
def _locals_case() -> object:
    token = "local-token"
    local_map = SYMBOL()
    return {
        "token": local_map.get("token", token),
        "has_local_map": "local_map" in local_map,
    }


_record("target_locals_case", _locals_case)
