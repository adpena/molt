"""Purpose: CPython 3.12+ builtins semantic probe for symbol `open`."""
import builtins
import contextlib
import hashlib
import inspect
...
def _open_roundtrip_case() -> object:
    with tempfile.TemporaryDirectory() as td:
        path = Path(td) / "report.txt"
        with SYMBOL(path, "w", encoding="utf-8", newline="") as handle:
            handle.write("row-1\nrow-2\n")
        with SYMBOL(path, "r", encoding="utf-8") as handle:
            return handle.read().splitlines()


_record("target_open_roundtrip_case", _open_roundtrip_case)
