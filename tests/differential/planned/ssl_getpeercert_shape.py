...
"""Purpose: differential coverage for getpeercert shape."""
...
peer_bin = wrapped.getpeercert(binary_form=True)
wrapped.recv(2)
wrapped.close()

t.join(timeout=1.0)

print(isinstance(peer, dict), isinstance(peer_bin, (bytes, bytearray)))
print(sorted(peer.keys())[:3])
