...
def test_project_name_for_issue_uses_title_prefix_overrides() -> None:
    assert (
        linear_hygiene._project_name_for_issue(
            {"title": "symphony: role routing cleanup"}, {}
        )
        == "Tooling & DevEx"
    )
    assert (
...
def test_dspy_runtime_status_uses_custom_api_key_env(
    ...
    monkeypatch.setenv("MOLT_SYMPHONY_DSPY_ENABLE", "1")
    monkeypatch.setenv("MOLT_SYMPHONY_DSPY_MODEL", "openai/gpt-4.1-mini")
    monkeypatch.setenv("MOLT_SYMPHONY_DSPY_API_KEY_ENV", "ALT_KEY")
...
def test_dspy_route_decision_falls_back_when_not_ready(
    ...
    monkeypatch.setenv("MOLT_SYMPHONY_DSPY_ENABLE", "1")
    monkeypatch.delenv("MOLT_SYMPHONY_DSPY_MODEL", raising=False)
