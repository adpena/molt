...
_invalid_codepoints: frozenset[int] = frozenset(
    {
        # 0x0001 to 0x0008
        0x1,
        0x2,
...
def _replace_charref(s: _re.Match) -> str:
    s_inner = s.group(1)
    if s_inner[0] == "#":
        # numeric charref
        if s_inner[1] in "xX":
            num = int(s_inner[2:].rstrip(";"), 16)
        else:
            num = int(s_inner[1:].rstrip(";"))
        ...
        # named charref
        if s_inner in _html5:
            return _html5[s_inner]
        # find the longest matching name (as defined by the standard)
        for x in range(len(s_inner) - 1, 1, -1):
            if s_inner[:x] in _html5:
                return _html5[s_inner[:x]] + s_inner[x:]
        return "&" + s_inner
