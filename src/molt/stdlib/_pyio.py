...
def _load_text_io_classes():
    context = type(sys)("_pyio_text_context")
    context.BytesIO = BytesIO
    context.IOBase = IOBase
    context.SEEK_CUR = SEEK_CUR
    context.SEEK_END = SEEK_END
    context.UnsupportedOperation = UnsupportedOperation
    context._CHECK_ERRORS = _CHECK_ERRORS
    context.text_encoding = text_encoding
    sys.modules["_pyio_text_context"] = context

    module_dir = os.path.dirname(__file__) if "__file__" in globals() else None
    inserted_module_dir = False
    if module_dir and module_dir not in sys.path:
        sys.path.insert(0, module_dir)
        inserted_module_dir = True
    try:
        sys.modules.pop("_pyio_text", None)
...
_TEXT_IO_CLASSES = _load_text_io_classes()
IncrementalNewlineDecoder = _TEXT_IO_CLASSES.IncrementalNewlineDecoder
StringIO = _TEXT_IO_CLASSES.StringIO
TextIOBase = _TEXT_IO_CLASSES.TextIOBase
TextIOWrapper = _TEXT_IO_CLASSES.TextIOWrapper
del _TEXT_IO_CLASSES, _load_text_io_classes
