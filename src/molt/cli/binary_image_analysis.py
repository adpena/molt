...
def _backend_ir_op_source_site(
    ...
    if line is None and op.get("kind") == "line":
        line = _positive_int(op.get("value"))
        source = "line_marker"
    if line is None:
        return None, "missing"
    return (
        {
            "line": line,
            "col": _nonnegative_int(op.get("col_offset")),
            "end_col": _nonnegative_int(op.get("end_col_offset")),
        },
        source,
    )
...
def _backend_ir_source_site_payload(
    ...
    *,
    ...
    for func in functions:
        ...
        for op_index, op in enumerate(ops):
            if not isinstance(op, Mapping):
                continue
            site, source = _backend_ir_op_source_site(op)
            if site is None:
                continue
            kind = op.get("kind")
    ...
    return {
        ...
        "source_site_digest": _stable_payload_hash(site_records),
...
def _backend_ir_binary_image_analysis_payload(ir: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for func in functions:
        ...
        for op in ops:
            ...
            if kind == "call":
                call_op_count += 1
            if _backend_ir_op_source_site(op)[0] is not None:
