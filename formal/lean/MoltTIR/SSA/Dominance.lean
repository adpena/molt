...
/-- Path concatenation: given a path from a to b and a path from b to c,
    produce a path from a to c whose elements come from the two inputs. -/
...
/-- Suffix extraction: if d ∈ path from src to dst, there is a sub-path
    from d to dst whose elements are in the original path. -/
...
/-- Suffix with length bound: if d ∈ path and d ≠ src, there is a strictly
    shorter sub-path from d to dst. -/
  ...
  | cons l₁ l₂ dst' rest hedge htail ih =>
    ...
    · by_cases hd_eq : d = l₂
      ...
      · obtain ⟨sp, hsp, hlen⟩ := ih hd' hd_eq
        exact ⟨sp, hsp, by simp; omega⟩

/-- A dominator of a reachable node is itself reachable. -/
...
/-- Dominator chain property: for any reachable l, if a dominates l and
    b dominates l, then a dominates b or b dominates a (Prosser 1959).

    Proof by contradiction + well-founded descent on b→l path length.
    If ¬(a dom b), there is a path Pb: entry→b avoiding a. Similarly
    ¬(b dom a) gives Pa: entry→a avoiding b. For any b→l path containing
    a, extract the suffix from a (strictly shorter), concatenate Pa + suffix
    to reach l, discover b in that suffix (since b dom l and b ∉ Pa),
    extract a b→l sub-suffix (even shorter). The new b→l path is at least
    2 steps shorter. If a is still present, descend; if not, concatenate
    Pb + new path to get entry→l avoiding a — contradicting a dom l. -/
private theorem dom_chain {f : Func} {a b l : Label}
  ...
  push_neg at hc
  obtain ⟨hna, hnb⟩ := hc
  simp only [Dom] at hna hnb
  push_neg at hna hnb
  obtain ⟨hreach_b, pb, hpb, ha_notin_pb⟩ := hna
  obtain ⟨hreach_a, pa, hpa, hb_notin_pa⟩ := hnb
  have hab : a ≠ b := by
    intro heq; subst heq; exact hna ⟨hreach_a, pa, hpa, hb_notin_pa⟩
  have hba : b ≠ a := fun h => hab h.symm
  -- Get a b→l path from a path through l
  ...
  -- Key: no b→l path of length ≤ n can contain a.
  ...
  | succ k ih =>
    ...
    -- a ∈ sp, a ≠ b. Suffix from a is strictly shorter.
    obtain ⟨sal, hsal, hsal_len⟩ := CFGPath.suffix_shorter hsp ha_in hab
    -- Concat Pa (entry→a, no b) + sal (a→l)
    ...
    · -- b ∈ sal, b ≠ a. Suffix from b is even shorter.
      obtain ⟨sbl, hsbl, hsbl_len⟩ := CFGPath.suffix_shorter hsal hb_sal hba
      -- sbl is b→l with |sbl| < |sal| < |sp| ≤ k+1, so |sbl| ≤ k-1 ≤ k
      -- By IH: a ∉ sbl
      have ha_notin_sbl := ih sbl hsbl (by omega)
      -- Concat Pb + sbl → entry→l avoiding a
...
-- ── Maximum of a finite chain of strict dominators ───────────────

/-- Given a nonempty list of strict dominators of a reachable l (that
    covers all strict dominators), find the immediate dominator by
    induction on the list. The chain property ensures that at each step
    we can compare the current element with the running best and keep
    the one that is "closer to l" (i.e., dominated by the other). -/
private theorem find_idom (f : Func) (l : Label)
    ...
    ∀ (d : Label), SDom f d l →
      ...
      ∃ d', ImmDom f d' l := by
  intro d hd hd_best
  -- d is an idom candidate. Show the idom condition for ALL strict dominators.
  refine ⟨d, hd, fun d' hd' hne => ?_⟩
  -- d' sdom l, d' ≠ d. d' is reachable, hence in hS.
...
/-- Auxiliary: scan a list of labels and maintain a "best" candidate.
    Given the current best d (sdom l, dominated by all previously seen
    sdoms), process each element of the list. If the element sdom l,
    compare with d using the chain property and keep the deeper one. -/
private theorem scan_candidates (f : Func) (l : Label)
    ...
    ∀ (candidates : List Label) (d : Label),
      SDom f d l →
      -- d is dominated by all sdom-of-l elements NOT in candidates
  ...
  | nil =>
    intro d hd hbest
    -- No candidates left: hbest covers all of hS_full
  ...
  | cons c rest ih =>
    ...
    · -- c sdom l. By chain property: d dom c or c dom d.
      have hd_reach := dom_reachable hreach hd.1
      have hc_reach := dom_reachable hreach hc_sdom.1
      rcases dom_chain hreach hd.1 hc_sdom.1 with hd_dom_c | hc_dom_d
      · -- d dom c: d is "above" c, so c is deeper (closer to l).
        -- If d = c, keep d (= c).
        by_cases hdc : d = c
        · -- d = c. Continue with d.
          exact ih d hd (fun d' hd' hd'_in hd'_notin =>
            hbest d' hd' hd'_in (fun hmem =>
              rcases List.mem_cons.mp hmem with heq | hrest =>
                hd'_notin (heq ▸ hrest) |>.elim -- wait, this is wrong
              sorry))
        · -- d ≠ c. SDom f d c (d dom c ∧ d ≠ c).
          have hd_sdom_c : SDom f d c := ⟨hd_dom_c, hdc⟩
          -- c is a better candidate. Switch to c.
          -- Need: ∀ d' sdom l, d' ∈ hS_full, d' ∉ rest → d' = c ∨ SDom f d' c
          exact ih c hc_sdom (fun d' hd' hd'_in hd'_notin_rest => by
            -- d' is not in rest. Is d' = c?
            by_cases hd'c : d' = c
            · exact .inl hd'c
            · -- d' ≠ c. d' ∉ c :: rest, so d' ∉ candidates.
              -- Wait, d' ∉ rest but could d' = c? No, hd'c says d' ≠ c.
              have hd'_notin_cands : d' ∉ c :: rest := by
                simp [List.mem_cons]; exact ⟨hd'c, hd'_notin_rest⟩
              rcases hbest d' hd' hd'_in hd'_notin_cands with heq | hsdom
              · -- d' = d. Then SDom f d' c = SDom f d c = hd_sdom_c.
                exact .inr (heq ▸ hd_sdom_c)
              · -- SDom f d' d. By transitivity: SDom f d' c.
                exact .inr (SDom.trans hc_reach hsdom hd_sdom_c))
      · -- c dom d: c is "above" d, so d is deeper. Keep d.
        by_cases hcd : c = d
        · -- c = d. Continue with d.
        ...
        · -- c ≠ d. SDom f c d.
          ...
          exact ih d hd (fun d' hd' hd'_in hd'_notin_rest => by
            ...
            · have : d' ∉ c :: rest := by
                simp [List.mem_cons]; exact ⟨hd'c, hd'_notin_rest⟩
              exact hbest d' hd' hd'_in this)
    · -- c does not sdom l. Skip c.
      exact ih d hd (fun d' hd' hd'_in hd'_notin_rest => by
        by_cases hd'c : d' = c
        · -- d' = c but c doesn't sdom l. d' sdom l. Contradiction.
...
theorem domTree_is_tree (f : Func) (l : Label)
  ...
  exact scan_candidates f l hreach S hS S f.entry h_entry_sdom
    (fun _ _ _ hnotin => absurd (hS _ ‹_›) hnotin |>.elim)
