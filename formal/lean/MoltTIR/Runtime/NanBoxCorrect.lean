...
theorem ptr_roundtrip (addr : UInt64) (hrange : addr &&& POINTER_MASK = addr) :
  ...
  unfold fromNanBox
  split
  · -- First if is true: bits &&& QNAN ≠ QNAN. But we know it equals QNAN. Contradiction.
    ...
    · split
      ...
      · split
        ...
        · split
          ...
          · rename_i _ _ _ _ hne
            exact absurd (fromPtr_isPtr_aux addr) hne

/-- Key helper: the payload toNat for the non-negative case. -/
private theorem int_payload_toNat_nonneg (n : Int) (hlo : 0 ≤ n) (hhi : n < 2^46) :
    (UInt64.mk (BitVec.ofInt 64 n) &&& INT_MASK).toNat = n.toNat := by
  rw [UInt64.toNat_and, UInt64.toNat_mk, BitVec.toNat_ofInt]
  have hint_mask_val : INT_MASK.toNat = 140737488355327 := by native_decide
  rw [hint_mask_val]
  -- (n % 2^64).toNat &&& 140737488355327
  -- Since 0 ≤ n < 2^46: n % 2^64 = n, so (n % 2^64).toNat = n.toNat
  have hmod : n % (2 ^ 64 : Int) = n := by omega
  rw [hmod]
  -- n.toNat &&& (2^47 - 1) = n.toNat since n.toNat < 2^46 < 2^47
  have h47 : (140737488355327 : Nat) = 2^47 - 1 := by norm_num
  rw [h47]
  have hnat : n.toNat < 2^47 := by omega
  omega

/-- Key helper: the sign bit is clear for non-negative n. -/
private theorem int_sign_clear_nonneg (n : Int) (hlo : 0 ≤ n) (hhi : n < 2^46) :
    ¬((UInt64.mk (BitVec.ofInt 64 n) &&& INT_MASK) &&& INT_SIGN ≠ 0) := by
  simp only [not_not]
  -- payload.toNat = n.toNat < 2^46
  -- INT_SIGN = 2^46
  -- payload &&& INT_SIGN = 0 iff payload.toNat / 2^46 % 2 = 0
  -- Since payload.toNat < 2^46, this is 0.
  apply uint64_eq_of_toNat_eq
  rw [UInt64.toNat_and]
  have hpay := int_payload_toNat_nonneg n hlo hhi
  rw [hpay]
  have hint_sign_val : INT_SIGN.toNat = 70368744177664 := by native_decide
  rw [hint_sign_val]
  have h46 : (70368744177664 : Nat) = 2^46 := by norm_num
  rw [h46]
  have hnat : n.toNat < 2^46 := by omega
  -- n.toNat &&& 2^46 = 0 since n.toNat < 2^46
  omega

/-- Key helper: the payload toNat for the negative case. -/
private theorem int_payload_toNat_neg (n : Int) (hlo : -(2^46) ≤ n) (hhi : n < 0) :
    (UInt64.mk (BitVec.ofInt 64 n) &&& INT_MASK).toNat = (n + 2^47).toNat := by
  rw [UInt64.toNat_and, UInt64.toNat_mk, BitVec.toNat_ofInt]
  have hint_mask_val : INT_MASK.toNat = 140737488355327 := by native_decide
  rw [hint_mask_val]
  -- n % 2^64: since n < 0, this is n + 2^64 (for n ≥ -2^46 > -2^64)
  have hmod : n % (2 ^ 64 : Int) = n + 2^64 := by omega
  rw [hmod]
  -- (n + 2^64).toNat &&& (2^47 - 1)
  have h47 : (140737488355327 : Nat) = 2^47 - 1 := by norm_num
  rw [h47]
  -- (n + 2^64).toNat ≥ 2^64 - 2^46
  -- Low 47 bits: (n + 2^64).toNat % 2^47 = (n + 2^47).toNat (since 2^64 = 2^47 * 2^17)
  have hnn : (n + 2^64).toNat = (n + 2^64).toNat := rfl
  -- (n + 2^64) mod 2^47 = (n + 2^47) mod 2^47 = n + 2^47 (since 0 < n + 2^47 < 2^47)
  have hrange_pos : 0 < n + 2^47 := by omega
  have hrange_lt : n + 2^47 < 2^47 := by omega
  -- n + 2^64 = (n + 2^47) + 2^64 - 2^47 = (n + 2^47) + (2^17 - 1) * 2^47
  have hnat_eq : (n + 2^64).toNat = (n + 2^47).toNat + (2^17 - 1) * 2^47 := by omega
  rw [hnat_eq]
  -- ((n + 2^47).toNat + k * 2^47) &&& (2^47 - 1) = (n + 2^47).toNat &&& (2^47 - 1)
  -- = (n + 2^47).toNat since (n + 2^47).toNat < 2^47
  omega

/-- Key helper: the sign bit is set for negative n. -/
private theorem int_sign_set_neg (n : Int) (hlo : -(2^46) ≤ n) (hhi : n < 0) :
    (UInt64.mk (BitVec.ofInt 64 n) &&& INT_MASK) &&& INT_SIGN ≠ 0 := by
  intro h
  have hpay := int_payload_toNat_neg n hlo hhi
  have h0 := congrArg UInt64.toNat h
  rw [UInt64.toNat_and] at h0
  rw [hpay] at h0
  have hint_sign_val : INT_SIGN.toNat = 70368744177664 := by native_decide
  rw [hint_sign_val] at h0
  have h46 : (70368744177664 : Nat) = 2^46 := by norm_num
  rw [h46] at h0
  -- (n + 2^47).toNat &&& 2^46 = 0
  -- But (n + 2^47).toNat ≥ 2^46 (since n ≥ -2^46) and < 2^47
  -- So bit 46 is set. Contradiction.
  have hge : (n + 2^47).toNat ≥ 2^46 := by omega
  have hlt : (n + 2^47).toNat < 2^47 := by omega
  have : (0 : UInt64).toNat = 0 := by native_decide
  rw [this] at h0
  omega
...
theorem int_roundtrip (n : Int) (hrange : -2^46 ≤ n ∧ n < 2^46) :
  ...
  -- Step 1: unfold toNanBox, then step through fromNanBox's if-chain
  simp only [toNanBox]
  unfold fromNanBox
  -- Let raw := UInt64.mk (BitVec.ofInt 64 n)
  -- bits := QNAN ||| TAG_INT ||| (raw &&& INT_MASK)
  -- Step through the if-chain using split
  split
  · -- bits &&& QNAN ≠ QNAN — contradiction (QNAN is set)
    rename_i hne
    exact absurd (int_encoded_qnan (UInt64.mk (BitVec.ofInt 64 n))) hne
  · split
    · -- bits &&& TAG_CHECK = QNAN ||| TAG_INT — correct branch!
      -- Goal: the sign-extension extraction recovers n
      -- payload = bits &&& INT_MASK = raw &&& INT_MASK (by int_payload_extract)
      -- We need to show the if-then-else sign extension produces n.
      -- Rewrite payload extraction
      have hpayload := int_payload_extract (UInt64.mk (BitVec.ofInt 64 n))
      -- Now we need to case-split on the sign bit
      -- payload = UInt64.mk (BitVec.ofInt 64 n) &&& INT_MASK
      -- Let's work with the payload directly
      rw [hpayload]
      -- Now the goal involves:
      --   let payload := UInt64.mk (BitVec.ofInt 64 n) &&& INT_MASK
      --   if payload &&& INT_SIGN ≠ 0 then
      --     some (.int (payload.toNat - 2^47))
      --   else
      --     some (.int payload.toNat)
      -- = some (.int n)
      -- Case split on sign of n
      obtain ⟨hlo, hhi⟩ := hrange
      -- Key fact: payload.toNat = (n % 2^64).toNat &&& INT_MASK.toNat
      -- = (n % 2^64).toNat mod 2^47 (since INT_MASK = 2^47 - 1)
      -- For n ≥ 0: payload.toNat = n.toNat, sign bit clear
      -- For n < 0: payload.toNat = n + 2^47, sign bit set
      sorry
...
theorem nanbox_roundtrip (v : Value)
  ...
  | int n =>
    ...
    sorry
  | ptr addr =>
    exact ptr_roundtrip addr hrange
